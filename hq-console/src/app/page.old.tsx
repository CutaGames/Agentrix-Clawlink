...
export default function CEOConsole() {
  ...
  const loadWorkspaceInfo = async () => {
    ...
    try {
      const [infoRes, treeRes] = await Promise.all([
        fetch(`${hqBaseUrl}/hq/workspace/info`),
        fetch(`${hqBaseUrl}/hq/workspace/tree?depth=2`)
      ]);
  ...
  const handleSaveKb = async () => {
    ...
    const hqBaseUrl = '/api';
    try {
      await fetch(`${hqBaseUrl}/hq/knowledge-base`, {
        method: 'POST',
  ...
  const handleSend = async () => {
    ...
    const hqBaseUrl = '/api';
    try {
      const response = await fetch(`${hqBaseUrl}/hq/chat`, {
        method: 'POST',
        headers: {

...
用户支付 → 智能合约 → 自动分账 → 商户MPC钱包
                ↓
          PayMind Treasury（分佣，可为0）
```

**关键点**：
- ✅ 资金**始终在智能合约中**，PayMind 从未"持有"资金
- ✅ 分账由**智能合约自动执行**，PayMind 无法干预
- ✅ PayMind 只是**技术服务商**，不涉及资金托管

### 2. 非托管证明

**技术保障**：
1. **MPC 钱包 2/3 签名**：PayMind 只持有 1/3 私钥分片，无法单独动用资金
2. **智能合约自动分账**：分账规则在链上，PayMind 无法修改或干预
3. **资金流向透明**：所有分账操作都在链上可查，完全透明

**法律保障**：
1. **TOS 明确声明**：PayMind 仅为技术服务商，不托管资金
...
- **PayMind**：收取 0.1% 服务费（非托管模式）

**区别**：
- Stripe：资金先进入 Stripe 账户，再分账给商户（托管）
- PayMind：资金在智能合约中，自动分账给商户（非托管）
...
**风险 1**：可能被认定为"资金处理商"
- **缓解**：即使分佣为 0，PayMind 也不处理资金，只是提供技术服务

**风险 2**：可能被认定为"支付处理商"
- **缓解**：实际支付处理由 Provider 完成，PayMind 只提供路由和聚合

**风险 3**：可能被认定为"资金托管"
- **缓解**：
  - 资金在智能合约中，不在 PayMind 账户
  - PayMind 只持有 1/3 私钥，无法单独动用资金
  - 分账由智能合约自动执行，PayMind 无法干预
...
PAYMIND_OFF_RAMP_RATE=0.001  # 0.1%
# 或
...
constructor(private configService: ConfigService) {
  ...
  this.PAYMIND_OFF_RAMP_RATE = customRate !== undefined ? customRate : 0.001;
...
struct SplitConfig {
    // ...
    uint256 offRampFee;  // PayMind Off-ramp分佣（可为0）
    // ...
}

function _autoSplit(bytes32 orderId, uint256 totalAmount) internal {
    // ...
    // 分账到PayMind Treasury（Off-ramp分佣，可为0）
...
2. **分佣可以自定义**
   - 通过环境变量 `PAYMIND_OFF_RAMP_RATE` 配置
   - 可以设为 0（不收取服务费）

...
本指南将帮助您安装 PayMind 项目所需的所有依赖和软件。
...
- **前端依赖**: `paymindfrontend/package.json`
...
#### 或使用 Docker:
```bash
docker run --name paymind-postgres \
  -e POSTGRES_PASSWORD=paymind123 \
  -e POSTGRES_USER=paymind \
  -e POSTGRES_DB=paymind \
  -p 5432:5432 \
  -d postgres
```
...
使用 Docker:
```bash
docker run --name paymind-redis -p 6379:6379 -d redis
```
...
#### 前端配置 (`paymindfrontend/.env.local`):
```bash
cd paymindfrontend

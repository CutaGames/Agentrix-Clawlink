...
# Agentrix V2.2 自动安装和配置脚本

set -e  # 遇到错误立即退出

echo "🚀 Agentrix V2.2 自动安装和配置"
echo "=================================="
echo ""
...
EOF
    ...
    echo -e "${GREEN}✅ agentrixfrontend/.env.local 已存在${NC}"
fi
...
if [ ! -d "node_modules" ]; then
    ...
    echo "   如需重新安装，请运行: cd agentrixfrontend && rm -rf node_modules && npm install"
fi
...
if command -v psql &> /dev/null; then
    ...
    echo "   创建数据库命令:"
    echo "   psql -U postgres -c 'CREATE DATABASE agentrix;'"
else

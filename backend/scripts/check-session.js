...
// 写一个简单的 heartbeat，确认脚本有执行
const heartbeatPath = path.resolve(__dirname, 'session-output.json');
fs.writeFileSync(
  heartbeatPath,
  JSON.stringify({ status: 'running', timestamp: Date.now() }, null, 2),
);
...
async function main() {
  ...
  if (!contractAddress) {
    console.error('请在 backend/.env 中配置 ERC8004_CONTRACT_ADDRESS');
    process.exit(1);
  }

...
sequenceDiagram
    ...
    participant X402Net as X402 网络
    participant Chain as BSC 链

    %% 步骤1: 加载商品列表
    rect rgb(200, 230, 200)
        Note over User, DB: 步骤1: 加载 X402 商品列表（无需登录）
        User->>Frontend: 访问 X402 专区
        Frontend->>Backend: GET /api/products?type=x402
        Backend->>DB: 查询 x402Enabled 商品
        DB-->>Backend: 返回商品列表
        Backend-->>Frontend: 商品数据
        Frontend-->>User: 展示商品卡片
    end

    %% 步骤2: 自动获取
    rect rgb(200, 220, 250)
        Note over User, X402Net: 步骤2: 自动获取 X402 商品（需要登录）
        User->>Frontend: 点击"自动获取 X402 商品"
        Frontend->>Backend: POST /api/products/x402/auto-fetch<br/>(带 Authorization Header)
        Backend->>X402Net: 请求 X402 服务发现端点
        X402Net-->>Backend: 返回服务元数据
        Backend->>DB: 创建/更新商品
        Backend-->>Frontend: 同步结果
        Frontend->>Backend: GET /api/products?type=x402
        Backend-->>Frontend: 更新后的商品列表
        Frontend-->>User: 刷新展示
    end

    %% 步骤3: 购买商品
    rect rgb(250, 220, 200)
        Note over User, Chain: 步骤3: 购买 X402 商品
        User->>Frontend: 选择商品并购买
        Frontend->>Backend: 创建支付订单
        Backend->>Chain: 调用 X402 支付合约
        Chain-->>Backend: 支付确认
        Backend->>DB: 记录交易
        Backend-->>Frontend: 购买成功
        Frontend-->>User: 获取服务访问权
    end
```

...
- ✅ 前端组件集成（`paymindfrontend/components/payment/StripePayment.tsx`）
...
5. **前端配置**（可选）
   ```bash
   # 在 paymindfrontend/.env.local 文件中添加
...
**当前实现**:
- ✅ 前端登录UI（`paymindfrontend/components/auth/LoginModal.tsx`）
   ...
   async socialLogin(profile: any, provider: 'google' | 'apple' | 'x') {
     ...
     if (!user) {
       ...
       user = this.userRepository.create({
         email: profile.email,
         paymindId: `pm-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
         roles: ['user'],
         // 保存OAuth provider信息
...
2. **创建数据库**
   ...
   CREATE USER paymind WITH PASSWORD 'your-password';
...
PINECONE_INDEX_NAME=paymind-products      # 如果使用Pinecone
CHROMA_URL=http://localhost:8000          # 如果使用ChromaDB
MILVUS_URL=http://localhost:19530        # 如果使用Milvus

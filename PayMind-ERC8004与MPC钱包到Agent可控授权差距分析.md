# Agentrix ERC8004 与 MPC 钱包到 Agent 可控授权差距分析
...
- `agentrixfrontend/hooks/useSessionManager.ts`
- `agentrixfrontend/components/payment/SessionManager.tsx`
...
export class MPCWallet {
  ...
  chain: string;                   // 链类型
  currency: string;                // 币种
  encryptedShardB: string;         // 加密的分片B（Agentrix持有）

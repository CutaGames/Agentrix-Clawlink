# ERC8004 合约作用说明

**版本**: V7.0  
**日期**: 2025年1月  
**标准**: ERC-8004 (Session Key 管理标准)

---

## 📋 目录

1. [合约概述](#1-合约概述)
2. [核心功能](#2-核心功能)
3. [在支付流程中的作用](#3-在支付流程中的作用)
4. [后端集成位置](#4-后端集成位置)
5. [工作流程](#5-工作流程)
6. [安全机制](#6-安全机制)
7. [部署与配置](#7-部署与配置)

---

## 1. 合约概述

### 1.1 合约信息

- **合约名称**: `ERC8004SessionManager`
- **文件位置**: `contract/contracts/ERC8004SessionManager.sol`
- **部署脚本**: `contract/scripts/deploy-erc8004.ts`
- **标准**: ERC-8004 (Session Key 管理标准)

### 1.2 设计目的

ERC8004 合约是 Agentrix QuickPay 免密支付的核心智能合约，实现：

- ✅ **Session Key 管理**: 用户创建免密授权额度
- ✅ **限额控制**: 单笔和每日限额双重保护
- ✅ **签名验证**: 链上验证 Session Key 签名
- ✅ **批量执行**: 支持批量支付，节省 Gas
- ✅ **自动重置**: 每日限额自动重置

---

## 2. 核心功能

### 2.1 Session 数据结构

```solidity
struct Session {
    address signer;           // Session Key 地址（用于签名）
    address owner;            // 主钱包地址（资金从该地址扣除）
    uint256 singleLimit;      // 单笔限额（USDC，6 decimals）
    uint256 dailyLimit;       // 每日限额（USDC，6 decimals）
    uint256 usedToday;        // 今日已用（USDC，6 decimals）
    uint256 expiry;           // 过期时间戳
    uint256 lastResetDate;    // 上次重置日期（用于每日限额重置）
    bool isActive;           // 是否激活
}
```

### 2.2 主要函数

#### `createSession()` - 创建 Session

```solidity
function createSession(
    address signer,
    uint256 singleLimit,
    uint256 dailyLimit,
    uint256 expiry
) external returns (bytes32 sessionId)
```

**功能**:
- 用户通过主钱包创建 Session
- 设置单笔和每日限额
- 设置过期时间
- 生成 Session ID (bytes32)
- 记录到链上 `sessions` mapping

**调用者**: 用户主钱包

#### `executeWithSession()` - 执行支付

```solidity
function executeWithSession(
    bytes32 sessionId,
    address to,
    uint256 amount,
    bytes32 paymentId,
    bytes calldata signature
) external onlyRelayer
```

**功能**:
- 验证 Session Key 签名
- 检查单笔限额
- 检查每日限额（自动重置）
- 从用户钱包转账 USDC 到收款地址
- 更新已用额度

**调用者**: Relayer（只有 Relayer 可以调用）

#### `executeBatchWithSession()` - 批量执行

```solidity
function executeBatchWithSession(
    bytes32[] calldata sessionIds,
    address[] calldata recipients,
    uint256[] calldata amounts,
    bytes32[] calldata paymentIds,
    bytes[] calldata signatures
) external onlyRelayer
```

**功能**:
- 批量处理多个支付
- 最多 50 笔/批次
- 节省 Gas 费用

#### `getSession()` - 查询 Session

```solidity
function getSession(bytes32 sessionId) 
    external view returns (Session memory)
```

**功能**:
- 查询 Session 详情
- 用于 Pre-Flight Check 和状态验证

#### `getUserSessions()` - 获取用户所有 Session

```solidity
function getUserSessions(address user) 
    external view returns (bytes32[])
```

**功能**:
- 获取用户的所有 Session ID
- 用于查找活跃 Session

#### `revokeSession()` - 撤销 Session

```solidity
function revokeSession(bytes32 sessionId) external
```

**功能**:
- 用户可随时撤销 Session
- 设置 `isActive = false`
- 防止后续支付

---

## 3. 在支付流程中的作用

### 3.1 QuickPay 支付流程

```
用户创建 Session
    ↓
ERC8004.createSession() (链上)
    ↓
Session 记录到链上
    ↓
用户支付时使用 Session Key 签名
    ↓
RelayerService 链下验证签名
    ↓
即时确认支付（用户无需等待）
    ↓
加入队列，异步调用 ERC8004.executeWithSession()
    ↓
链上执行转账，从用户钱包扣除 USDC
```

### 3.2 关键作用点

1. **Session 创建**: 用户授权免密额度
2. **状态查询**: Pre-Flight Check 时查询 Session 状态
3. **支付执行**: Relayer 代为执行链上支付
4. **限额控制**: 合约层面强制执行单笔和每日限额

---

## 4. 后端集成位置

### 4.1 PreflightCheckService

**文件**: `backend/src/modules/payment/preflight-check.service.ts`

**作用**: Pre-Flight Check 时查询 Session 状态

```typescript
// 初始化合约连接
private sessionManagerContract: Contract | null = null;

private initializeContracts() {
  const contractAddress = this.configService.get<string>('ERC8004_CONTRACT_ADDRESS');
  if (contractAddress) {
    this.sessionManagerContract = new Contract(
      contractAddress,
      ERC8004_ABI,
      this.provider,
    );
  }
}

// 查询用户 Session 列表
private async getUserSessions(walletAddress: string): Promise<string[]> {
  if (!this.sessionManagerContract) return [];
  return await this.sessionManagerContract.getUserSessions(walletAddress);
}

// 查询 Session 详情
private async getSessionFromChain(sessionId: string) {
  if (!this.sessionManagerContract) return null;
  return await this.sessionManagerContract.getSession(sessionId);
}
```

**使用场景**:
- 支付前检查用户是否有活跃 Session
- 验证 Session 限额是否足够
- 判断 QuickPay 是否可用

### 4.2 RelayerService

**文件**: `backend/src/modules/relayer/relayer.service.ts`

**作用**: Relayer 代为执行链上支付

```typescript
// 初始化合约连接（使用 Relayer 钱包）
private sessionManagerContract: Contract | null = null;

private initializeRelayer() {
  const contractAddress = this.configService.get<string>('ERC8004_CONTRACT_ADDRESS');
  if (contractAddress) {
    this.sessionManagerContract = new Contract(
      contractAddress,
      ERC8004_ABI,
      this.relayerWallet,  // 使用 Relayer 钱包
    );
  }
}

// 执行支付（链上）
private async executePaymentOnChain(dto: QuickPayRequest) {
  if (!this.sessionManagerContract) {
    throw new Error('Session manager contract not initialized');
  }

  const tx = await this.sessionManagerContract.executeWithSession(
    dto.sessionId,
    dto.to,
    dto.amount,
    dto.paymentId,
    dto.signature,
  );

  return tx.hash;
}
```

**使用场景**:
- 链下验证通过后，异步上链执行
- 批量处理多个支付
- 从用户钱包转账 USDC

### 4.3 SessionService

**文件**: `backend/src/modules/session/session.service.ts`

**作用**: 用户创建 Session 时调用合约

```typescript
// 创建 Session（链上）
async createSession(userId: string, dto: CreateSessionDto) {
  // 1. 获取用户钱包
  const wallet = await this.walletRepository.findOne({
    where: { userId, isDefault: true },
  });

  // 2. 生成 Session Key
  const sessionKey = Wallet.createRandom();

  // 3. 调用合约创建 Session
  const tx = await this.sessionManagerContract.createSession(
    sessionKey.address,
    parseUnits(dto.singleLimit.toString(), 6),
    parseUnits(dto.dailyLimit.toString(), 6),
    expiryTimestamp,
  );

  // 4. 等待确认
  await tx.wait();

  // 5. 保存到数据库
  const session = this.sessionRepository.create({
    userId,
    sessionId: tx.hash,
    signer: sessionKey.address,
    // ...
  });

  return this.sessionRepository.save(session);
}
```

**使用场景**:
- 用户在前端创建 QuickPay Session
- 调用合约创建链上 Session
- 保存 Session 信息到数据库

---

## 5. 工作流程

### 5.1 Session 创建流程

```
1. 用户在前端点击"创建 Session"
   ↓
2. 前端调用 SessionService.createSession()
   ↓
3. 后端生成 Session Key
   ↓
4. 调用 ERC8004.createSession() (链上)
   ↓
5. 等待链上确认
   ↓
6. 保存 Session 信息到数据库
   ↓
7. 返回 Session ID 给前端
```

### 5.2 QuickPay 支付流程

```
1. 用户点击支付
   ↓
2. PreflightCheckService 查询 Session 状态
   - 调用 ERC8004.getUserSessions()
   - 调用 ERC8004.getSession()
   ↓
3. 前端使用 Session Key 签名支付消息
   ↓
4. 调用 RelayerService.processQuickPay()
   ↓
5. 链下验证签名（毫秒级）
   ↓
6. 即时确认支付（用户无需等待）
   ↓
7. 加入队列，异步调用 ERC8004.executeWithSession()
   ↓
8. 链上执行转账
   - 验证签名
   - 检查限额
   - 转账 USDC
   - 更新已用额度
```

### 5.3 批量支付流程

```
1. 多个支付请求加入队列
   ↓
2. 定时批量处理（每 10 秒或达到 50 笔）
   ↓
3. 调用 ERC8004.executeBatchWithSession()
   ↓
4. 批量执行转账（节省 Gas）
```

---

## 6. 安全机制

### 6.1 签名验证

```solidity
// 构建消息哈希（EIP-191 标准）
bytes32 messageHash = keccak256(
    abi.encodePacked(
        "\x19Ethereum Signed Message:\n32",
        keccak256(
            abi.encodePacked(
                sessionId,
                to,
                amount,
                paymentId,
                block.chainid
            )
        )
    )
);

// 恢复签名者地址
address recoveredSigner = recoverSigner(messageHash, signature);

// 验证签名者是否为 Session Key
require(recoveredSigner == session.signer, "Invalid signature");
```

**保护措施**:
- 使用 EIP-191 标准签名
- 包含 chainId 防止跨链重放
- 包含 paymentId 防止重复支付

### 6.2 限额控制

```solidity
// 检查单笔限额
require(amount <= session.singleLimit, "Exceeds single limit");

// 检查每日限额
require(
    session.usedToday + amount <= session.dailyLimit,
    "Exceeds daily limit"
);
```

**保护措施**:
- 单笔限额：防止大额误操作
- 每日限额：防止账户被盗用
- 自动重置：每日限额每天自动重置

### 6.3 权限控制

```solidity
modifier onlyRelayer() {
    require(msg.sender == relayer, "Only relayer can execute");
    _;
}
```

**保护措施**:
- 只有 Relayer 可以执行支付
- 防止恶意调用
- Relayer 地址由合约 Owner 设置

### 6.4 重入保护

```solidity
contract ERC8004SessionManager is Ownable, ReentrancyGuard {
    function executeWithSession(...) 
        external 
        onlyRelayer 
        validSession(sessionId) 
        nonReentrant  // 重入保护
    {
        // ...
    }
}
```

**保护措施**:
- 使用 OpenZeppelin 的 `ReentrancyGuard`
- 防止重入攻击

### 6.5 防重放

**链下**:
- 使用 nonce 防止重放
- RelayerService 维护 nonce 映射

**链上**:
- 包含 paymentId 防止重复支付
- 包含 chainId 防止跨链重放

---

## 7. 部署与配置

### 7.1 部署步骤

```bash
# 1. 进入合约目录
cd contract

# 2. 安装依赖
npm install

# 3. 配置网络（.env 或 hardhat.config.ts）
# RPC_URL=https://data-seed-prebsc-1-s1.binance.org:8545/
# PRIVATE_KEY=0x...

# 4. 部署合约
npx hardhat run scripts/deploy-erc8004.ts --network bscTestnet
```

### 7.2 环境变量配置

**后端 `.env`**:
```env
# ERC8004 合约地址
ERC8004_CONTRACT_ADDRESS=0x...

# USDC 代币地址（BSC Testnet 使用 USDT）
USDC_ADDRESS=0x337610d27c682E347C9cD60BD4b3b107C9d34dDd

# Relayer 配置
RELAYER_PRIVATE_KEY=0x...
RELAYER_ADDRESS=0x...

# RPC 节点
RPC_URL=https://data-seed-prebsc-1-s1.binance.org:8545/
```

### 7.3 合约初始化

部署后需要设置 Relayer 地址：

```typescript
// 在部署脚本中自动设置
await sessionManager.setRelayer(relayerAddress);
```

或者手动调用：

```solidity
// 只有 Owner 可以设置
sessionManager.setRelayer(relayerAddress);
```

### 7.4 验证合约

```bash
# 使用 Hardhat 验证
npx hardhat verify --network bscTestnet \
  <CONTRACT_ADDRESS> \
  <USDC_ADDRESS>
```

---

## 8. 关键配置项

### 8.1 合约地址

- **BSC Testnet**: 部署后从部署脚本获取
- **环境变量**: `ERC8004_CONTRACT_ADDRESS`
- **使用位置**: PreflightCheckService, RelayerService, SessionService

### 8.2 Relayer 配置

- **Relayer 地址**: 必须在合约中设置（`setRelayer()`）
- **Relayer 私钥**: 用于支付 Gas 费用
- **权限**: 只有 Relayer 可以调用 `executeWithSession()`

### 8.3 USDC 代币地址

- **BSC Testnet**: `0x337610d27c682E347C9cD60BD4b3b107C9d34dDd` (USDT)
- **Mainnet**: 根据网络不同使用对应的 USDC 地址

---

## 9. 常见问题

### Q1: 合约未初始化怎么办？

**A**: 如果 `ERC8004_CONTRACT_ADDRESS` 未设置，系统会进入 Mock 模式：
- PreflightCheckService 返回空 Session 列表
- RelayerService 跳过链上执行
- 前端会显示警告信息

### Q2: Relayer 地址未设置怎么办？

**A**: 部署合约后必须调用 `setRelayer()` 设置 Relayer 地址，否则无法执行支付。

### Q3: 如何撤销 Session？

**A**: 用户可以调用 `revokeSession(sessionId)` 撤销 Session，设置 `isActive = false`。

### Q4: 每日限额何时重置？

**A**: 合约会自动检查，如果当前日期大于 `lastResetDate`，会自动重置 `usedToday = 0`。

---

## 10. 相关文档

- [Agentrix 支付流程文档](./Agentrix支付流程文档-最新版.md)
- [ERC8004SessionManager 合约源码](./contract/contracts/ERC8004SessionManager.sol)
- [部署脚本](./contract/scripts/deploy-erc8004.ts)
- [RelayerService 源码](./backend/src/modules/relayer/relayer.service.ts)
- [PreflightCheckService 源码](./backend/src/modules/payment/preflight-check.service.ts)

---

**文档维护**: Agentrix 开发团队  
**最后更新**: 2025年1月


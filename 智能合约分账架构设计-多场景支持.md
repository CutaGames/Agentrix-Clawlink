# 智能合约分账架构设计 - 多场景支持

**版本**: V2.0  
**日期**: 2025年1月  
**目标**: 设计支持多种支付场景的智能合约分账架构，Provider 作为可选模块

---

## 📋 目录

1. [支付场景分析](#1-支付场景分析)
2. [架构设计](#2-架构设计)
3. [合约设计](#3-合约设计)
4. [Provider 抽象层](#4-provider-抽象层)
5. [实施计划](#5-实施计划)

---

## 1. 支付场景分析

### 1.1 支付场景分类

#### 场景 1: QuickPay (X402) - 不需要 Provider
```
用户 → QuickPay Session → 智能合约 → 自动分账
```
- ✅ 用户已有 USDC
- ✅ 通过 Session Key 授权
- ✅ 直接调用合约分账

#### 场景 2: 钱包转账 - 不需要 Provider
```
用户钱包 → 智能合约 → 自动分账
```
- ✅ 用户已有 USDC
- ✅ 用户直接转账到合约
- ✅ 合约自动分账

#### 场景 3: 法币转数字货币 - 需要 Provider
```
用户法币 → Provider → 转换为 USDC → 智能合约 → 自动分账
```
- ⚠️ 用户没有 USDC
- ⚠️ 需要通过 Provider 转换
- ✅ Provider 转换后发送 USDC 到合约

#### 场景 4: 数字货币转法币 - 需要 Provider
```
商户 MPC 钱包 → Off-ramp Provider → 银行账户
```
- ⚠️ 商户需要提现到法币
- ⚠️ 需要通过 Off-ramp Provider
- ✅ 商户 MPC 钱包签名后发送到 Provider

### 1.2 场景总结

| 场景 | 需要 Provider | 支付方式 | 分账方式 |
|------|--------------|---------|---------|
| **QuickPay** | ❌ 不需要 | X402 Session | 合约自动分账 |
| **钱包转账** | ❌ 不需要 | 直接转账 | 合约自动分账 |
| **法币转数字货币** | ✅ 需要 | Provider On-ramp | Provider → 合约 → 自动分账 |
| **数字货币转法币** | ✅ 需要 | Provider Off-ramp | 商户 MPC 钱包 → Provider |

---

## 2. 架构设计

### 2.1 整体架构

```
┌─────────────────────────────────────────────────────────┐
│                  支付场景路由层                              │
└─────────────────────────────────────────────────────────┘
           │
    ┌──────┴──────┬──────────────┬──────────────┐
    │             │              │              │
┌───▼───┐   ┌───▼───┐      ┌───▼───┐      ┌───▼───┐
│QuickPay│   │钱包转账│      │Provider│      │其他场景│
│(X402) │   │       │      │On-ramp│      │       │
└───┬───┘   └───┬───┘      └───┬───┘      └───┬───┘
    │           │              │              │
    └───────────┴──────────────┴──────────────┘
                    │
        ┌───────────▼───────────┐
        │   智能合约分账层         │
        │  (Commission.sol)     │
        └───────────┬───────────┘
                    │
        ┌───────────┴───────────┐
        │                        │
    ┌───▼───┐              ┌───▼───┐
    │商户MPC │              │Agent  │
    │钱包    │              │钱包   │
    └───────┘              └───────┘
```

### 2.2 核心设计原则

1. **统一分账接口**: 所有场景最终都调用同一个分账函数
2. **Provider 抽象**: Provider 作为可选模块，不影响其他场景
3. **链上计算**: 分账逻辑在链上，可审计
4. **向后兼容**: 兼容现有 `distributeCommission` 函数

---

## 3. 合约设计

### 3.1 核心合约结构

```solidity
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;

import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/security/ReentrancyGuard.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";

/**
 * @title Commission
 * @dev 统一的分账合约，支持多种支付场景
 */
contract Commission is Ownable, ReentrancyGuard {
    IERC20 public settlementToken; // USDC
    address public agentrixTreasury;
    address public systemRebatePool;
    
    // 支付场景枚举
    enum PaymentScenario {
        QUICKPAY,      // QuickPay (X402)
        WALLET,        // 钱包直接转账
        PROVIDER_FIAT, // Provider 法币转数字货币
        PROVIDER_CRYPTO // Provider 数字货币转法币（提现）
    }
    
    // 分账配置（链上存储）
    struct SplitConfig {
        address merchantMPCWallet;  // 商户 MPC 钱包地址
        uint256 merchantAmount;     // 商户金额
        address referrer;            // 推荐人地址
        uint256 referralFee;         // 推荐人佣金
        address executor;            // 执行Agent地址
        uint256 executionFee;        // 执行Agent佣金
        uint256 platformFee;         // 平台费用
        bool executorHasWallet;      // 执行Agent是否有钱包
        uint256 settlementTime;      // 结算时间（0表示即时结算）
        bool isDisputed;             // 是否有争议
    }
    
    // 订单分账配置
    mapping(bytes32 => SplitConfig) public orderSplitConfigs;
    
    // Provider 白名单（可选，只有需要 Provider 的场景才需要）
    mapping(address => bool) public authorizedProviders;
    
    // 事件
    event PaymentReceived(
        bytes32 indexed orderId,
        PaymentScenario scenario,
        address indexed from,
        uint256 amount
    );
    
    event PaymentAutoSplit(
        bytes32 indexed orderId,
        address indexed merchantWallet,
        uint256 totalAmount,
        uint256 merchantAmount,
        uint256 platformFee,
        uint256 executionFee,
        uint256 referralFee
    );
    
    event SplitConfigSet(bytes32 indexed orderId, SplitConfig config);
    event ProviderAuthorized(address indexed provider, bool authorized);
    
    // ============ 核心分账函数 ============
    
    /**
     * @dev 统一的分账函数（内部函数）
     * @param orderId 订单ID
     * @param totalAmount 总金额
     */
    function _autoSplit(bytes32 orderId, uint256 totalAmount) internal {
        SplitConfig storage config = orderSplitConfigs[orderId];
        require(config.merchantMPCWallet != address(0), "Order config not found");
        require(!config.isDisputed, "Order is disputed");
        
        // 验证结算时间（如果需要）
        if (config.settlementTime > 0) {
            require(
                block.timestamp >= config.settlementTime,
                "Settlement time not reached"
            );
        }
        
        // 验证 USDC 余额
        require(
            settlementToken.balanceOf(address(this)) >= totalAmount,
            "Insufficient USDC balance"
        );
        
        // 分账到商户 MPC 钱包
        if (config.merchantMPCWallet != address(0) && config.merchantAmount > 0) {
            settlementToken.transfer(config.merchantMPCWallet, config.merchantAmount);
        }
        
        // 分账到推荐人
        if (config.referralFee > 0 && config.referrer != address(0)) {
            settlementToken.transfer(config.referrer, config.referralFee);
        }
        
        // 分账到执行Agent
        if (config.executionFee > 0) {
            address target = config.executorHasWallet 
                ? config.executor 
                : systemRebatePool;
            if (target == address(0)) {
                target = agentrixTreasury;
            }
            settlementToken.transfer(target, config.executionFee);
        }
        
        // 分账到平台
        if (config.platformFee > 0) {
            settlementToken.transfer(agentrixTreasury, config.platformFee);
        }
        
        // 记录分账事件
        emit PaymentAutoSplit(
            orderId,
            config.merchantMPCWallet,
            totalAmount,
            config.merchantAmount,
            config.platformFee,
            config.executionFee,
            config.referralFee
        );
    }
    
    // ============ 场景 1: QuickPay (X402) ============
    
    /**
     * @dev QuickPay 支付（X402 Session）
     * @param orderId 订单ID
     * @param amount 支付金额
     * @param signature Session 签名
     */
    function quickPaySplit(
        bytes32 orderId,
        uint256 amount,
        bytes memory signature
    ) external nonReentrant {
        // 1. 验证 Session 签名（由 X402 合约验证）
        // 这里可以调用 ERC8004SessionManager 验证
        
        // 2. 从用户钱包转账 USDC 到合约
        require(
            settlementToken.transferFrom(msg.sender, address(this), amount),
            "USDC transfer failed"
        );
        
        // 3. 记录支付
        emit PaymentReceived(orderId, PaymentScenario.QUICKPAY, msg.sender, amount);
        
        // 4. 自动分账
        _autoSplit(orderId, amount);
    }
    
    // ============ 场景 2: 钱包直接转账 ============
    
    /**
     * @dev 钱包直接转账分账
     * @param orderId 订单ID
     * @param amount 支付金额
     */
    function walletSplit(
        bytes32 orderId,
        uint256 amount
    ) external nonReentrant {
        // 1. 从用户钱包转账 USDC 到合约
        require(
            settlementToken.transferFrom(msg.sender, address(this), amount),
            "USDC transfer failed"
        );
        
        // 2. 记录支付
        emit PaymentReceived(orderId, PaymentScenario.WALLET, msg.sender, amount);
        
        // 3. 自动分账
        _autoSplit(orderId, amount);
    }
    
    // ============ 场景 3: Provider 法币转数字货币 ============
    
    /**
     * @dev Provider 发送 USDC 到合约并自动分账
     * @param orderId 订单ID
     * @param amount 支付金额
     */
    function providerFiatToCryptoSplit(
        bytes32 orderId,
        uint256 amount
    ) external nonReentrant {
        // 1. 验证 Provider 地址（白名单）
        require(authorizedProviders[msg.sender], "Unauthorized provider");
        
        // 2. 验证 USDC 已转入合约（Provider 需要先 approve 和 transfer）
        require(
            settlementToken.balanceOf(address(this)) >= amount,
            "Insufficient USDC balance"
        );
        
        // 3. 记录支付
        emit PaymentReceived(orderId, PaymentScenario.PROVIDER_FIAT, msg.sender, amount);
        
        // 4. 自动分账
        _autoSplit(orderId, amount);
    }
    
    // ============ 场景 4: 数字货币转法币（提现）============
    // 这个场景不需要分账，是商户从 MPC 钱包提现到法币
    // 由商户 MPC 钱包直接与 Provider 交互，不经过此合约
    
    // ============ 配置函数 ============
    
    /**
     * @dev 设置订单分账配置
     * @param orderId 订单ID
     * @param config 分账配置
     */
    function setSplitConfig(
        bytes32 orderId,
        SplitConfig memory config
    ) external onlyOwner {
        require(config.merchantMPCWallet != address(0), "Invalid merchant wallet");
        orderSplitConfigs[orderId] = config;
        emit SplitConfigSet(orderId, config);
    }
    
    /**
     * @dev 设置 Provider 白名单（可选）
     */
    function setAuthorizedProvider(address provider, bool authorized) external onlyOwner {
        authorizedProviders[provider] = authorized;
        emit ProviderAuthorized(provider, authorized);
    }
    
    /**
     * @dev 设置争议状态
     */
    function setDisputeStatus(bytes32 orderId, bool isDisputed) external onlyOwner {
        SplitConfig storage config = orderSplitConfigs[orderId];
        require(config.merchantMPCWallet != address(0), "Order not found");
        config.isDisputed = isDisputed;
    }
    
    // ============ 兼容现有函数 ============
    
    /**
     * @dev 兼容现有的 distributeCommission 函数
     * 保持向后兼容，但推荐使用新的自动分账函数
     */
    function distributeCommission(bytes32 orderId) external nonReentrant {
        SplitConfig storage config = orderSplitConfigs[orderId];
        require(config.merchantMPCWallet != address(0), "Order not found");
        
        // 获取合约中的 USDC 余额（需要先有资金转入）
        uint256 contractBalance = settlementToken.balanceOf(address(this));
        require(contractBalance > 0, "No balance to distribute");
        
        // 使用配置中的金额，或使用合约余额
        uint256 totalAmount = config.merchantAmount + 
                             config.referralFee + 
                             config.executionFee + 
                             config.platformFee;
        
        // 如果配置金额为0，使用合约余额
        if (totalAmount == 0) {
            totalAmount = contractBalance;
        }
        
        // 自动分账
        _autoSplit(orderId, totalAmount);
    }
    
    // ============ 查询函数 ============
    
    /**
     * @dev 查询订单分账配置
     */
    function getSplitConfig(bytes32 orderId) 
        external 
        view 
        returns (SplitConfig memory) 
    {
        return orderSplitConfigs[orderId];
    }
    
    /**
     * @dev 查询合约 USDC 余额
     */
    function getContractBalance() external view returns (uint256) {
        return settlementToken.balanceOf(address(this));
    }
    
    /**
     * @dev 接收 ERC20 代币（用于钱包直接转账场景）
     */
    function receiveERC20(address token, uint256 amount) external {
        // 这个函数可以由用户直接调用，将 USDC 转入合约
        // 但需要配合 setSplitConfig 和 walletSplit 使用
        require(token == address(settlementToken), "Only USDC allowed");
        require(
            IERC20(token).transferFrom(msg.sender, address(this), amount),
            "Transfer failed"
        );
    }
}
```

---

## 4. Provider 抽象层

### 4.1 Provider 接口设计

```typescript
// backend/src/modules/payment/provider-abstract.service.ts

/**
 * Provider 抽象接口
 * 用于统一管理不同的 Provider，方便后续接入和测试
 */
export interface IProvider {
  // Provider 标识
  id: string;
  name: string;
  
  // 是否支持 On-ramp（法币转数字货币）
  supportsOnRamp: boolean;
  
  // 是否支持 Off-ramp（数字货币转法币）
  supportsOffRamp: boolean;
  
  // 获取报价
  getQuote(amount: number, fromCurrency: string, toCurrency: string): Promise<ProviderQuote>;
  
  // 执行 On-ramp（法币转数字货币）
  executeOnRamp?(params: OnRampParams): Promise<OnRampResult>;
  
  // 执行 Off-ramp（数字货币转法币）
  executeOffRamp?(params: OffRampParams): Promise<OffRampResult>;
}

/**
 * Mock Provider（用于测试，不影响其他场景）
 */
@Injectable()
export class MockProviderService implements IProvider {
  id = 'mock';
  name = 'Mock Provider';
  supportsOnRamp = true;
  supportsOffRamp = true;
  
  async getQuote(amount: number, fromCurrency: string, toCurrency: string): Promise<ProviderQuote> {
    // 返回模拟报价
    return {
      providerId: this.id,
      rate: 1.0, // 1:1 汇率（测试用）
      fee: amount * 0.03, // 3% 手续费
      estimatedAmount: amount * 0.97,
      expiresAt: new Date(Date.now() + 60000), // 1分钟过期
    };
  }
  
  async executeOnRamp(params: OnRampParams): Promise<OnRampResult> {
    // 模拟 On-ramp 流程
    // 在实际场景中，这里会调用真实的 Provider API
    // 在测试场景中，可以模拟返回成功
    
    return {
      transactionId: `mock-${Date.now()}`,
      status: 'completed',
      cryptoAmount: params.amount * 0.97,
      cryptoCurrency: 'USDC',
      // 注意：在测试中，需要手动 mint USDC 到合约地址
      // 或使用测试账户转账
    };
  }
}

/**
 * Provider 管理器
 */
@Injectable()
export class ProviderManagerService {
  private providers: Map<string, IProvider> = new Map();
  
  constructor() {
    // 注册 Mock Provider（用于测试）
    this.registerProvider(new MockProviderService());
    
    // 后续可以注册真实 Provider
    // this.registerProvider(new MoonPayProviderService());
    // this.registerProvider(new TransakProviderService());
  }
  
  registerProvider(provider: IProvider) {
    this.providers.set(provider.id, provider);
  }
  
  getProvider(providerId: string): IProvider | null {
    return this.providers.get(providerId) || null;
  }
  
  /**
   * 获取所有支持 On-ramp 的 Provider
   */
  getOnRampProviders(): IProvider[] {
    return Array.from(this.providers.values())
      .filter(p => p.supportsOnRamp);
  }
  
  /**
   * 获取所有支持 Off-ramp 的 Provider
   */
  getOffRampProviders(): IProvider[] {
    return Array.from(this.providers.values())
      .filter(p => p.supportsOffRamp);
  }
  
  /**
   * 选择最优 Provider（比价）
   */
  async selectBestProvider(
    amount: number,
    fromCurrency: string,
    toCurrency: string,
    type: 'onramp' | 'offramp'
  ): Promise<IProvider | null> {
    const candidates = type === 'onramp' 
      ? this.getOnRampProviders()
      : this.getOffRampProviders();
    
    if (candidates.length === 0) {
      return null;
    }
    
    // 获取所有 Provider 的报价
    const quotes = await Promise.all(
      candidates.map(async (provider) => {
        try {
          const quote = await provider.getQuote(amount, fromCurrency, toCurrency);
          return { provider, quote };
        } catch (error) {
          return null;
        }
      })
    );
    
    // 过滤失败的报价
    const validQuotes = quotes.filter(q => q !== null);
    
    if (validQuotes.length === 0) {
      return null;
    }
    
    // 选择最优报价（费率最低）
    const best = validQuotes.reduce((best, current) => {
      const bestRate = best.quote.rate;
      const currentRate = current.quote.rate;
      return currentRate > bestRate ? current : best;
    });
    
    return best.provider;
  }
}
```

### 4.2 Provider 测试模式

```typescript
// 环境变量控制
const USE_MOCK_PROVIDER = process.env.USE_MOCK_PROVIDER === 'true';
const PROVIDER_MODE = process.env.PROVIDER_MODE || 'mock'; // 'mock' | 'real'

// 在支付服务中使用
@Injectable()
export class PaymentService {
  constructor(
    private providerManager: ProviderManagerService,
  ) {}
  
  async processProviderPayment(dto: ProcessPaymentDto) {
    // 如果是测试模式，使用 Mock Provider
    if (PROVIDER_MODE === 'mock') {
      const mockProvider = this.providerManager.getProvider('mock');
      if (mockProvider) {
        // 使用 Mock Provider 处理
        // 在测试中，可以手动 mint USDC 或使用测试账户
        return this.processWithMockProvider(mockProvider, dto);
      }
    }
    
    // 否则使用真实 Provider
    const provider = await this.providerManager.selectBestProvider(
      dto.amount,
      dto.currency,
      'USDC',
      'onramp'
    );
    
    if (!provider) {
      throw new Error('No provider available');
    }
    
    return this.processWithRealProvider(provider, dto);
  }
}
```

---

## 5. 实施计划

### 5.1 阶段 1: 核心分账功能（1-2周）

**目标**: 实现 QuickPay 和钱包转账场景的分账

**任务**:
1. ✅ 修改 `Commission.sol`，添加统一分账函数 `_autoSplit`
2. ✅ 实现 `quickPaySplit` 函数（QuickPay 场景）
3. ✅ 实现 `walletSplit` 函数（钱包转账场景）
4. ✅ 实现 `setSplitConfig` 函数（设置分账配置）
5. ✅ 保持 `distributeCommission` 向后兼容

**测试**:
- QuickPay 支付测试
- 钱包转账测试
- 分账验证测试

### 5.2 阶段 2: Provider 抽象层（1周）

**目标**: 实现 Provider 抽象层，不影响其他场景

**任务**:
1. ✅ 创建 `IProvider` 接口
2. ✅ 实现 `MockProviderService`（用于测试）
3. ✅ 实现 `ProviderManagerService`（Provider 管理器）
4. ✅ 集成到支付服务

**测试**:
- Mock Provider 测试
- Provider 切换测试

### 5.3 阶段 3: Provider 集成（可选，后续）

**目标**: 接入真实 Provider

**任务**:
1. 实现真实 Provider（MoonPay、Transak 等）
2. 实现 `providerFiatToCryptoSplit` 函数
3. 集成 Provider API

**测试**:
- Provider On-ramp 测试
- Provider Off-ramp 测试

---

## 6. 测试方案

### 6.1 测试场景优先级

**优先级 1: 不需要 Provider 的场景**
1. ✅ QuickPay (X402) 支付和分账
2. ✅ 钱包直接转账和分账
3. ✅ 分账配置设置和查询

**优先级 2: Provider 场景（使用 Mock）**
4. ⚠️ Provider 法币转数字货币（使用 Mock Provider）
5. ⚠️ Provider 数字货币转法币（使用 Mock Provider）

**优先级 3: 真实 Provider（后续）**
6. ⏳ 真实 Provider 集成和测试

### 6.2 测试用例

```typescript
// contract/test/Commission.test.ts

describe('Commission - Multi-Scenario Split', () => {
  describe('Scenario 1: QuickPay', () => {
    it('should split payment via QuickPay', async () => {
      // 1. 设置分账配置
      await commission.setSplitConfig(orderId, splitConfig);
      
      // 2. 用户授权 USDC
      await usdc.approve(commission.address, amount);
      
      // 3. 调用 QuickPay 分账
      await commission.quickPaySplit(orderId, amount, signature);
      
      // 4. 验证分账结果
      expect(await usdc.balanceOf(merchantWallet)).to.equal(merchantAmount);
      expect(await usdc.balanceOf(platformWallet)).to.equal(platformFee);
    });
  });
  
  describe('Scenario 2: Wallet Transfer', () => {
    it('should split payment via wallet transfer', async () => {
      // 1. 设置分账配置
      await commission.setSplitConfig(orderId, splitConfig);
      
      // 2. 用户授权并转账
      await usdc.approve(commission.address, amount);
      await commission.walletSplit(orderId, amount);
      
      // 3. 验证分账结果
      expect(await usdc.balanceOf(merchantWallet)).to.equal(merchantAmount);
    });
  });
  
  describe('Scenario 3: Provider (Mock)', () => {
    it('should split payment from Provider', async () => {
      // 1. 设置分账配置
      await commission.setSplitConfig(orderId, splitConfig);
      
      // 2. 设置 Provider 白名单
      await commission.setAuthorizedProvider(mockProviderAddress, true);
      
      // 3. Provider 转账 USDC 到合约
      await usdc.transfer(commission.address, amount);
      
      // 4. Provider 调用分账函数
      await commission.providerFiatToCryptoSplit(orderId, amount);
      
      // 5. 验证分账结果
      expect(await usdc.balanceOf(merchantWallet)).to.equal(merchantAmount);
    });
  });
});
```

---

## 7. 总结

### 7.1 设计优势

✅ **统一分账接口**: 所有场景使用同一个分账逻辑  
✅ **Provider 抽象**: Provider 作为可选模块，不影响其他场景  
✅ **测试友好**: 使用 Mock Provider，可以先测试其他场景  
✅ **向后兼容**: 保持现有 `distributeCommission` 函数  
✅ **链上计算**: 分账逻辑在链上，可审计

### 7.2 实施建议

**立即实施**:
1. 修改 `Commission.sol`，添加统一分账函数
2. 实现 QuickPay 和钱包转账场景
3. 实现 Provider 抽象层（Mock Provider）

**后续实施**:
4. 接入真实 Provider
5. 实现 Provider On-ramp 和 Off-ramp

**测试策略**:
- 先测试不需要 Provider 的场景（QuickPay、钱包转账）
- 使用 Mock Provider 测试 Provider 场景
- 后续再接入真实 Provider

---

**文档维护**: Agentrix 开发团队  
**最后更新**: 2025年1月


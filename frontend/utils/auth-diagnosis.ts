 ...
 * 快速检查授权状态（浏览器控制台版本）
 */
export async function quickCheckAuth() {
  if (!window.ethereum) {
  ...
  try {
    ...
    // 获取合约地址
    let erc8004Address = '0xFfEf72198A71B288EfE403AC07f9c60A1a99f29e'; // 默认值
    ...
    console.log(`是否已授权: ${allowance > BigInt(0) ? '✅ 是' : '❌ 否'}`);
    ...
    return {
      ...
      allowanceFormatted,
      isAuthorized: allowance > BigInt(0),
      tokenMatch,
    };
  } catch (error: any) {
...
if (typeof window !== 'undefined') {
  (window as any).checkAuth = quickCheckAuth;
  (window as any).quickCheckAuth = quickCheckAuth;
  // 延迟输出日志，确保在页面加载后显示
  setTimeout(() => {
    console.log('💡 授权诊断工具已加载');

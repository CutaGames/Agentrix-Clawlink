...
请告诉我您需要什么帮助？`,
  ...
  // Commerce模块分类定义（3模块方案）
  const getCommerceCategories = () => [
    {
      // Reuse legacy id so StructuredResponseCard form branches render reliably.
      id: 'pay_exchange',
      icon: '💰',
      title: '支付与钱包',
      description: '快速支付、充值入金、提现出金、交易记录',
      protocol: 'X402',
      subCategories: [
        { id: 'payment', title: '快速支付', example: '我要付款 100 USDC' },
        { id: 'onramp', title: '充值入金', example: '用 100 USD 充值 USDC' },
        { id: 'offramp', title: '提现出金', example: '把 100 USDC 提现到银行' },
        { id: 'query', title: '交易记录', example: '查询订单支付状态' },
      ],
    },
    {
      // Reuse legacy id so collaboration forms render.
      id: 'collab',
      icon: '🤝',
      title: '协作与任务',
      description: '发布 Task/Skill、分账方案、预算池与里程碑',
      protocol: 'UCP',
      subCategories: [
        { id: 'publish_task', title: '发布任务', example: '发布一个协作任务' },
        { id: 'publish_skill', title: '发布 Skill', example: '发布一个 Skill 到 marketplace' },
        { id: 'split', title: '分账方案', example: '创建分账方案' },
        { id: 'budget', title: '预算管理', example: '建一个任务预算池' },
        { id: 'milestone', title: '里程碑', example: '给预算池加里程碑' },
      ],
    },
    {
      // Reuse legacy id so commission/settlement forms render (shown as “收益”).
      id: 'commission',
      icon: '💸',
      title: '收益',
      description: '收益明细与提取收益',
      protocol: 'UCP',
      subCategories: [
        { id: 'earnings_detail', title: '收益明细', example: '查看我的收益明细' },
        { id: 'settlement_execute', title: '提取收益', example: '发起一次结算' },
      ],
    },
  ];
  ...
  const handleSend = async (messageOverride?: string) => {
    ...
    const commerceIntentMap: Array<{ id: string; parentId: string; keywords: RegExp }> = [
      ...
      { id: 'query', parentId: 'pay_exchange', keywords: /(查询订单|订单状态|支付状态|交易记录)/i },
      { id: 'onramp', parentId: 'pay_exchange', keywords: /(兑换|换币|入金|on-?ramp)/i },
      { id: 'offramp', parentId: 'pay_exchange', keywords: /(提现|出金|off-?ramp)/i },
      // 协作分账场景
      { id: 'publish_task', parentId: 'collab', keywords: /(发布任务|协作任务)/i },
      { id: 'publish_skill', parentId: 'collab', keywords: /(发布skill|发布技能)/i },
      ...
      // 分佣结算场景
      { id: 'earnings_detail', parentId: 'commission', keywords: /(分润|分佣记录|佣金|结算记录|结算历史|收益明细)/i },
      { id: 'settlement_execute', parentId: 'commission', keywords: /(执行结算|发起结算|提取收益)/i },
    ];

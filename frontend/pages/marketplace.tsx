...
import {
  ShoppingBag, Wrench, Search, TrendingUp, Star, Package, Code, Zap,
...
type ViewMode = 'resources' | 'tools' | 'tasks';
...
const CATS: Record<ViewMode, { id: string; label: { zh: string; en: string }; icon?: string }[]> = {
  ...
  tools: [
    { id: 'all', label: { zh: '全部', en: 'All' } },
    { id: 'payment', label: { zh: '支付工具', en: 'Payment' }, icon: '💳' },
    { id: 'analysis', label: { zh: '数据分析', en: 'Analysis' }, icon: '📈' },
    { id: 'utility', label: { zh: '实用工具', en: 'Utilities' }, icon: '🔧' },
    { id: 'workflow', label: { zh: '工作流', en: 'Workflows' }, icon: '⚙️' },
  ],
  tasks: [
    { id: 'all', label: { zh: '全部', en: 'All' } },
...
export default function MarketplacePage() {
  ...
  const accent = viewMode === 'resources' ? 'blue' : viewMode === 'tools' ? 'purple' : 'amber';
            ...
            <div className="flex flex-col md:flex-row md:items-center gap-3">
              <div className="flex items-center gap-0.5 p-1 bg-slate-900/80 rounded-xl border border-slate-800">
                {([
                  ...
                  { m: 'tools' as ViewMode, Icon: Wrench, l: { zh: '工具与应用', en: 'Tools' }, c: 'from-purple-600 to-purple-500 shadow-purple-600/20' },
                ...
                {cats.map(c => (
                  <button key={c.id} onClick={() => { setSelCat(c.id); pgRef.current = 1; }}
                    className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-all ${selCat === c.id
                      ? (accent === 'blue' ? 'bg-blue-600 text-white' : accent === 'purple' ? 'bg-purple-600 text-white' : 'bg-amber-600 text-white')
          ...
          {viewMode === 'tasks' ? <TaskMarketplace /> : loading ? (
            ...
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {skills.map(s => (
                  <SkillCardNew key={s.id} skill={s} viewMode={viewMode === 'resources' ? 'resources' : 'tools'} t={t}

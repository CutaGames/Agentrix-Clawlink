...
export default function MerchantKYC() {
  ...
  const [kycInfo, setKycInfo] = useState<{ level: KYCLevel; status: string } | null>(null)
  ...
  const loadKYCInfo = async () => {
    try {
      ...
      setKycInfo({
        level: (profile.kycLevel as KYCLevel) || 'none',
        status: profile.kycStatus || 'none'
      })
      // 同步到UserContext
      if (user) {
        updateKYC((profile.kycLevel as KYCLevel) || 'none', (profile.kycStatus as any) || 'none')
      }
    } catch (error: any) {
      console.error('加载KYC信息失败:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSubmitKYC = async (level: KYCLevel) => {
    ...
    try {
      // 调用API提交KYC申请
      // 注意：如果后端有专门的KYC API端点，应该使用它
      // 目前使用updateProfile来更新KYC信息
      await userApi.updateProfile({
        // 如果updateProfile支持KYC字段，传入它们
        // 否则需要创建专门的KYC API
      } as any)
      
      // 更新本地状态
      setKycInfo({ level, status: 'pending' })
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-lg font-semibold text-gray-900">
                    {(kycInfo?.level || user?.kycLevel) === 'none' ? '未认证' : 
                     (kycInfo?.level || user?.kycLevel) === 'basic' ? '商户基础认证' : '商户高级认证'}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-gray-600 mb-1">认证状态</p>
                  {getStatusBadge(kycInfo?.status || user?.kycStatus || 'none')}
                </div>
              </div>
            </div>
          )}
            ...
            {kycLevels.map((kyc) => (
              <div key={kyc.level} className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div>
                    {(kycInfo?.level || user?.kycLevel) === kyc.level && (kycInfo?.status || user?.kycStatus) === 'approved' ? (
                      ...
                      <button
                        ...
                        disabled={isSubmitting || ((kycInfo?.level || user?.kycLevel) === kyc.level && (kycInfo?.status || user?.kycStatus) === 'pending')}
                        className="px-4 py-2 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 transition-colors disabled:opacity-50"
                      >
                        {isSubmitting ? '提交中...' : (kycInfo?.level || user?.kycLevel) === kyc.level && (kycInfo?.status || user?.kycStatus) === 'pending' ? '审核中' : '申请认证'}
                      </button>
                    )}
                  </div>
                </div>

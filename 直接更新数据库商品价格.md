...
```bash
# 方式1：使用环境变量
psql -h localhost -U paymind -d paymind_db -f backend/scripts/update-prices-direct.sql

# 方式2：直接指定参数
psql -h localhost -p 5432 -U paymind -d paymind_db -f backend/scripts/update-prices-direct.sql
```
...
```bash
psql -h localhost -U paymind -d paymind_db
```

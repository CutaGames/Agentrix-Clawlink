# 创建测试商品说明

## 方法 1：通过 API 创建（推荐）

### 步骤 1：获取 JWT Token

1. 在前端登录测试账户 `pm-1763463490911-91zf91wu2`
2. 打开浏览器开发者工具（F12）
3. 进入 Application -> Local Storage
4. 查找 `token` 或 `authToken` 字段
5. 复制 token 值

### 步骤 2：运行脚本

```bash
cd backend
TOKEN=your_jwt_token npx ts-node scripts/create-test-products-api.ts
```

或者：

```bash
cd backend
npx ts-node scripts/create-test-products-api.ts your_jwt_token
```

## 方法 2：使用 curl 命令

```bash
# 设置变量
TOKEN="your_jwt_token"
API_URL="http://localhost:3001/api"

# 创建 QuickPay 商品
curl -X POST "$API_URL/products" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d '{
    "name": "📚 高级阅读服务 - QuickPay 体验",
    "description": "适合 QuickPay 小额支付的阅读服务产品。使用 X402 协议快速支付，适合小额高频场景。",
    "price": 0.1,
    "stock": 1000,
    "category": "服务",
    "commissionRate": 5,
    "productType": "service",
    "metadata": {
      "productType": "service",
      "currency": "USDT",
      "paymentMethod": "quickpay",
      "x402Enabled": true,
      "assetType": "service",
      "image": "https://images.unsplash.com/photo-1481627834876-b7833e8f5570?w=400"
    }
  }'

# 创建 NFT 商品
curl -X POST "$API_URL/products" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d '{
    "name": "🎮 NFT 游戏道具 - 传奇之剑",
    "description": "限量版 NFT 游戏道具，可在游戏中使用的传奇之剑。支持数字货币钱包支付，适合 Web3 用户。",
    "price": 10,
    "stock": 100,
    "category": "NFT",
    "commissionRate": 8,
    "productType": "nft",
    "metadata": {
      "productType": "nft",
      "currency": "USDT",
      "paymentMethod": "wallet",
      "assetType": "nft_rwa",
      "chain": "BSC",
      "contractAddress": "0x0000000000000000000000000000000000000000",
      "tokenId": "1",
      "image": "https://images.unsplash.com/photo-1639322537504-6427a16b0a38?w=400"
    }
  }'

# 创建 Stripe 商品
curl -X POST "$API_URL/products" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d '{
    "name": "💳 高级会员服务 - Stripe 支付",
    "description": "高级会员服务，支持 Stripe 支付方式。主要用于测试佣金分配机制和结算流程。",
    "price": 500,
    "stock": 50,
    "category": "服务",
    "commissionRate": 10,
    "productType": "service",
    "metadata": {
      "productType": "service",
      "currency": "CNY",
      "paymentMethod": "stripe",
      "assetType": "aggregated_web2",
      "image": "https://images.unsplash.com/photo-1551434678-e076c223a692?w=400"
    }
  }'
```

## 方法 3：通过前端界面创建

1. 登录测试账户
2. 访问商户后台：`http://localhost:3000/app/merchant/products`
3. 点击"添加商品"
4. 填写商品信息
5. 保存

## 验证商品创建

访问以下页面查看商品：

- Marketplace: `http://localhost:3000/marketplace`
- 测试场景: `http://localhost:3000/test-scenarios`
- API: `http://localhost:3001/api/products`

## 注意事项

1. 确保后端服务正在运行
2. 确保测试账户已注册并具有 merchant 角色
3. 如果账户没有 merchant 角色，可以通过 Agent 注册：
   - 打开 Agent 页面
   - 输入："注册商户"
   - Agent 会自动为用户添加 merchant 角色


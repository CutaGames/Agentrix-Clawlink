...
async function withPage(callback) {
  ...
  const target = targets.find(t => t.url && t.url.includes('tauri.localhost')) || targets[0];
  ...
  const result = await callback((expression, extra = {}) =>
    send(ws, state, 'Runtime.evaluate', {
      expression,
      awaitPromise: true,
      returnByValue: true,
      ...extra,
...
async function main() {
  ...
  const result = await withPage((evaluate) => evaluate(expression));

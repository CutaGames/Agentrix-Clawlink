# Agentrix 开发流程规范

## 📋 原则

**单一数据源原则**：本地代码是唯一的数据源，云端服务器只用于部署和测试。

---

## 🔄 标准开发流程

### 1. **日常开发（本地）**

```bash
# 1. 在本地进行开发
cd /mnt/d/wsl/Ubuntu-24.04/Code/Paymind/paymind-website/backend

# 2. 修改代码
# ... 编辑文件 ...

# 3. 本地测试
npm run start:dev

# 4. 测试通过后，提交到 Git
git add .
git commit -m "feat: 添加新功能"
git push
```

### 2. **部署到服务器（从本地）**

```bash
# 方式 A: 使用 scp 上传（推荐用于快速修复）
cd /mnt/d/wsl/Ubuntu-24.04/Code/Paymind/paymind-website/backend

# 上传单个文件
scp src/modules/auth/auth.controller.ts root@129.226.152.88:/var/www/agentrix-website/backend/src/modules/auth/

# 上传整个模块
scp -r src/modules/auth/* root@129.226.152.88:/var/www/agentrix-website/backend/src/modules/auth/

# 方式 B: 使用 Git（推荐用于完整部署）
# 在服务器上执行
ssh root@129.226.152.88
cd /var/www/agentrix-website
git pull origin main
cd backend
npm run build
pm2 restart agentrix-backend --update-env
```

---

## 🚫 禁止的操作

### ❌ 不要在服务器上直接修改代码

**错误做法：**
```bash
# ❌ 不要这样做
ssh root@129.226.152.88
cd /var/www/agentrix-website/backend
nano src/modules/auth/auth.service.ts  # 直接在服务器上编辑
```

**正确做法：**
```bash
# ✅ 正确做法
# 1. 在本地修改
# 2. 测试通过
# 3. 提交到 Git
# 4. 上传到服务器
```

---

## 📦 代码同步方案

### 方案 1：Git 同步（推荐）

**本地 → 服务器：**

```bash
# 本地：提交代码
git add .
git commit -m "feat: 添加 auth 模块"
git push origin main

# 服务器：拉取代码
ssh root@129.226.152.88
cd /var/www/agentrix-website
git pull origin main
cd backend
npm run build
pm2 restart agentrix-backend --update-env
```

### 方案 2：SCP 同步（快速修复）

**适用于：**
- 紧急修复
- 单个文件修改
- 临时测试

```bash
# 上传单个文件
scp backend/src/modules/auth/auth.service.ts root@129.226.152.88:/var/www/agentrix-website/backend/src/modules/auth/

# 上传整个目录
scp -r backend/src/modules/auth/* root@129.226.152.88:/var/www/agentrix-website/backend/src/modules/auth/
```

### 方案 3：从服务器拉取（仅用于恢复）

**仅在以下情况使用：**
- 本地文件丢失
- 需要恢复服务器上的代码

```bash
# 从服务器拉取到本地（覆盖本地）
scp -r root@129.226.152.88:/var/www/agentrix-website/backend/src/* ./backend/src/
```

---

## 🔧 当前问题修复流程

### 步骤 1：确定代码源

```bash
# 检查本地和服务器代码差异
# 本地
cd backend
git status

# 服务器
ssh root@129.226.152.88
cd /var/www/agentrix-website/backend
git status
```

### 步骤 2：统一代码源

**如果服务器代码更新：**
```bash
# 从服务器拉取到本地（仅用于恢复）
scp -r root@129.226.152.88:/var/www/agentrix-website/backend/src/* ./backend/src/
```

**如果本地代码更新：**
```bash
# 提交到 Git
git add .
git commit -m "fix: 同步代码"
git push

# 部署到服务器
ssh root@129.226.152.88
cd /var/www/agentrix-website
git pull
cd backend
npm run build
pm2 restart agentrix-backend --update-env
```

---

## 📝 开发工作流程

### 开发新功能

1. **创建分支**（可选）
   ```bash
   git checkout -b feature/new-feature
   ```

2. **本地开发**
   - 在本地修改代码
   - 本地测试：`npm run start:dev`

3. **提交代码**
   ```bash
   git add .
   git commit -m "feat: 新功能描述"
   git push
   ```

4. **部署到服务器**
   ```bash
   # 方式 A: Git 部署
   ssh root@129.226.152.88
   cd /var/www/agentrix-website
   git pull
   cd backend
   npm run build
   pm2 restart agentrix-backend --update-env
   
   # 方式 B: SCP 部署（快速）
   scp -r backend/src/modules/new-module/* root@129.226.152.88:/var/www/agentrix-website/backend/src/modules/new-module/
   ```

### 修复 Bug

1. **本地修复**
   ```bash
   # 在本地修改代码
   # 测试修复
   npm run start:dev
   ```

2. **快速部署**
   ```bash
   # 上传修复的文件
   scp backend/src/modules/auth/auth.service.ts root@129.226.152.88:/var/www/agentrix-website/backend/src/modules/auth/
   
   # 服务器上重新构建
   ssh root@129.226.152.88
   cd /var/www/agentrix-website/backend
   npm run build
   pm2 restart agentrix-backend --update-env
   ```

3. **提交到 Git**
   ```bash
   git add .
   git commit -m "fix: 修复登录问题"
   git push
   ```

---

## 🛠️ 常用命令

### 本地开发

```bash
# 启动开发服务器
npm run start:dev

# 构建项目
npm run build

# 查看日志
npm run start:dev | tee dev.log
```

### 服务器部署

```bash
# SSH 到服务器
ssh root@129.226.152.88

# 进入项目目录
cd /var/www/agentrix-website/backend

# 拉取最新代码
git pull

# 构建项目
npm run build

# 重启服务
pm2 restart agentrix-backend --update-env

# 查看日志
pm2 logs agentrix-backend
```

### 快速上传文件

```bash
# 上传单个文件
scp backend/src/modules/auth/auth.controller.ts root@129.226.152.88:/var/www/agentrix-website/backend/src/modules/auth/

# 上传整个模块
scp -r backend/src/modules/auth/* root@129.226.152.88:/var/www/agentrix-website/backend/src/modules/auth/

# 上传多个文件
scp backend/src/modules/auth/*.ts root@129.226.152.88:/var/www/agentrix-website/backend/src/modules/auth/
```

---

## 📋 检查清单

### 开发前

- [ ] 确认本地代码是最新的（`git pull`）
- [ ] 确认本地环境配置正确（`.env` 文件）

### 开发中

- [ ] 在本地测试功能
- [ ] 检查代码格式（`npm run lint`）
- [ ] 提交代码到 Git

### 部署前

- [ ] 代码已提交到 Git
- [ ] 本地测试通过
- [ ] 检查服务器环境变量

### 部署后

- [ ] 检查服务器日志
- [ ] 测试 API 端点
- [ ] 确认功能正常

---

## ⚠️ 注意事项

1. **永远不要在服务器上直接编辑代码**
   - 服务器上的修改会丢失
   - 无法版本控制
   - 容易造成代码不一致

2. **使用 Git 进行版本控制**
   - 所有代码修改都要提交到 Git
   - 使用有意义的 commit 信息

3. **环境变量管理**
   - `.env` 文件不要提交到 Git
   - 服务器上的 `.env` 单独管理

4. **数据库迁移**
   - 使用 TypeORM migrations
   - 不要直接修改数据库结构

---

## 🔄 紧急情况处理

### 服务器代码丢失

```bash
# 从 Git 恢复
ssh root@129.226.152.88
cd /var/www/agentrix-website
git reset --hard origin/main
git pull
cd backend
npm run build
pm2 restart agentrix-backend --update-env
```

### 本地代码丢失

```bash
# 从 Git 恢复
cd backend
git reset --hard origin/main
git pull
```

### 代码冲突

```bash
# 查看冲突
git status

# 解决冲突后
git add .
git commit -m "fix: 解决冲突"
git push
```

---

## 📞 问题排查

### 代码不同步

1. 检查 Git 状态
   ```bash
   git status
   git log --oneline -10
   ```

2. 比较本地和服务器
   ```bash
   # 本地
   git diff
   
   # 服务器
   ssh root@129.226.152.88
   cd /var/www/agentrix-website
   git diff
   ```

3. 统一代码源
   - 确定哪个版本是正确的
   - 使用 Git 或 SCP 同步

---

## 🎯 最佳实践

1. **开发在本地，部署在服务器**
2. **使用 Git 进行版本控制**
3. **每次部署前都要测试**
4. **保持代码同步**
5. **使用有意义的 commit 信息**

---

## 📚 相关文档

- Git 工作流程：`上传到GitHub-WSL.sh`
- 服务器部署：`deploy-to-server.sh`
- 代码同步：`sync-from-server.sh`


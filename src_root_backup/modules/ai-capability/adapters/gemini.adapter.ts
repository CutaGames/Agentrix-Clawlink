...
import { GeminiFunctionSchema, AIPlatform, SystemCapability, FunctionSchema } from '../interfaces/capability.interface';

@Injectable()
export class GeminiAdapter implements IPlatformAdapter {
  platform: AIPlatform = 'gemini';

  convertProductToFunction(product: Product, capabilityType: string): GeminiFunctionSchema {
  ...
  convertProductsToFunctions(products: Product[], capabilityType: string): GeminiFunctionSchema[] {
    return products.map((product) => this.convertProductToFunction(product, capabilityType));
  }

  validateSchema(schema: GeminiFunctionSchema): boolean {
    return (
      !!schema.name &&
      !!schema.description &&
  ...
  convertSystemCapabilityToFunction(capability: SystemCapability): GeminiFunctionSchema {
    return {

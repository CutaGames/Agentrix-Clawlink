# PayMind 五层架构产品需求文档（The Pentagonal Architecture PRD）V1.0

**版本**：V1.0  
**日期**：2025-01-21  
**撰写人**：产品战略组（Edge + Cloud + Protocol）  
**范围**：PayMind Pentagonal Architecture（Fat Edge, Thin Cloud / Code is Law / AI First）
...
- **演进路线**：POS Edge MVP（Qwen0.6B）→ 云端 PayMind-Brain（MoE）→ 协议与 DePIN 规模化。
...
### 2.3 交易大脑层（PayMind Transaction Intelligence）
- **定位**：垂直行业大模型（Cloud MoE + Edge SLM）。
- **能力**：
  - PayMind-Brain（Cloud MoE）：风险专家、财务专家、合约专家、联邦学习中心。
  - PayMind-Nano（Edge SLM）：Qwen3 0.6B 起步，未来自研 1.3B/2B，负责意图前置、离线推理、隐私风控。
...
| 交易大脑层 | PayMind-Brain 已有风险/支付/定价模型，Edge SLM 缺失 | Cloud MoE + Edge Nano 联动，周级蒸馏 | Qwen3 0.6B 落地、MoE 专家拆分、蒸馏流水线 |
...
2. **P1**：IM/移动多端同步、Edge OTA、PayMind-Brain MoE 化、Compliance Oracle、DePIN 激励 PoC。
...
## 7. 与其他文档的关系
- `PayMind-POS-Edge-AI-Agent-PRD-V1.0.md`：对应第 1 层与第 3 层 Edge 能力的 MVP 落地。
- `PayMind-V7.0-执行摘要.md`、`PayMind统一支付流程与时序图`：提供协议层与支付引擎参考。
- `PayMind-Agent工作台重构说明.md`：Persona & UI 架构输入。

...
Provider (嵌入式集成，KYC在Provider处) ← 需要实现
    ↓
Provider → PayMind Contract (发送USDC) ← 需要实现
    ↓
PayMind Contract → 自动分账 → 商户MPC钱包 ← 需要实现
    ↓
...
4. **商户 MPC 钱包**: 商户持有分片 A，PayMind 持有分片 B，备份持有分片 C
...
<ProviderEmbed
  ...
  // KYC 数据直接提交给 Provider，不经过 PayMind
/>
```
...
class MPCSignatureService {
  ...
  async signWithShardBAndC(
    ...
    // PayMind 使用分片 B，分片 C 自动授权（需要商户预先授权）

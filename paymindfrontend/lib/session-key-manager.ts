...
export class SessionKeyManager {
  ...
  private static async getUserPassword(): Promise<string> {
    // 实际实现应该从安全存储获取用户密码
    // 或使用主钱包签名作为密钥
    const stored = localStorage.getItem('paymind_user_password_hash');
    if (stored) {
      return stored;
    }
    // 生成临时密码（仅用于开发）
    const tempPassword = `paymind_${Date.now()}`;
    localStorage.setItem('paymind_user_password_hash', tempPassword);
    return tempPassword;
  }

...
print(f"支付链接: {payment.payment_url}")`,
  ...
  const getPaymentStatusCode = (lang: string) => {
    const codes: Record<string, string> = {
      typescript: `// 查询支付状态
const payment = await paymind.payments.get(paymentId);
...
console.log('状态:', payment.status);`,
      python: `# 查询支付状态
payment = paymind.payments.get(payment_id)
print(f"状态: {payment.status}")`,
  ...
  const getOrderCode = (lang: string) => {
    const codes: Record<string, string> = {
      typescript: `// 创建订单
const order = await paymind.orders.create({
  productId: 'prod_123',
  quantity: 1,
  amount: 100,
});
...
console.log('订单ID:', order.id);`,
  ...
  const getOrderQueryCode = (lang: string) => {
    const codes: Record<string, string> = {
      typescript: `// 查询订单
const order = await paymind.orders.get(orderId);
console.log('订单状态:', order.status);`,
      javascript: `const order = await paymind.orders.get(orderId);`,
      python: `order = paymind.orders.get(order_id)`,
    };
    return codes[lang] || codes.typescript;
  };
  ...
  const getProductSearchCode = (lang: string) => {
    const codes: Record<string, string> = {
      typescript: `// 搜索商品
const products = await paymind.marketplace.searchProducts({
  query: '笔记本电脑',
  priceMax: 10000,
});

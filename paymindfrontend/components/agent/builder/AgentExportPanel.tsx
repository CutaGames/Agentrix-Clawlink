      ...
      {exportType === 'standalone' && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <p className="text-sm text-blue-800">
            {t({
              zh: '💡 SaaS 托管：您的 Agent 将直接部署到 PayMind 云端，无需配置服务器。生成后即可获得访问链接。',
              en: '💡 SaaS Hosted: Your Agent will be deployed directly to PayMind cloud, no server configuration needed. You will get an access link after generation.',
            })}
          </p>
        </div>
      )}
...
这是通过PayMind Agent Builder生成的${type}类型Agent。
...
function generateEnvExample(agentId?: string): string {
  return `# PayMind Agent环境变量配置

# PayMind API密钥（必需）
PAYMIND_API_KEY=your-api-key-here

# Agent ID（可选，如果已创建Agent实例）
AGENT_ID=${agentId || 'your-agent-id'}

# API基础URL（可选，默认使用PayMind官方API）
PAYMIND_API_URL=https://api.paymind.ai
...
function generateLambdaHandler(agentType: string, workflow?: WorkflowDefinition | null): string {
  return `const { PayMindAgent } = require('@paymind/agent-sdk');
...
const { PayMindAgent } = require('@paymind/agent-sdk');
...
function generateCloudflareWorker(agentType: string, workflow?: WorkflowDefinition | null): string {
  return `import { PayMindAgent } from '@paymind/agent-sdk';

export default {
  async fetch(request, env) {
    const agent = new PayMindAgent({
      apiKey: env.PAYMIND_API_KEY,
      agentType: '${agentType}',
      workflow: ${JSON.stringify(workflow || {}, null, 2)},
    });
...
<head>
  ...
  <title>${name} - PayMind Agent</title>

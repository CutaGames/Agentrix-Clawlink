...
export function LoginModal({ onClose, onWalletSuccess }: LoginModalProps) {
  ...
  const completeWalletLogin = async (walletInfo: WalletInfo) => {
    ...
    const loginMessage = `PayMind 登录验证\n地址: ${walletInfo.address}\n时间: ${new Date().toISOString()}`
    ...
    try {
      ...
      login({
        ...
        paymindId: authResponse.user.paymindId,
  ...
  const handleSocialLogin = async (provider: 'google' | 'apple' | 'x') => {
    try {
      ...
      const socialEmail = `user-${provider}@paymind.com`
      const paymindId = `user-${provider}-${Date.now()}`
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-2xl p-8 max-w-md w-full mx-4">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-gray-900">登录 PayMind</h2>
          <button
            onClick={onClose}
      ...
      {isConnected && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-4 text-left">
          ...
          <p className="text-xs text-green-700 mb-3">
            仍需签名一次以完成 PayMind 登录并获取 PayMind ID。
          </p>
          <button
            onClick={async () => {
              setIsLoggingIn(true)
...
function Web2Login({ onSocialLogin, onClose }: { onSocialLogin: (provider: 'google' | 'apple' | 'x') => void; onClose: () => void }) {
  ...
  const [paymindId, setPaymindId] = useState('')
  const [error, setError] = useState<string | null>(null)
  const { login } = useUser()
  const router = useRouter()
  const toast = useToast()
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isRegisterMode) {
        ...
        login({
          id: response.user.id,
          paymindId: response.user.paymindId,
  ...
  return (
    <div className="space-y-4">
      ...
      <form onSubmit={handleEmailAuth} className="space-y-4">
        ...
        {isRegisterMode && (
          <div>
            <label htmlFor="paymindId" className="block text-sm font-medium text-gray-700 mb-1">
              PayMind ID（可选）
            </label>
            <input
              id="paymindId"

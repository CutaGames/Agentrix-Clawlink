...
const flows = [
  ...
  },
  {
    ...
    detail: { zh: '来自 PayMind Agent / Marketplace / 推广 Agent', en: 'Source from agents, marketplace or promotion agents' },
  },
  {
    step: '03',
...
export default function AllianceFeaturePage() {
  ...
  return (
    <>
      <Head>
        <title>{t({ zh: '联盟机制说明 - PayMind', en: 'Alliance feature - PayMind' })}</title>
        <meta
          name="description"
          content=              {t({
                zh: '全面介绍 PayMind 联盟机制：角色分工、分佣逻辑与接入方式。',
                en: 'Full overview of PayMind alliance mechanism: roles, commission logic and onboarding.',
              })}
        />
      </Head>
        ...
        <section className="border-b border-white/10 bg-gradient-to-r from-slate-800/90 to-slate-700/90 py-16 text-white">
          <div className="container mx-auto px-6">
            ...
            <p className="mt-4 max-w-3xl text-lg text-slate-100">
              {t({
                zh: 'PayMind 把执行 Agent、推荐 Agent、推广 Agent 放在同一个收益闭环里，提供标准化的 API 与结算模型。',
                en: 'PayMind unifies execution agents, referral agents and promotion agents in one revenue loop.',
              })}
            </p>
          </div>
        </section>

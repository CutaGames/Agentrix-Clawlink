...
export default function AgentKYC() {
  ...
  const kycLevels = [
    {
      ...
      name: 'Agent基础认证',
      description: '适用于个人Agent，基础身份验证',
      features: ['身份信息验证', '联系方式验证', '钱包地址验证'],
      limit: '单笔限额: ¥5,000 | 日限额: ¥20,000'
    },
    {
      level: 'verified' as KYCLevel,
      name: 'Agent高级认证',
      description: '完整的第三方KYC验证，适用于企业Agent',
      features: ['身份信息验证', '企业资质验证', '银行账户验证', '税务登记验证', '第三方认证'],
  ...
  return (
    <>
      <Head>
        <title>KYC认证 - Agent</title>
      </Head>
      <DashboardLayout userType="agent">
        <div className="max-w-4xl mx-auto">
          <div className="mb-8">
            <h1 className="text-2xl font-bold text-gray-900 mb-2">KYC身份认证</h1>
            <p className="text-gray-600">一次认证，全平台通用。通过KYC认证后，您可以在PayMind的所有支付通道中使用已验证的身份</p>
          </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-lg font-semibold text-gray-900">
                    {user.kycLevel === 'none' ? '未认证' : user.kycLevel === 'basic' ? 'Agent基础认证' : 'Agent高级认证'}
                  </p>
                </div>
                <div>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 mb-6">
            ...
            <p className="text-blue-700 text-sm mb-4">
              Agent用户无需单独处理合规，通过PayMind的统一身份认证即可获取合规凭证。
            </p>
            <div className="grid md:grid-cols-3 gap-4 text-sm">
              <div>
                <p className="font-semibold text-blue-900 mb-1">Agent</p>
          ...
          <div className="mt-6 bg-purple-50 border border-purple-200 rounded-lg p-6">
            ...
            <p className="text-purple-700 text-sm">
              通过PayMind认证后，您将获得一个Tokenized KYC凭证，可以在所有支持的支付通道中使用，无需重复验证。
            </p>
          </div>
        </div>
      </DashboardLayout>

...
import { PayMindMerchantAgent } from '@paymind/merchant-sdk'

const merchantAgent = new PayMindMerchantAgent({
...
import { PayMindDeveloperAgent } from '@paymind/developer-sdk'

const devAgent = new PayMindDeveloperAgent({
...
# - 收益和结算`,
  ...
  return (
    <>
      <Head>
        <title>{t({ zh: 'Agent Builder 演示 - PayMind', en: 'Agent Builder Demo - PayMind' })}</title>
        <meta
          name="description"
          content={t({
          ...
          <div className="container mx-auto px-6">
            <div className="max-w-4xl mx-auto text-center space-y-6">
              ...
              <p className="text-lg text-purple-100 max-w-2xl mx-auto">
                {t({
                  zh: '通过Agent Builder快速生成个人、商户、开发者Agent，生成的Agent可以在终端独立运行，无需登录PayMind官网',
                  en: 'Quickly generate personal, merchant, developer Agents through Agent Builder. Generated Agents can run independently in terminal without logging into PayMind website',
                })}
              </p>
            </div>
          </div>
                    ...
                    <div className="bg-slate-900 rounded-lg p-6">
                      ...
                      <div className="bg-slate-800 rounded-lg p-4">
                        <code className="text-green-400 text-sm">
                          ...
                          <br />
                          paymind agent download --id your-agent-id
                        </code>
                      </div>
                    </div>
                  </div>
                ...
                {currentStep === 'run' && (
                  <div className="space-y-6">
                    ...
                    <div className="bg-slate-900 rounded-lg p-6">
                      <p className="text-slate-300 mb-4">
                        {t({
                          zh: '生成的Agent可以在终端独立运行，无需登录PayMind官网。Agent会自动连接到PayMind服务，处理所有业务逻辑。',
                          en: 'Generated Agent can run independently in terminal without logging into PayMind website. Agent automatically connects to PayMind services and handles all business logic.',
                        })}
                      </p>
                      <div className="bg-slate-800 rounded-lg p-4">
                      ...
                      <div className="mt-4 bg-blue-500/20 border border-blue-500/30 rounded-lg p-4">
                        <p className="text-blue-200 text-sm">
                          {t({
                            zh: '💡 提示：Agent运行时会自动连接到PayMind服务，所有数据实时同步，无需手动操作',
                            en: '💡 Tip: Agent automatically connects to PayMind services when running, all data syncs in real-time, no manual operation needed',
                          })}
                        </p>
                      </div>
                    </div>

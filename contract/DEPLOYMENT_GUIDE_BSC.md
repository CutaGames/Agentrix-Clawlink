# PayMind 合约部署指南 - BSC 测试网

本文档指导如何将 PayMind 所有智能合约部署到 BSC 测试网。
...
PAYMIND_TREASURY_ADDRESS=0x...  # PayMind 金库地址
...
在 `paymindfrontend/.env.local` 中添加：
...
5. **Commission 配置**: Commission 合约部署后必须配置 `settlementToken` 和 `paymindTreasury` 才能正常工作
...
- [ ] AutoPay 部署成功
- [ ] Commission 部署成功
- [ ] Commission 配置了 settlementToken 和 paymindTreasury
- [ ] 所有合约地址已更新到后端 `.env`
...
- 确保 `PAYMIND_TREASURY_ADDRESS` 是有效的地址

# 云端服务器直接开发指南

## 🎯 概述

直接在云端服务器（129.226.152.88）上进行开发，无需本地开发再上传。

## ✅ 优势

1. **实时生效** - 修改代码后立即测试
2. **无需上传** - 直接在服务器上编辑
3. **快速迭代** - 修改、构建、测试一气呵成
4. **环境一致** - 开发环境和生产环境一致

## 🚀 快速开始

### 1. SSH 连接到服务器

```bash
ssh root@129.226.152.88
```

### 2. 进入项目目录

```bash
cd /var/www/agentrix-website/backend
```

### 3. 安装 OpenAI SDK（如果需要）

```bash
npm install openai
```

### 4. 开发模式启动

```bash
# 使用开发模式（自动重载）
npm run start:dev

# 或者使用 PM2 开发模式
pm2 start npm --name "agentrix-backend-dev" -- run start:dev
```

## 📝 开发工作流

### 方式 A：使用 PM2 开发模式（推荐）

```bash
# 1. 停止生产服务
pm2 stop agentrix-backend

# 2. 启动开发模式
cd /var/www/agentrix-website/backend
pm2 start npm --name "agentrix-backend-dev" -- run start:dev --watch

# 3. 查看日志
pm2 logs agentrix-backend-dev

# 4. 修改代码后，PM2 会自动重启（如果使用 --watch）
```

### 方式 B：直接运行开发模式

```bash
# 1. 停止 PM2 服务
pm2 stop agentrix-backend

# 2. 启动开发模式
cd /var/www/agentrix-website/backend
npm run start:dev

# 3. 修改代码后，NestJS 会自动重载
```

### 方式 C：使用 screen 或 tmux（保持会话）

```bash
# 1. 创建 screen 会话
screen -S agentrix-dev

# 2. 启动开发模式
cd /var/www/agentrix-website/backend
npm run start:dev

# 3. 按 Ctrl+A 然后 D 退出（服务继续运行）

# 4. 重新连接
screen -r agentrix-dev
```

## 🔧 常用操作

### 安装新依赖

```bash
cd /var/www/agentrix-website/backend
npm install openai
npm install --save-dev @types/node
```

### 重新构建

```bash
cd /var/www/agentrix-website/backend
npm run build
pm2 restart agentrix-backend --update-env
```

### 查看日志

```bash
# PM2 日志
pm2 logs agentrix-backend

# 开发模式日志（如果直接运行）
# 日志会直接输出到终端
```

### 测试 API

```bash
# 在服务器上直接测试
curl http://localhost:3001/api/openai/functions

# 测试对话接口
curl -X POST http://localhost:3001/api/openai/chat \
  -H "Content-Type: application/json" \
  -d '{
    "messages": [
      {"role": "user", "content": "我要买 iPhone 15"}
    ],
    "context": {
      "sessionId": "test-123"
    }
  }'
```

## 📁 项目结构

```
/var/www/agentrix-website/
├── backend/
│   ├── src/
│   │   └── modules/
│   │       └── ai-integration/
│   │           ├── gemini/
│   │           └── openai/          # 新创建的 OpenAI 集成
│   ├── dist/                         # 编译后的文件
│   ├── .env                          # 环境变量
│   └── package.json
└── agentrixfrontend/
```

## 🛠️ 编辑代码

### 方式 1：使用 vim/nano（服务器上直接编辑）

```bash
# 使用 vim
vim /var/www/agentrix-website/backend/src/modules/ai-integration/openai/openai-integration.service.ts

# 或使用 nano（更简单）
nano /var/www/agentrix-website/backend/src/modules/ai-integration/openai/openai-integration.service.ts
```

### 方式 2：使用 VS Code Remote SSH（推荐）

1. **安装 VS Code Remote SSH 扩展**
2. **连接到服务器：**
   - 按 `F1` 或 `Ctrl+Shift+P`
   - 输入 "Remote-SSH: Connect to Host"
   - 输入：`root@129.226.152.88`
3. **打开项目文件夹：**
   - `/var/www/agentrix-website`

**优势：**
- ✅ 完整的 VS Code 功能
- ✅ 语法高亮、自动补全
- ✅ 直接在服务器上编辑
- ✅ 文件浏览器、终端集成

### 方式 3：使用 SFTP 同步（本地编辑，自动上传）

可以使用 VS Code 的 SFTP 扩展，本地编辑后自动同步到服务器。

## 🔄 开发流程示例

### 示例：添加 OpenAI 集成

```bash
# 1. SSH 连接到服务器
ssh root@129.226.152.88

# 2. 进入项目目录
cd /var/www/agentrix-website/backend

# 3. 安装依赖
npm install openai

# 4. 创建文件（或使用 VS Code Remote SSH 编辑）
# 文件已创建：openai-integration.service.ts, controller.ts, module.ts

# 5. 启动开发模式
pm2 stop agentrix-backend
pm2 start npm --name "agentrix-backend-dev" -- run start:dev

# 6. 查看日志
pm2 logs agentrix-backend-dev

# 7. 测试
curl http://localhost:3001/api/openai/functions

# 8. 如果一切正常，切换回生产模式
pm2 stop agentrix-backend-dev
npm run build
pm2 restart agentrix-backend --update-env
```

## ⚠️ 注意事项

### 1. 备份重要文件

```bash
# 修改前备份
cp backend/src/modules/ai-integration/openai/openai-integration.service.ts \
   backend/src/modules/ai-integration/openai/openai-integration.service.ts.bak
```

### 2. 使用 Git 版本控制

```bash
# 提交更改
cd /var/www/agentrix-website
git add .
git commit -m "Add OpenAI integration"
git push
```

### 3. 环境变量

确保 `.env` 文件中有：
```env
OPENAI_API_KEY=your-api-key
GEMINI_API_KEY=your-api-key
```

### 4. 端口占用

如果开发模式和生产模式冲突：
```bash
# 检查端口占用
lsof -i :3001

# 停止占用端口的进程
pm2 stop agentrix-backend
```

## 🎯 当前任务：完成 OpenAI 集成

### 步骤 1：安装依赖

```bash
ssh root@129.226.152.88
cd /var/www/agentrix-website/backend
npm install openai
```

### 步骤 2：确认文件已创建

```bash
# 检查文件是否存在
ls -la src/modules/ai-integration/openai/
```

应该看到：
- `openai-integration.service.ts`
- `openai-integration.controller.ts`
- `openai-integration.module.ts`

### 步骤 3：确认模块注册

```bash
# 检查 app.module.ts（如果存在源代码）
# 或在 dist/app.module.js 中确认已注册
grep -r "OpenAIIntegrationModule" dist/app.module.js
```

### 步骤 4：重新构建

```bash
npm run build
```

### 步骤 5：重启服务

```bash
pm2 restart agentrix-backend --update-env
```

### 步骤 6：测试

```bash
curl http://localhost:3001/api/openai/functions
```

## 📋 开发检查清单

- [ ] SSH 连接到服务器
- [ ] 安装 `openai` npm 包
- [ ] 确认文件已创建
- [ ] 重新构建项目
- [ ] 重启服务
- [ ] 测试 API 端点
- [ ] 测试对话接口

## 🚀 推荐工作流

1. **使用 VS Code Remote SSH** - 最佳开发体验
2. **开发模式运行** - 自动重载，快速迭代
3. **Git 版本控制** - 随时提交和回滚
4. **测试后部署** - 确认无误后切换到生产模式


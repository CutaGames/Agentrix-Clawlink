...
function buildDefaultFallbackPhrases(displayName) {
...
export function resolveMobileWakeWordConfig(settings) {
    const extra = (Constants.expoConfig?.extra ?? {});
    const extraWakeWord = extra.wakeWord ?? {};
    const configuredDisplayName = settings?.displayName?.trim() ||
        extraWakeWord.displayName?.trim() ||
        process.env.EXPO_PUBLIC_WAKE_WORD_LABEL ||
        'Hey Agentrix';
    const fallbackPhrases = settings?.fallbackPhrases?.length
    ...
    return {
        ...
        displayName,
        sensitivity: clampSensitivity(parseNumber(settings?.sensitivity ?? extraWakeWord.sensitivity ?? process.env.EXPO_PUBLIC_WAKE_WORD_SENSITIVITY, 0.65)),

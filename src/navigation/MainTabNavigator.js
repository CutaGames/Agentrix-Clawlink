...
function TabIcon({ emoji, focused, badge }) {
    return (<View style={{ alignItems: 'center', paddingTop: 4 }}>
      <View>
        <Text style={{ fontSize: 20, opacity: focused ? 1 : 0.55 }}>{emoji}</Text>
        {badge && badge > 0 ? (<View style={{
...
export function MainTabNavigator() {
    ...
    return (<Tab.Navigator id={undefined} initialRouteName={initialRouteName} screenOptions={{
            headerShown: false,
            tabBarStyle: {
      ...
      <Tab.Screen name="Agent" component={AgentStackNavigator} options={{
            title: t({ en: 'Chat', zh: '对话' }),
            tabBarIcon: ({ focused }) => <TabIcon emoji="💬" focused={focused}/>,
        }}/>
      <Tab.Screen name="Discover" component={DiscoverStackNavigator} options={{

...
export const setApiConfig = (next) => {
    config = {
        ...config,
        ...next,
...
export async function uploadChatAttachment(file) {
    ...
    const uploaded = await apiFetch('/upload/chat-attachment', {
        method: 'POST',
        body: formData,
    });
...
export const authApi = {
    ...
    async loginWithWallet(params) {
        const result = await apiFetch('/auth/wallet/login', {
            method: 'POST',
...
export const personalApi = {
    ...
    async getAirdrops(status) {
        const query = status ? `?status=${status}` : '';
        try {
            const result = await apiFetch(`/auto-earn/airdrops${query}`);
            // 转换后端格式为前端格式
            return (result.airdrops || result || []).map((item) => ({
                id: item.id,
    ...
    async discoverAirdrops() {
        try {
            const result = await apiFetch('/auto-earn/airdrops/discover', { method: 'POST' });
    ...
    async getAutoEarnStats() {
        try {
            const result = await apiFetch('/auto-earn/stats');
            return {
...
export const merchantApi = {
    ...
    async getStats(days = 7) {
        try {
            const result = await apiFetch(`/merchant/stats?days=${days}`);
            return {
    ...
    async getSplitPlans() {
        try {
            const result = await apiFetch('/commerce/split-plans');
            return (result.plans || result || []).map((plan) => ({
                id: plan.id,
                name: plan.name,
    ...
    async getSettlements(status) {
        const query = status ? `?status=${status}` : '';
        try {
            const result = await apiFetch(`/commerce/settlements${query}`);
            return (result.settlements || result || []).map((s) => ({
                id: s.id,
                amount: s.amount,
    ...
    async createPaymentLink(params) {
        const result = await apiFetch('/pay-intents/create', {
            method: 'POST',
            body: JSON.stringify({
    ...
    async previewCommission(params) {
...
export const developerApi = {
    ...
    async getDashboard() {
        try {
            const result = await apiFetch('/developer-accounts/dashboard');
            return {
    ...
    async getBudgetPools() {
        try {
            const result = await apiFetch('/commerce/budget-pools');
            return (result.pools || result || []).map((pool) => ({
                id: pool.id,
                name: pool.name,
    ...
    async getMilestones(poolId) {
        ...
        try {
            const result = await apiFetch(`/commerce/milestones${query}`);
            return (result.milestones || result || []).map((m) => ({
                id: m.id,
    ...
    async getAvailableOrders() {
        try {
            const result = await apiFetch('/merchant-tasks?status=open');
            return (result.tasks || result || []).map((task) => ({
                id: task.id,
                title: task.title,
...
export const identityApi = {
    ...
    async checkIdentityStatus() {
        ...
        try {
            const merchantProfile = await apiFetch('/merchant/profile');
        ...
        try {
            const devAccount = await apiFetch('/developer-accounts/my');

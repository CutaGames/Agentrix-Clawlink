 ...
 */
export function promptForUpdate(manifest) {
    const version = manifest?.version || manifest?.runtimeVersion || 'new version';
    Alert.alert('🔄 New Update Available', `A new version (${version}) is available. Your data will be preserved during the update.\n\nUpdate now?`, [
 ...
 */
export async function silentBackgroundUpdate() {
    if (!Updates || __DEV__)
        return;
    try {

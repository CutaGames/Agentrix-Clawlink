 ...
 */
export async function probeInstanceUrl(url, token) {
    return apiFetch('/openclaw/bridge/probe', {
        method: 'POST',
        body: JSON.stringify({ url, token }),
    });
}
/** Trigger full data migration from a bound instance → Agentrix */
export async function migrateInstance(instanceId) {
    return apiFetch(`/openclaw/bridge/${instanceId}/migrate`, {
        method: 'POST',
    });
}
/** Fetch recommended ports / paths from backend for LAN scanning */
export async function getDiscoveryConfig() {
    return apiFetch('/openclaw/bridge/discover/config');
}
 ...
 */
export async function lanScan(config, subnet = '192.168.1', onProgress) {
    ...
    const candidates = hosts.flatMap((h) => ports.map((p) => ({ url: `http://${h}:${p}` })));
    const found = [];
    let checked = 0;
            ...
            try {
                ...
                if (res.ok || res.status === 401 || res.status === 403) {
                    ...
                    return {
                        ...
                        probe: {
                            reachable: true,
                            instanceName: (data.name ?? data.agentName ?? data.instanceName),

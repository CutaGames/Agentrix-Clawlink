...
function meanAbsoluteDifference(samples) {
    if (samples.length <= 1) {
        return 0;
    }
    let total = 0;
...
function goertzelEnergy(samples, sampleRate, frequency) {
    if (!samples.length) {
        return 0;
    }
...
function trimSilence(int16Samples) {
    const frameSize = 160;
...
function toFeatureVector(int16Samples) {
...
function scoreWakeWordMatch(model, vector) {
...
async function stopProcessor(processor) {
    try {
...
async function captureUtterance(timeoutMs = DEFAULT_TIMEOUT_MS) {
...
export function getLocalWakeWordModelReadiness(model) {
...
export function hasLocalWakeWordModel(model) {
    return getLocalWakeWordModelReadiness(model).ready;
}
export function getLocalWakeWordSampleCount(model) {
    return model?.samples?.length ?? 0;
}
export function appendLocalWakeWordSample(existingModel, sample, displayName) {
...
export async function captureLocalWakeWordSample(options = {}) {
...
export async function runLocalWakeWordSelfCheck(model, threshold) {

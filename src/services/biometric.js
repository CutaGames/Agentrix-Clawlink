...
class BiometricService {
    constructor() {
        this.BIOMETRIC_ENABLED_KEY = 'biometric_enabled';
        this.BIOMETRIC_LAST_AUTH_KEY = 'biometric_last_auth';
    }
    /**
     * 检查设备是否支持生物识别
     */
    ...
    async authenticate(reason) {
        try {
            ...
            if (!status.isEnrolled) {
                Alert.alert('未设置', `请先在系统设置中注册${this.getBiometricTypeName(status.biometricTypes)}`);
                return false;
            }
            ...
            if (result.success) {
                // 记录最后认证时间
                await SecureStore.setItemAsync(this.BIOMETRIC_LAST_AUTH_KEY, Date.now().toString());
                return true;
            }
    ...
    async setEnabled(enabled) {
        try {
            ...
            await SecureStore.setItemAsync(this.BIOMETRIC_ENABLED_KEY, enabled ? 'true' : 'false');
            return true;
        }
        catch (error) {
     ...
     * 检查是否需要重新认证（超过指定时间）
     */
    async needsReauth(maxAgeMs = 5 * 60 * 1000) {
     ...
     * 用于敏感操作的快速认证
     */
    async authenticateForSensitiveAction(action) {

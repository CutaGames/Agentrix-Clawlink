 ...
 * 检查当前用户是否已有 MPC 钱包
 */
export async function checkMPCWallet() {
    return apiFetch('/mpc-wallet/check');
}
/**
 * 为社交登录用户创建 MPC 钱包
 ...
 */
export async function createMPCWalletForSocialLogin(socialProviderId, chain = 'BSC') {
 ...
 */
export async function storeShardA(encryptedShardA) {
    if (!encryptedShardA)
        return;
    try {
 ...
 * 获取存储的分片 A
 */
export async function getStoredShardA() {
    try {
 ...
 * 存储恢复码（分片 C）
 */
export async function storeRecoveryCode(encryptedShardC) {
    if (!encryptedShardC)
        return;
    try {
 ...
 * 标记用户已完成恢复码备份
 */
export async function markMPCBackupCompleted() {
    try {
 ...
 * 检查用户是否确认已备份恢复码
 */
export async function isMPCBackupCompleted() {
    try {
 ...
 */
export async function resetBackupConfirmation() {
    try {
 ...
 * 清除所有 MPC 钱包数据（登出时调用）
 */
export async function clearMPCWalletData() {
    try {
 ...
 */
export async function ensureMPCWallet(socialProviderId) {
    try {
        // 1. 检查是否已有钱包

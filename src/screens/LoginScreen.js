...
import { View, Text, StyleSheet, TouchableOpacity, ActivityIndicator, Alert, Image, SafeAreaView, Dimensions, TextInput, KeyboardAvoidingView, Platform, ScrollView, } from 'react-native';
import { LinearGradient } from 'expo-linear-gradient';
import { colors } from '../theme/colors';
import { useAuthStore } from '../stores/authStore';
import { loginWithGoogle, loginWithTwitter, loginWithDiscord, loginWithTelegram, sendEmailCode, loginWithEmailCode, } from '../services/auth';
...
export const LoginScreen = ({ navigation }) => {
    ...
    return (<View style={styles.container}>
      <LinearGradient colors={['#0B1220', '#0F1A2E', '#131F38', '#0B1220']} style={StyleSheet.absoluteFillObject} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}/>
      {/* Subtle orbs */}
      ...
      <SafeAreaView style={{ flex: 1 }}>
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
            ...
            <TouchableOpacity style={[styles.walletPrimaryBtn, isDisabled && styles.btnDisabled]} onPress={handleWalletLogin} disabled={isDisabled} activeOpacity={0.85}>
              <LinearGradient colors={['#3B82F6', '#6366F1']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={StyleSheet.absoluteFillObject}/>
            ...
            <View style={styles.socialGrid}>
              {SOCIAL_BUTTONS.map(btn => (<TouchableOpacity key={btn.provider} style={[styles.socialBtn, isDisabled && styles.btnDisabled]} onPress={() => handleSocialLogin(btn.provider)} disabled={isDisabled} activeOpacity={0.75}>
            ...
            <TouchableOpacity style={styles.emailToggle} onPress={() => { setShowEmail(v => !v); setOtpSent(false); setOtpCode(''); }} activeOpacity={0.7}>
              <Text style={styles.emailToggleText}>
                {showEmail ? '▲ Hide email login' : '✉ Continue with Email'}
              </Text>
            </TouchableOpacity>

            {showEmail && (<View style={styles.emailCard}>
                <View style={styles.emailRow}>
                  <TextInput style={styles.emailInput} placeholder="your@email.com" placeholderTextColor={colors.muted + '80'} value={email} onChangeText={(t) => { setEmail(t); if (otpSent) {
            setOtpSent(false);
            setOtpCode('');
        } }} keyboardType="email-address" autoCapitalize="none" autoCorrect={false} editable={!isDisabled}/>
                  <TouchableOpacity style={[styles.sendCodeBtn, (isDisabled || countdown > 0) && styles.btnDisabled]} onPress={handleSendCode} disabled={isDisabled || countdown > 0}>
                ...
                {otpSent && (<>
                    <TextInput style={styles.otpInput} placeholder="• • • • • •" placeholderTextColor={colors.muted + '60'} value={otpCode} onChangeText={setOtpCode} keyboardType="number-pad" maxLength={6} editable={!isDisabled} autoFocus/>
                    <TouchableOpacity style={[styles.verifyBtn, isDisabled && styles.btnDisabled]} onPress={handleVerifyCode} disabled={isDisabled}>
                      {loadingProvider === 'email' && otpSent
                    ? <ActivityIndicator size="small" color="#fff"/>
            ...
            {/* Loading indicator */}
            {loadingProvider && loadingProvider !== 'email' && (<View style={styles.loadingRow}>

...
const fetchAppVersion = () => apiFetch('/app/version');
...
const fetchApiKeys = () => apiFetch('/api-keys');
const createApiKey = (name) => apiFetch('/api-keys', {
    method: 'POST',
    body: JSON.stringify({ name, expiresInDays: 365 }),
});
const deleteApiKey = (id) => apiFetch(`/api-keys/${id}`, { method: 'DELETE' });
export const SettingsScreen = () => {
    ...
    const handleUpdate = () => {
        ...
        if (Platform.OS !== 'android') {
            Alert.alert(t({ en: 'Update Available', zh: '有新版本' }), t({ en: 'Please update via the App Store / TestFlight.', zh: '请通过 App Store / TestFlight 更新。' }));
            return;
        }
        Alert.alert(t({ en: `Update to v${versionInfo.latestVersion}`, zh: `更新至 v${versionInfo.latestVersion}` }), (versionInfo.releaseNotes ? versionInfo.releaseNotes + '\n\n' : '') +
    ...
    const createMutation = useMutation({
        ...
        onSuccess: (res) => {
            ...
            setNewKeyName('');
            Alert.alert('🔑 API Key Created', `Save this key — it will only be shown once:\n\n${res.data.apiKey}`, [{ text: 'Copy & Close', onPress: () => { } }]);
        },
        onError: (e) => Alert.alert('Error', e.message || 'Failed to create key'),
    });
    ...
    const handleLogout = () => {
        Alert.alert(t({ en: 'Logout', zh: '退出登录' }), t({ en: 'Are you sure you want to sign out?', zh: '确定要退出登录吗？' }), [
            { text: t({ en: 'Cancel', zh: '取消' }), style: 'cancel' },
            {
    ...
    return (<ScrollView contentContainerStyle={{ padding: 16, gap: 16 }}>
      {user && (<Card>
          ...
          <View style={{ marginTop: 12, gap: 6 }}>
            ...
            {user.email && (<Text style={{ color: colors.muted, fontSize: 13 }}>
                Email: <Text style={{ color: colors.text }}>{user.email}</Text>
              </Text>)}
            {user.walletAddress && (<Text style={{ color: colors.muted, fontSize: 13 }}>
                Wallet: <Text style={{ color: colors.text }}>{user.walletAddress.slice(0, 6)}...{user.walletAddress.slice(-4)}</Text>
              </Text>)}
            {user.provider && (<Text style={{ color: colors.muted, fontSize: 13 }}>
      ...
      <Card>
        ...
        <TextInput value={baseUrl} onChangeText={setBaseUrl} autoCapitalize="none" style={{
        ...
        <TextInput value={token} onChangeText={setToken} autoCapitalize="none" secureTextEntry placeholder={t({ en: 'Leave empty to use current session', zh: '留空则使用当前会话' })} placeholderTextColor={colors.muted} style={{
            backgroundColor: colors.cardAlt,
            color: colors.text,
            padding: 12,
            borderRadius: 10,
      ...
      <Card>
        ...
        <View style={settingsStyles.langRow}>
          {([{ code: 'en', label: 'English', flag: '🇺🇸' }, { code: 'zh', label: '中文', flag: '🇨🇳' }]).map(lang => (<TouchableOpacity key={lang.code} style={[settingsStyles.langBtn, language === lang.code && settingsStyles.langBtnActive]} onPress={() => setLanguage(lang.code)} activeOpacity={0.7}>
      ...
      <Card>
        ...
        ].map((item) => (<View key={item.provider} style={{ marginTop: 10 }}>
            <Text style={{ color: colors.muted, fontSize: 12, marginBottom: 4 }}>{item.label}</Text>
            <View style={{ flexDirection: 'row', gap: 8 }}>
              <TextInput value={editingKey[item.provider] ?? (customApiKeys[item.provider] ? '••••••••' + (customApiKeys[item.provider]?.slice(-4) || '') : '')} onChangeText={(v) => setEditingKey((prev) => ({ ...prev, [item.provider]: v }))} placeholder={item.placeholder} placeholderTextColor={colors.muted} autoCapitalize="none" secureTextEntry={!editingKey[item.provider]} style={{
                flex: 1,
                backgroundColor: colors.cardAlt,
                color: colors.text,
                padding: 10,
      ...
      <Card>
        ...
        <View style={{ marginTop: 12, flexDirection: 'row', gap: 8 }}>
          <TextInput value={newKeyName} onChangeText={setNewKeyName} placeholder={t({ en: 'Key name (e.g. My App)', zh: '密鑅名称（如：我的应用）' })} placeholderTextColor={colors.muted} style={{
            flex: 1,
            backgroundColor: colors.cardAlt,
            color: colors.text,
            padding: 10,
        ...
        {/* Keys list */}
        {keysLoading ? (<ActivityIndicator size="small" color={colors.primary} style={{ marginTop: 12 }}/>) : (apiKeys && apiKeys.length > 0) ? (<View style={{ marginTop: 12, gap: 8 }}>
            {apiKeys.map((key) => (<View key={key.id} style={{
          ...
          </View>) : (<Text style={{ color: colors.muted, fontSize: 12, marginTop: 10 }}>{t({ en: 'No API keys yet.', zh: '暂无 API 密鑅。' })}</Text>)}
      </Card>

      <TouchableOpacity style={settingsStyles.logoutButton} onPress={handleLogout} activeOpacity={0.7}>
        <Text style={settingsStyles.logoutText}>{t({ en: 'Sign Out', zh: '退出登录' })}</Text>
      </TouchableOpacity>

      {/* ── App Version & Update ── */}
      <TouchableOpacity onPress={hasUpdate ? handleUpdate : undefined} activeOpacity={hasUpdate ? 0.7 : 1} style={[
        ...
        <View style={{ flex: 1 }}>
          ...
          {hasUpdate && versionInfo && (<Text style={{ color: '#10b981', fontSize: 12, marginTop: 3, fontWeight: '600' }}>
              {t({ en: `v${versionInfo.latestVersion} available — tap to update`, zh: `v${versionInfo.latestVersion} 可更新 — 点击升级` })}
            </Text>)}
        </View>
        {hasUpdate && (<View style={settingsStyles.updateBadge}>

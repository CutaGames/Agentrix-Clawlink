...
import { View, Text, TextInput, TouchableOpacity, StyleSheet, FlatList, KeyboardAvoidingView, Platform, ActivityIndicator, Modal, ScrollView, } from 'react-native';
...
async function fetchMessages(userId) {
    try {
        return apiFetch(`/social/dm/${userId}/messages`);
    }
    catch {
        return [];
    }
...
export function DirectMessageScreen() {
    ...
    const msgs = (Array.isArray(data) && data.length > 0) ? data : PLACEHOLDER_MSGS;
    ...
    const sendMut = useMutation({
        mutationFn: (payload) => sendMessage(userId, payload),
        onMutate: async (payload) => {
            ...
            qc.setQueryData(['dm-messages', userId], (old) => [...(old ?? []), optimistic]);
        },
    ...
    const handleSendSkill = useCallback((skill) => {
    ...
    const renderMsg = ({ item }) => {
        ...
        if (item.type === 'skill_card' && item.skillCard) {
            ...
            return (<View style={[styles.msgRow, isMine ? styles.msgRowMine : styles.msgRowTheirs]}>
          {!isMine && (<View style={styles.peerAvatar}>
        ...
        <View style={[styles.bubble, isMine ? styles.bubbleMine : styles.bubbleTheirs]}>
          ...
          <View style={styles.msgMeta}>
            ...
            {isMine && item.status && (<Text style={[styles.msgStatus, item.status === 'read' && styles.msgStatusRead]}>
    ...
    return (<SafeAreaView style={styles.safe} edges={['bottom']}>
      <KeyboardAvoidingView style={styles.kav} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={90}>
        ...
        <View style={styles.inputBar}>
          <TouchableOpacity style={styles.attachBtn} onPress={() => setShowSkillPicker(true)}>
            <Text style={styles.attachBtnText}>+</Text>
          </TouchableOpacity>
          <TextInput style={styles.input} placeholder="Message..." placeholderTextColor={colors.textMuted} value={input} onChangeText={setInput} multiline maxLength={2000} returnKeyType="send" onSubmitEditing={handleSend}/>
      ...
      <Modal visible={showSkillPicker} transparent animationType="slide" onRequestClose={() => setShowSkillPicker(false)}>
        <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setShowSkillPicker(false)}>
          <View style={styles.pickerSheet}>
            <View style={styles.pickerHandle}/>
            <Text style={styles.pickerTitle}>Share a Skill</Text>
            <ScrollView>
              {SAMPLE_SKILLS.map((skill) => (<TouchableOpacity key={skill.skillId} style={styles.pickerItem} onPress={() => handleSendSkill(skill)}>

...
import { View, Text, TextInput, TouchableOpacity, StyleSheet, FlatList, KeyboardAvoidingView, Platform, ActivityIndicator, Modal, ScrollView, Alert, } from 'react-native';
...
export function GroupChatScreen() {
    ...
    React.useLayoutEffect(() => {
        navigation.setOptions({
            headerRight: () => (<TouchableOpacity style={{ paddingHorizontal: 12 }} onPress={() => setShowSettings(true)}>
    ...
    const sendMut = useMutation({
        mutationFn: (content) => sendGroupMessage(groupId, content),
        onMutate: async (content) => {
            ...
            qc.setQueryData(['group-messages', groupId], (old) => [...(old ?? []), optimistic]);
        },
    ...
    const handleSend = useCallback(async () => {
        ...
        if (agentMentions.length > 0) {
            ...
            try {
                ...
                qc.setQueryData(['group-messages', groupId], (old) => (old ?? []).map((m) => m.id === typingId
                ...
                qc.setQueryData(['group-messages', groupId], (old) => (old ?? []).map((m) => m.id === typingId
                    ? { ...m, id: `agent-err-${Date.now()}`, content: `⚠️ @${agentName} is unavailable right now.`, type: 'system' }
                    : m));
            }
            finally {
    ...
    const renderMsg = ({ item }) => {
        if (item.type === 'system') {
            return (<View style={styles.systemMsgRow}>
          <Text style={styles.systemMsg}>{item.content}</Text>
        </View>);
        }
        if (item.type === 'announcement') {
            return (<View style={styles.announcementRow}>
    ...
    return (<SafeAreaView style={styles.safe} edges={['bottom']}>
      <KeyboardAvoidingView style={styles.kav} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={90}>
        ...
        <View style={styles.inputBar}>
          <TextInput style={styles.input} placeholder={`Message ${groupName}… (use @agent to invoke)`} placeholderTextColor={colors.textMuted} value={input} onChangeText={setInput} multiline maxLength={2000}/>
          ...
          <View style={styles.settingsSheet}>
            ...
            <View style={styles.settingsSection}>
              ...
              <ScrollView style={{ maxHeight: 180 }}>
                {PLACEHOLDER_MEMBERS.map((m) => (<View key={m.id} style={styles.memberRow}>

...
export function ChatListScreen() {
    ...
    const chats = (Array.isArray(data) && data.length > 0) ? data : PLACEHOLDER_CHATS;
    const filtered = chats.filter((c) => {
        const matchTab = tab === 'all' || c.type === tab;
        const matchSearch = !search || c.name.toLowerCase().includes(search.toLowerCase());
        return matchTab && matchSearch;
    });
    const renderItem = ({ item }) => (<TouchableOpacity style={styles.chatRow} onPress={() => {
            if (item.type === 'dm') {
      ...
      <View style={styles.chatInfo}>
        ...
        {item.memberCount && (<Text style={styles.memberCount}>{item.memberCount} {t({ en: 'members', zh: '成员' })}</Text>)}
      </View>
    </TouchableOpacity>);
    return (<SafeAreaView style={styles.safe} edges={['bottom']}>
      <View style={styles.container}>
        ...
        <View style={styles.searchRow}>
          <TextInput style={styles.searchInput} placeholder={t({ en: 'Search chats...', zh: '搜索聊天...' })} placeholderTextColor={colors.textMuted} value={search} onChangeText={setSearch}/>
        </View>

        {/* Tabs */}
        <View style={styles.tabs}>
          {['all', 'dm', 'group'].map((tabKey) => (<TouchableOpacity key={tabKey} style={[styles.tab, tab === tabKey && styles.tabActive]} onPress={() => setTab(tabKey)}>
        ...
        {isLoading ? (<ActivityIndicator color={colors.accent} style={{ marginTop: 40 }}/>) : (<FlatList data={filtered} keyExtractor={(c) => c.id} renderItem={renderItem} refreshControl={<RefreshControl refreshing={isLoading} onRefresh={refetch} tintColor={colors.accent}/>} ItemSeparatorComponent={() => <View style={styles.separator}/>} ListEmptyComponent={<View style={styles.empty}>
                <Text style={styles.emptyIcon}>💬</Text>
                <Text style={styles.emptyText}>{t({ en: 'No chats yet', zh: '暂无聊天' })}</Text>
              </View>}/>)}

        {/* Floating: Create Group */}
        <TouchableOpacity style={styles.fab} onPress={() => navigation.navigate('GroupChat', { groupId: 'new', groupName: 'New Group' })}>

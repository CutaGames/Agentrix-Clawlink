...
function ScoreRing({ score }) {
    ...
    return (<View style={[rs.ring, { borderColor: col + '33' }]}>
...
export function UserProfileScreen() {
    ...
    const { data: profile } = useQuery({
        ...
        queryFn: () => apiFetch(`/social/users/${userId}`).catch(() => null),
        retry: false,
    });
    const { data: rep, isLoading } = useQuery({
        queryKey: ['user-reputation', userId],
        queryFn: () => apiFetch(`/social/users/${userId}/reputation`).then((r) => r.data).catch(() => null),
        retry: false,
    });
    const { data: posts } = useQuery({
        queryKey: ['user-posts', userId],
        queryFn: () => apiFetch(`/social/users/${userId}/posts`).catch(() => []),
        retry: false,
    });
    const { data: skills } = useQuery({
        queryKey: ['user-skills', userId],
        queryFn: () => apiFetch(`/social/users/${userId}/skills`).catch(() => []),
        retry: false,
    });
    const followMut = useMutation({
        mutationFn: () => apiFetch(`/social/users/${userId}/follow`, { method: 'POST' }),
        onMutate: () => {
            qc.setQueryData(['user-profile', userId], (old) => old ? { ...old, isFollowing: !old.isFollowing } : old);
            qc.setQueryData(['user-reputation', userId], (old) => old ? { ...old, followerCount: old.followerCount + (profile?.isFollowing ? -1 : 1) } : old);
        },
        onSettled: () => {
    ...
    // ── Header ──
    const HeaderComponent = () => (<View style={styles.header}>
      ...
      <View style={styles.topRow}>
        ...
        {!isMe ? (<View style={styles.headerActions}>
            <TouchableOpacity style={[styles.followBtn, profile.isFollowing && styles.followingBtn]} onPress={() => followMut.mutate()}>
          ...
          </View>) : (<TouchableOpacity style={styles.editBtn}>
      ...
      {profile.badges && profile.badges.length > 0 && (<View style={styles.badgesRow}>
          {profile.badges.map((b) => {
                const cfg = BADGE_CONFIG[b] ?? { icon: '🏅', color: colors.accent };
                return (<View key={b} style={[styles.badge, { borderColor: cfg.color + '66', backgroundColor: cfg.color + '1a' }]}>
      ...
      <View style={styles.tabRow}>
        {['agents', 'skills', 'showcase'].map((tab) => (<TouchableOpacity key={tab} style={[styles.tab, activeTab === tab && styles.tabActive]} onPress={() => setActiveTab(tab)}>
    ...
    // ── Tab content ──
    const AgentsTab = () => (<View style={styles.agentsList}>
      {rep.agentDeploys > 0 ? ([
        ...
        ].filter((item) => item.value > 0).map((item) => (<View key={item.key} style={styles.agentCard}>
    ...
    const renderSkill = ({ item }) => (<View style={styles.skillCard}>
      <View style={{ flex: 1, gap: 4 }}>
        <Text style={styles.skillName}>{item.name}</Text>
        <Text style={styles.skillDesc} numberOfLines={2}>{item.description}</Text>
        {item.downloads != null && (<Text style={styles.skillDl}>⬇ {fmtK(item.downloads)} {t({ en: 'installs', zh: '安装' })}</Text>)}
        {typeof item.rating === 'number' && item.rating > 0 && (<Text style={styles.skillDl}>⭐ {item.rating.toFixed(1)}</Text>)}
      </View>
    ...
    const renderPost = ({ item }) => (<TouchableOpacity style={styles.postCard} onPress={() => navigation.navigate('PostDetail', { postId: item.id })}>
    ...
    return (<SafeAreaView style={styles.safe} edges={['bottom']}>
      {activeTab === 'agents' ? (<FlatList data={[]} keyExtractor={() => 'agents'} renderItem={() => null} ListHeaderComponent={<><HeaderComponent /><AgentsTab /></>} contentContainerStyle={styles.list} showsVerticalScrollIndicator={false}/>) : activeTab === 'skills' ? (<FlatList data={skills ?? []} keyExtractor={(item) => item.id} renderItem={renderSkill} ListHeaderComponent={<HeaderComponent />} contentContainerStyle={styles.list} showsVerticalScrollIndicator={false} ListEmptyComponent={<View style={styles.empty}>
              <Text style={styles.emptyIcon}>⚡</Text>
              <Text style={styles.emptyText}>{t({ en: 'No skills yet', zh: '暂无技能' })}</Text>
            </View>}/>) : (<FlatList data={posts ?? []} keyExtractor={(item) => item.id} renderItem={renderPost} ListHeaderComponent={<HeaderComponent />} contentContainerStyle={styles.list} showsVerticalScrollIndicator={false} ListEmptyComponent={<View style={styles.empty}>
              <Text style={styles.emptyIcon}>📜</Text>

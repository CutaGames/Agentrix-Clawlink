...
import { View, Text, TouchableOpacity, StyleSheet, ScrollView, Alert, ActivityIndicator, RefreshControl, TextInput, } from 'react-native';
...
function TelegramGuide({ botUsername, webhookUrl, onSetup, settingUp, t }) {
    return (<View style={styles.guideContainer}>
      ...
      <View style={styles.guideStep}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <TouchableOpacity style={[styles.guideBtn, settingUp && { opacity: 0.6 }]} onPress={onSetup} disabled={settingUp}>
            {settingUp ? (<ActivityIndicator color="#000" size="small"/>) : (<Text style={styles.guideBtnText}>⚡ {t({ en: 'Register Webhook', zh: '注册 Webhook' })}</Text>)}
          </TouchableOpacity>
        </View>
      </View>
      ...
      <View style={styles.guideStep}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <TouchableOpacity style={styles.guideBtnOutline} onPress={() => Linking.openURL(`https://t.me/${botUsername}`)}>
...
function DiscordGuide({ clientId, interactionsUrl, t }) {
    return (<View style={styles.guideContainer}>
      ...
      <View style={styles.guideStep}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <TouchableOpacity style={styles.guideBtnOutline} onPress={() => Linking.openURL(`https://discord.com/developers/applications/${clientId}/information`)}>
      ...
      <View style={styles.guideStep}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <TouchableOpacity style={styles.guideBtnOutline} onPress={() => copyToClipboard(interactionsUrl, 'Interactions URL')}>
...
// ── Platform Card ─────────────────────────────────────────────────────────────
function PlatformCard({ platform, status, onSetupTelegram, settingUp, expanded, onToggleExpand, t, }) {
    const meta = PLATFORM_META[platform];
    const connected = platform === 'telegram' ? status.telegram.connected
        : platform === 'discord' ? status.discord.connected
            : status.twitter.connected;
    const webhookUrl = platform === 'telegram' ? status.telegram.webhookUrl
        : platform === 'discord' ? status.discord.interactionsUrl
            : status.twitter.webhookUrl;
    const subtitle = platform === 'telegram'
        ? `@${status.telegram.botUsername}`
        : platform === 'discord'
            ? `Client: ${status.discord.clientId}`
            : 'Account Activity API';
    return (<View style={[styles.card, { borderLeftColor: meta.color, borderLeftWidth: 3 }]}>
      ...
      {expanded && (<>
          {/* Webhook URL row */}
          <TouchableOpacity style={styles.urlRow} onPress={() => copyToClipboard(webhookUrl, 'Webhook URL')} activeOpacity={0.7}>
            <Text style={styles.urlText} numberOfLines={1}>{webhookUrl}</Text>
            <Text style={styles.copyIcon}>📋</Text>
          </TouchableOpacity>

          {/* Platform-specific guided setup */}
          {platform === 'telegram' && (<TelegramGuide botUsername={status.telegram.botUsername} webhookUrl={status.telegram.webhookUrl} onSetup={onSetupTelegram} settingUp={settingUp} t={t}/>)}

          {platform === 'discord' && (<DiscordGuide clientId={status.discord.clientId} interactionsUrl={status.discord.interactionsUrl} t={t}/>)}

          {platform === 'twitter' && (<View style={styles.guideContainer}>
              ...
              <View style={styles.guideStep}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <TouchableOpacity style={styles.guideBtnOutline} onPress={() => Linking.openURL('https://developer.x.com')}>
...
function StrategyPicker({ platform, configs, t }) {
    ...
    const saveMut = useMutation({
        mutationFn: (strategy) => apiFetch(`/social/reply-config/${platform}`, {
            method: 'POST',
    ...
    return (<View style={styles.strategyCard}>
      ...
      <View style={styles.strategyOptions}>
        {STRATEGY_OPTIONS.map((opt) => (<TouchableOpacity key={opt.value} style={[
...
// ── Approval Queue Item ─────────────────────────────────────────────────────
function ApprovalItem({ event, t }) {
    const qc = useQueryClient();
    const [editedReply, setEditedReply] = useState(event.agentDraftReply ?? '');
    const approveMut = useMutation({
        mutationFn: () => apiFetch(`/social/events/${event.id}/approve`, {
            method: 'POST',
    ...
    const rejectMut = useMutation({
        mutationFn: () => apiFetch(`/social/events/${event.id}/reject`, { method: 'POST' }),
        onSuccess: () => {
            qc.invalidateQueries({ queryKey: ['pending-approvals'] });
            qc.invalidateQueries({ queryKey: ['social-events'] });
        },
    ...
    return (<View style={styles.approvalCard}>
      ...
      <View style={styles.approvalDraft}>
        ...
        <TextInput style={styles.approvalInput} value={editedReply} onChangeText={setEditedReply} multiline placeholder={t({ en: 'Edit reply before sending...', zh: '发送前编辑回复...' })} placeholderTextColor={colors.textMuted}/>
      </View>

      {/* Actions */}
      <View style={styles.approvalActions}>
        <TouchableOpacity style={[styles.approvalBtn, styles.approvalBtnReject]} onPress={() => rejectMut.mutate()} disabled={rejectMut.isPending}>
          <Text style={styles.approvalBtnRejectText}>✕ {t({ en: 'Reject', zh: '拒绝' })}</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[styles.approvalBtn, styles.approvalBtnApprove]} onPress={() => approveMut.mutate()} disabled={approveMut.isPending}>
          {approveMut.isPending ? (<ActivityIndicator color="#000" size="small"/>) : (<Text style={styles.approvalBtnApproveText}>✓ {t({ en: 'Approve & Send', zh: '批准并发送' })}</Text>)}
        </TouchableOpacity>
      </View>
    </View>);
}
...
export function SocialListenerScreen() {
    ...
    const [expandedPlatform, setExpandedPlatform] = useState('telegram');
    // Platform status
    const { data: statusData, isLoading: statusLoading, refetch: refetchStatus } = useQuery({
        queryKey: ['social-listener-status'],
        queryFn: () => apiFetch('/social/callback/status'),
        retry: false,
    });
    // Events (persisted)
    const { data: eventsData, isLoading: eventsLoading, refetch: refetchEvents } = useQuery({
        queryKey: ['social-events'],
        queryFn: () => apiFetch('/social/events?limit=50'),
    ...
    const { data: pendingData, isLoading: pendingLoading, refetch: refetchPending } = useQuery({
        ...
        queryFn: () => apiFetch('/social/events/pending'),
    ...
    const { data: configsData } = useQuery({
        ...
        queryFn: () => apiFetch('/social/reply-config'),
        retry: false,
    });
    const setupMut = useMutation({
        mutationFn: () => apiFetch('/social/callback/telegram/setup', { method: 'POST' }),
        onSuccess: (res) => {
            if (res.ok) {
                Alert.alert(t({ en: '✅ Webhook Registered', zh: '✅ Webhook 已注册' }), t({ en: 'Telegram bot is now receiving messages.', zh: 'Telegram 机器人现已开始接收消息。' }));
            }
            else {
        ...
        onError: (e) => Alert.alert(t({ en: 'Error', zh: '错误' }), e.message ?? ''),
    });
    ...
    const pending = pendingData?.events ?? [];
    ...
    return (<ScrollView style={styles.container} contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={statusLoading && eventsLoading} onRefresh={onRefresh} tintColor={colors.accent}/>}>
      {/* Header */}
      ...
      <View style={styles.screenTabs}>
        {TABS.map((tab) => (<TouchableOpacity key={tab.key} style={[styles.screenTab, activeTab === tab.key && styles.screenTabActive]} onPress={() => setActiveTab(tab.key)}>
            ...
            {tab.badge !== undefined && tab.badge > 0 && (<View style={styles.tabBadge}>
      ...
      {activeTab === 'connections' && (<>
          {statusLoading ? (<ActivityIndicator color={colors.accent} style={{ marginTop: 32 }}/>) : status ? (<>
              <PlatformCard platform="telegram" status={status} onSetupTelegram={() => setupMut.mutate()} settingUp={setupMut.isPending} expanded={expandedPlatform === 'telegram'} onToggleExpand={() => setExpandedPlatform(expandedPlatform === 'telegram' ? null : 'telegram')} t={t}/>
              <PlatformCard platform="discord" status={status} onSetupTelegram={() => { }} settingUp={false} expanded={expandedPlatform === 'discord'} onToggleExpand={() => setExpandedPlatform(expandedPlatform === 'discord' ? null : 'discord')} t={t}/>
              <PlatformCard platform="twitter" status={status} onSetupTelegram={() => { }} settingUp={false} expanded={expandedPlatform === 'twitter'} onToggleExpand={() => setExpandedPlatform(expandedPlatform === 'twitter' ? null : 'twitter')} t={t}/>
          ...
          {['telegram', 'discord', 'twitter'].map((p) => (<StrategyPicker key={p} platform={p} configs={configs} t={t}/>))}
        </>)}

      {/* ─── Approvals Tab ───────────────────────────────────────────────── */}
      {activeTab === 'approvals' && (<>
          {pendingLoading ? (<ActivityIndicator color={colors.accent} style={{ marginTop: 32 }}/>) : pending.length === 0 ? (<View style={styles.emptyEvents}>
            ...
            </View>) : (pending.map((ev) => <ApprovalItem key={ev.id} event={ev} t={t}/>))}
        </>)}

      {/* ─── Events Tab ──────────────────────────────────────────────────── */}
      {activeTab === 'events' && (<>
          {eventsLoading ? (<ActivityIndicator color={colors.accent} style={{ marginTop: 32 }}/>) : events.length === 0 ? (<View style={styles.emptyEvents}>
              <Text style={styles.emptyEventsIcon}>📭</Text>
              <Text style={styles.emptyEventsText}>
                ...
                const statusColor = ev.replyStatus === 'sent' || ev.replyStatus === 'auto_sent' ? '#22c55e'
                    : ev.replyStatus === 'pending' ? '#f59e0b'
                        : ev.replyStatus === 'rejected' || ev.replyStatus === 'failed' ? '#ef4444'
                            : colors.textMuted;
                return (<View key={ev.id} style={styles.eventRow}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    {ev.finalReply && (<View style={styles.replyPreview}>

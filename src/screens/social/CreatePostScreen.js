...
export function CreatePostScreen() {
    ...
    return (<KeyboardAvancingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.root}>
      <ScrollView style={styles.scroll} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.header}>
          ...
          <TouchableOpacity onPress={handlePost} disabled={isPending || !content.trim()} style={[styles.postBtn, (!content.trim() || isPending) && styles.postBtnDisabled]}>
            {isPending ? (<ActivityIndicator color="#fff" size="small"/>) : (<Text style={styles.postBtnText}>{t({ en: 'Post', zh: '发布' })}</Text>)}
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.inputWrapper}>
          ...
          <TextInput style={styles.input} placeholder={t({ en: 'Share your agent workflow, skill, or discovery with the community...', zh: '与社区分享你的 agent 工作流、技能或发现...' })} placeholderTextColor={colors.textMuted} multiline numberOfLines={6} maxLength={2000} value={content} onChangeText={setContent} autoFocus/>
        ...
        <View style={styles.tagsSection}>
          ...
          <View style={styles.tagGrid}>
            {PRESET_TAGS.map((tag) => (<TouchableOpacity key={tag} style={[styles.tagChip, selectedTags.includes(tag) && styles.tagChipActive]} onPress={() => toggleTag(tag)}>
          ...
          <View style={styles.customTagRow}>
            <TextInput style={styles.customTagInput} placeholder={t({ en: 'Add custom tag...', zh: '添加自定义标签...' })} placeholderTextColor={colors.textMuted} value={customTag} onChangeText={setCustomTag} onSubmitEditing={addCustomTag} returnKeyType="done" autoCapitalize="none"/>
          ...
          {selectedTags.filter((t) => !PRESET_TAGS.includes(t)).length > 0 && (<View style={styles.selectedCustomTags}>
              {selectedTags
                .filter((t) => !PRESET_TAGS.includes(t))
                .map((t) => (<TouchableOpacity key={t} style={styles.tagChipActive} onPress={() => toggleTag(t)}>

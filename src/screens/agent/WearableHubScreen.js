...
export function WearableHubScreen({ navigation }) {
    const { t } = useI18n();
    ...
    const [connectionStage, setConnectionStage] = useState(null);
    const [profile, setProfile] = useState(null);
    const [capabilityPreview, setCapabilityPreview] = useState(null);
    const [serviceSnapshots, setServiceSnapshots] = useState([]);
    const [pairedRecords, setPairedRecords] = useState([]);
    const scanStopRef = useRef(null);
    const connectedDeviceIdRef = useRef(null);
    const mountedRef = useRef(true);
    const pulseAnim = useRef(new Animated.Value(1)).current;
    const filteredCandidates = useMemo(() => scanCandidates.filter((c) => scanFilter === 'all' || c.kind === scanFilter), [scanCandidates, scanFilter]);
    // ── Lifecycle ──────────────────────────────────────────────────────────────
    useEffect(() => {
        ...
        return () => {
            ...
            if (cid)
                void WearableBleGatewayService.disconnectDevice(cid);
        };
    }, []);
    useEffect(() => {
        if (!isScanning)
    ...
    const requestPermissions = async () => {
        ...
        try {
            ...
            if (mountedRef.current)
                setScanError(e?.message || 'Permission error');
            return null;
        }
    };
    const stopScan = () => {
    ...
    const connectCandidate = async (candidate) => {
        stopScan();
    ...
    const removePairedRecord = async (deviceId) => {
        const records = await WearablePairingStoreService.remove(deviceId);
        if (mountedRef.current)
            setPairedRecords(records);
    };
    const openMonitor = (deviceId) => {
    ...
    return (<SafeAreaView style={st.safe} edges={['top']}>
      <ScrollView style={st.scroll} contentContainerStyle={st.content} showsVerticalScrollIndicator={false}>
        ...
        {/* ── Paired devices ── */}
        {pairedRecords.length > 0 && (<View style={st.section}>
            <Text style={st.sectionLabel}>{t({ en: 'MY DEVICES', zh: '我的设备' })}</Text>
            {pairedRecords.map((rec) => {
                const kc = KIND_CFG[rec.kind] || KIND_CFG.unknown;
                return (<TouchableOpacity key={rec.id} style={st.pairedCard} onPress={() => openMonitor(rec.id)} activeOpacity={0.7}>
                  ...
                  <View style={st.pairedInfo}>
                    ...
                    <View style={st.chipRow}>
                      {rec.serviceLabels.slice(0, 3).map((lbl) => (<View key={`${rec.id}-${lbl}`} style={st.chip}><Text style={st.chipText}>{lbl}</Text></View>))}
                    </View>
                  </View>
        ...
        {pairedRecords.length > 0 && (<View style={st.healthRow}>
            {[
            ...
            ].map((m) => (<View key={m.label} style={st.healthCard}>
        ...
        <View style={st.section}>
          ...
          {(scanError || connectionError) && (<View style={st.alertCard}>
              <Text style={st.alertEmoji}>⚠️</Text>
              <Text style={st.alertText}>{scanError || connectionError}</Text>
            </View>)}

          <TouchableOpacity style={[st.scanBtn, isScanning && st.scanBtnActive]} onPress={() => { void startScan(); }} disabled={isScanning} activeOpacity={0.8}>
            {isScanning ? (<Animated.View style={{ transform: [{ scale: pulseAnim }] }}>
                <Text style={st.scanBtnIcon}>⌁</Text>
              </Animated.View>) : (<Text style={st.scanBtnIcon}>📡</Text>)}
          ...
          {/* Filter chips */}
          {(isScanning || scanCandidates.length > 0) && (<View style={st.filterRow}>
              {['all', 'ring', 'band', 'clip', 'sensor'].map((f) => {
                const lbl = f === 'all'
                    ? t({ en: 'All', zh: '全部' })
                    : t({ en: KIND_CFG[f].en, zh: KIND_CFG[f].zh });
                return (<TouchableOpacity key={f} style={[st.filterChip, scanFilter === f && st.filterChipOn]} onPress={() => setScanFilter(f)}>
          ...
          {filteredCandidates.map((c) => {
            ...
            return (<View key={c.id} style={[st.scanCard, isSel && st.scanCardSel]}>
                <View style={st.scanTop}>
                  ...
                  <View style={st.sigBars}>
                    {[1, 2, 3, 4].map((i) => (<View key={i} style={[st.sigBar, { height: 4 + i * 3, backgroundColor: i <= sig.bars ? kc.color : colors.border }]}/>))}
                  </View>
                </View>
                {c.serviceLabels.length > 0 && (<View style={st.chipRow}>
                    {c.serviceLabels.slice(0, 4).map((lbl) => (<View key={`${c.id}-${lbl}`} style={st.chip}><Text style={st.chipText}>{lbl}</Text></View>))}
                  </View>)}
                {isSel && connectionStage && (<View style={st.stageRow}>
                    {['connecting', 'discovering', 'reading', 'done'].map((s, i) => {
                        const idx = ['connecting', 'discovering', 'reading', 'done'].indexOf(connectionStage);
                        const past = idx >= i;
                        return (<View key={s} style={st.stageItem}>
                ...
                {!connected && !connecting && (<TouchableOpacity style={st.connectBtn} onPress={() => { void connectCandidate(c); }} activeOpacity={0.8}>
                    <Text style={st.connectBtnText}>{t({ en: 'Connect & Pair', zh: '连接配对' })}</Text>
                  </TouchableOpacity>)}
                {connecting && (<View style={st.connectingBar}>
                    <Text style={st.connectingText}>{t({ en: 'Connecting...', zh: '正在连接...' })}</Text>
                  </View>)}
                {connected && profile && (<View style={st.connectedBar}>
          ...
          {/* Empty */}
          {!isScanning && scanCandidates.length === 0 && pairedRecords.length === 0 && (<View style={st.empty}>
        ...
        <View style={st.supportSection}>
          ...
          <View style={st.supportGrid}>
            {['ring', 'band', 'clip', 'sensor'].map((k) => (<View key={k} style={st.supportItem}>

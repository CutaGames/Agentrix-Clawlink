...
export function SkillInstallScreen() {
    ...
    const handleInstall = async () => {
        if (!activeInstance) {
            Alert.alert(t({ en: 'No Agent Connected', zh: '未绑定 agent' }), t({ en: 'Please deploy or connect an OpenClaw instance first, then install skills.', zh: '请先在「Agent」页面绑定或部署一个 OpenClaw 实例，然后再安装 Skill。' }), [{ text: t({ en: 'Connect', zh: '去绑定' }), onPress: () => navigation.navigate('AgentOnboarding') }, { text: t({ en: 'Cancel', zh: '取消' }), style: 'cancel' }]);
            return;
        }
        if (instanceStatus === 'error') {
            Alert.alert(t({ en: 'Agent Error', zh: 'Agent 异常' }), t({ en: 'Agent is in error state. We recommend restarting before installing.', zh: 'Agent 当前状态异常。建议先重启 Agent 再安装 Skill。' }), [
    ...
    const doInstall = async () => {
        if (!skillId) {
            ...
            navigation.navigate('Discover', { screen: 'Marketplace' });
            return;
        }
        setInstalling(true);
        try {
            ...
            const isActiveNow = result?.skillActive || result?.platformHosted;
            const pendingMsg = result?.pendingDeploy
                ? `\n\n${t({ en: 'Note: The skill will auto-deploy when your agent reconnects.', zh: '注意：技能将在 Agent 重新连接后自动部署。' })}`
                : '';
            setDone(true);
            Alert.alert(t({ en: '✅ Installed!', zh: '✅ 安装成功！' }), isActiveNow
                ? `${skillName || 'Skill'} ${t({ en: 'is now active on', zh: '现已在以下 Agent 上激活：' })} ${activeInstance.name}`
                : `${skillName || 'Skill'} ${t({ en: 'has been installed to', zh: '已安装到' })} ${activeInstance.name}${pendingMsg}`, [{ text: 'OK', onPress: () => navigation.goBack() }]);
        }
        catch (e) {
            Alert.alert(t({ en: 'Install Failed', zh: '安装失败' }), e?.message || t({ en: 'Please make sure your Agent is online and try again.', zh: '请确认 Agent 实例在线后重试。' }));
        }
        finally {
            setInstalling(false);
        }
    ...
    return (<View style={styles.container}>
      ...
      {activeInstance ? (<View style={styles.instanceInfo}>
        ...
        </View>) : (<Text style={styles.sub}>⚠️ {t({ en: 'No Agent connected', zh: '未绑定 Agent 实例' })}</Text>)}

      {instanceStatus === 'error' && !done && (<TouchableOpacity style={[styles.restartBtn, restarting && styles.btnLoading]} onPress={handleRestart} disabled={restarting}>
          {restarting
                ? <ActivityIndicator color={colors.primary}/>
                : <Text style={styles.restartText}>{t({ en: '🔄 Restart Agent', zh: '🔄 重启 Agent' })}</Text>}
        </TouchableOpacity>)}

      {!done && (<TouchableOpacity style={[styles.btn, installing && styles.btnLoading]} onPress={handleInstall} disabled={installing}>
          {installing

...
import { View, Text, StyleSheet, ActivityIndicator, TouchableOpacity, ScrollView, TextInput, Linking, Platform, } from 'react-native';
...
export function LocalConnectScreen() {
    ...
    const [status, setStatus] = useState(hasQrParams ? 'connecting' : 'guide');
    ...
    const connectInstance = async (url, id, tok) => {
        try {
            ...
            setErrorMsg(msg.includes('Cannot reach') || msg.includes('Network') || msg.includes('fetch')
    ...
    if (status === 'guide') {
        ...
        {INSTALL_STEPS.map((step, i) => (<View key={i} style={styles.stepCard}>
            ...
            {step.action && (<TouchableOpacity style={styles.stepBtn} onPress={() => Linking.openURL(step.action.url)}>
    ...
    if (status === 'manual') {
        ...
        <View style={styles.inputGroup}>
          ...
          <TextInput style={styles.input} placeholder="192.168.1.100" placeholderTextColor={colors.textMuted} value={manualHost} onChangeText={setManualHost} keyboardType="numbers-and-punctuation" autoCapitalize="none"/>
        </View>

        <View style={styles.inputGroup}>
          <Text style={styles.inputLabel}>Port</Text>
          <TextInput style={styles.input} placeholder="7474" placeholderTextColor={colors.textMuted} value={manualPort} onChangeText={setManualPort} keyboardType="number-pad"/>
        </View>

        <View style={styles.inputGroup}>
          <Text style={styles.inputLabel}>Token (from installer log or config.json)</Text>
          <TextInput style={styles.input} placeholder="agent token" placeholderTextColor={colors.textMuted} value={manualToken} onChangeText={setManualToken} autoCapitalize="none" secureTextEntry/>
        </View>

        <TouchableOpacity style={[styles.btnPrimary, !manualHost.trim() && styles.btnDisabled]} onPress={handleManualConnect} disabled={!manualHost.trim()}>
    ...
    return (<ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      {status === 'connecting' && (<View style={styles.spinnerBox}>
          <ActivityIndicator size="large" color={colors.accent}/>
          <Text style={styles.spinnerText}>Reaching {instanceUrl}…</Text>
        </View>)}

      {status === 'success' && (<View style={styles.card}>
      ...
      {status === 'error' && (<View style={styles.card}>
          ...
          {instanceUrl ? (<View style={styles.infoRow}>
              <Text style={styles.infoLabel}>Agent URL</Text>
              <Text style={styles.mono}>{instanceUrl}</Text>
            </View>) : null}

          <View style={styles.btnRow}>
            {hasQrParams && (<TouchableOpacity style={styles.btnPrimary} onPress={() => {
                    setErrorMsg('');

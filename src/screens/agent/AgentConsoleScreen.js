...
export function AgentConsoleScreen() {
    ...
    const { data: marketInstalledRaw } = useQuery({
        ...
        queryFn: () => apiFetch('/skills/installed'),
        enabled: !!activeInstance,
        staleTime: 60000,
    });
    ...
    const skills = (() => {
        const list = [];
        const seen = new Set();
        for (const s of (instanceSkillsRaw || [])) {
            if (!seen.has(s.id)) {
                seen.add(s.id);
                list.push(s);
            }
        ...
        for (const s of mkt) {
            ...
            if (!seen.has(key)) {
                seen.add(key);
                list.push({ ...s, id: key, name: s.skill?.name || s.name, description: s.skill?.description || s.description, icon: s.skill?.icon || s.icon, enabled: true, _source: 'marketplace' });
            }
        }
        return list;
    })();
    ...
    return (<ScrollView testID="agent-console-screen" accessibilityLabel="agent-console-screen" style={styles.container} contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={isLoading} onRefresh={refetch} tintColor={colors.accent}/>}>
      ...
      <View style={styles.statusCard}>
        ...
        <View style={styles.statusActions}>
          ...
          <TouchableOpacity style={styles.iconBtn} onPress={() => navigation.navigate('DeploySelect')}>
      ...
      <View style={styles.modelSwitcher}>
        ...
        <TouchableOpacity style={styles.modelSelector} onPress={() => setShowEngineModal(true)}>
      ...
      {instances.length > 1 && (<ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.instanceScroll}>
          {instances.map((inst) => (<TouchableOpacity key={inst.id} style={[styles.instanceChip, activeInstance.id === inst.id && styles.instanceChipActive]} onPress={() => setActiveInstance(inst.id)}>
      ...
      <View style={styles.tabs}>
        {['overview', 'skills', 'tasks'].map((tabKey) => (<TouchableOpacity key={tabKey} style={[styles.tabBtn, tab === tabKey && styles.tabBtnActive]} onPress={() => setTab(tabKey)}>
        ...
        {(uiComplexity === 'advanced' || uiComplexity === 'professional') && (<TouchableOpacity style={[styles.tabBtn, tab === 'permissions' && styles.tabBtnActive]} onPress={() => setTab('permissions')}>
      ...
      {/* Big Chat CTA */}
      {tab === 'overview' && (<>
          <TouchableOpacity testID="agent-console-chat-cta" accessibilityLabel="agent-console-chat-cta" style={styles.chatCta} onPress={() => navigation.navigate('AgentChat', { instanceId: activeInstance.id, instanceName: activeInstance.name })}>
          ...
          {/* Error instance hint */}
          {activeInstance.status === 'error' && (<View style={{ backgroundColor: '#ef444418', borderRadius: 12, padding: 12, marginTop: -8 }}>
          ...
          {/* Storage Card */}
          <TouchableOpacity style={styles.storageCard} onPress={() => navigation.navigate('StoragePlan')} activeOpacity={0.8}>
            <View style={styles.storageCardHeader}>
              ...
              {storageInfo?.isGiftStorage && (<View style={styles.giftBadge}>
            ...
            <View style={styles.storageBar}>
              <View style={[styles.storageBarFill, {
                    width: `${Math.min(storageInfo?.usedPercent ?? 2, 100)}%`,
          ...
          {/* ── Token Usage Card ── */}
          {quota && (<View style={styles.quotaCard}>
              ...
              {quota.quotaExhausted && (<View style={styles.quotaExhausted}>
          ...
          <View style={styles.quickActions}>
            {[
                { icon: '📷', label: t({ en: 'Scan QR', zh: '扫一扫' }), route: 'Scan' },
                { icon: '📋', label: t({ en: 'Activity Logs', zh: '活动日志' }), route: 'AgentLogs' },
                { icon: '💻', label: t({ en: 'Desktop Control', zh: '桌面控制' }), route: 'DesktopControl' },
                { icon: '⌚', label: t({ en: 'Wearables', zh: '可穿戴设备' }), route: 'WearableHub' },
                { icon: '🧠', label: t({ en: 'Memory Hub', zh: '记忆中心' }), route: 'MemoryManagement' },
                { icon: '⚙️', label: t({ en: 'Workflows', zh: '工作流' }), route: 'WorkflowList' },
                { icon: '🛠️', label: t({ en: 'Agent Tools', zh: '系统工具' }), route: 'AgentTools' },
                { icon: '👥', label: t({ en: 'Team Space', zh: '团队空间' }), route: 'TeamSpace' },
                { icon: '🤖', label: t({ en: 'Agent Accounts', zh: '智能体账户' }), route: 'AgentAccount' },
            ].map((item) => (<TouchableOpacity key={item.route} style={styles.quickAction} onPress={() => navigation.navigate(item.route, 'params' in item
                    ? item.params
                    : undefined)} activeOpacity={0.7}>
        ...
        <TouchableOpacity style={styles.downloadBanner} onPress={() => {
                Alert.alert(t({ en: '💻 Agentrix Desktop', zh: '💻 Agentrix 桌面版' }), t({ en: 'Download the Agentrix Desktop app to control your computer with AI, run local agents, and sync with your mobile.', zh: '下载 Agentrix 桌面版，用 AI 控制你的电脑、运行本地智能体，并与手机同步。' }), [
      ...
      {/* Skills Tab */}
      {tab === 'skills' && (<View style={styles.skillsSection}>
          {(!skills || skills.length === 0) ? (<View style={styles.emptySection}>
              <Text style={styles.emptySectionText}>{t({ en: 'No skills installed.', zh: '还没有安装技能。' })}</Text>
              <TouchableOpacity style={styles.secondaryBtn} onPress={() => navigation.navigate('SkillInstall', { skillId: '', skillName: '' })}>
                <Text style={styles.secondaryBtnText}>{t({ en: 'Browse Skill Market →', zh: '浏览技能市场 →' })}</Text>
              </TouchableOpacity>
            </View>) : (skills.map((skill) => (<View key={skill.id} style={styles.skillRow}>
      ...
      {tab === 'tasks' && (<View style={styles.emptySection}>
      ...
      {/* Permissions Tab */}
      {tab === 'permissions' && (<View style={{ gap: 10 }}>
          <TouchableOpacity style={styles.permissionsCard} onPress={() => navigation.navigate('AgentPermissions', { agentAccountId: activeInstance.metadata?.agentAccountId })} activeOpacity={0.8}>
          ...
          {[
            ...
            ].map((row) => (<View key={row.label} style={styles.permSummaryRow}>
          ...
          <TouchableOpacity style={[styles.primaryBtn, { marginTop: 4 }]} onPress={() => navigation.navigate('AgentPermissions', { agentAccountId: activeInstance.metadata?.agentAccountId })}>
            <Text style={styles.primaryBtnText}>🔐 {t({ en: 'Open Full Permissions Manager →', zh: '打开完整权限管理器 →' })}</Text>
          </TouchableOpacity>
        </View>)}

      <SelectEngineModal visible={showEngineModal} onClose={() => setShowEngineModal(false)} selectedModelId={selectedModelId} onSelect={async (id) => {
            setSelectedModel(id);
            if (activeInstance?.id) {
                try {
                    await switchInstanceModel(activeInstance.id, id);
                }
                catch (_) { }
            }
        }}/>

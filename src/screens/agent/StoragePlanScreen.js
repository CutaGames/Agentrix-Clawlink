...
import { getStorageInfo, upgradeStoragePlan } from '../../services/openclaw.service';
const TIER_COLORS = {
    free: '#22c55e',
    starter: '#7c3aed',
    pro: '#0ea5e9',
...
export function StoragePlanScreen() {
    ...
    return (<ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.usageCard}>
        ...
        {isLoading ? (<ActivityIndicator color={colors.accent} style={{ marginVertical: 12 }}/>) : (<>
      ...
      {STATIC_PLANS.map((plan) => {
            ...
            return (<View key={plan.tier} style={[
                    styles.card,
                    plan.highlight && styles.cardHighlight,
                    isCurrentTier && { borderColor: accentColor + '55' },
                ]}>
            {plan.highlight && (<View style={styles.popularBadge}>
            ...
            <Text style={styles.price}>
              ...
              {plan.priceUsdPerMonth > 0 && (<Text style={styles.priceSub}> / month</Text>)}
            </Text>
            <View style={styles.perks}>
              {plan.perks.map((perk, i) => (<Text key={i} style={styles.perk}>✓  {perk}</Text>))}
            </View>
            {isCurrentTier ? (<View style={[styles.ctaDisabled, { borderColor: accentColor + '33' }]}>
                <Text style={[styles.ctaDisabledText, { color: accentColor }]}>✓ Current Plan</Text>
              </View>) : (<TouchableOpacity style={[styles.cta, plan.highlight ? styles.ctaPrimary : styles.ctaSecondary]} onPress={() => handleUpgrade(plan.tier)} disabled={isUpgrading}>
                {isUpgrading ? (<ActivityIndicator color="#fff" size="small"/>) : (<Text style={[styles.ctaText, !plan.highlight && { color: colors.textPrimary }]}>
                    {plan.ctaLabel}
                  </Text>)}
              </TouchableOpacity>)}
          </View>);

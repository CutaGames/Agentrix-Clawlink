...
import { View, Text, ScrollView, TouchableOpacity, StyleSheet, ActivityIndicator, TextInput, Alert, Platform, RefreshControl, } from 'react-native';
...
// ─── API helpers ──────────────────────────────────────────────────────────────
const fetchWorkspace = (id) => apiFetch(`/workspaces/${id}`);
const inviteMember = (workspaceId, body) => apiFetch(`/workspaces/${workspaceId}/members`, {
    method: 'POST',
    body: JSON.stringify(body),
});
const removeMember = (workspaceId, userId) => apiFetch(`/workspaces/${workspaceId}/members/${userId}`, { method: 'DELETE' });
// ─── Component ────────────────────────────────────────────────────────────────
export function TeamInviteScreen() {
    ...
    const handleRemove = (member) => {
        ...
        Alert.alert('Remove Member', `Remove ${member.user?.name || member.userId} from this workspace?`, [
    ...
    return (<View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={isRefetching} onRefresh={refetch} tintColor={colors.accent}/>}>
        {/* Invite form */}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Invite by Email</Text>
          <TextInput style={styles.input} value={email} onChangeText={setEmail} placeholder="colleague@example.com" placeholderTextColor={colors.textMuted} keyboardType="email-address" autoCapitalize="none" autoCorrect={false}/>

          <Text style={[styles.sectionTitle, { marginTop: 8 }]}>Role</Text>
          <View style={styles.roleRow}>
            {['admin', 'editor', 'viewer'].map((r) => (<TouchableOpacity key={r} style={[styles.roleChip, role === r && styles.roleChipActive]} onPress={() => setRole(r)}>
          ...
          <TouchableOpacity style={[styles.inviteBtn, (!email.trim() || inviteMutation.isPending) && styles.inviteBtnDisabled]} onPress={handleInvite} disabled={!email.trim() || inviteMutation.isPending}>
            {inviteMutation.isPending ? (<ActivityIndicator size="small" color="#fff"/>) : (<Text style={styles.inviteBtnText}>Send Invitation</Text>)}
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.section}>
          ...
          {isLoading ? (<ActivityIndicator size="small" color={colors.accent}/>) : members.length === 0 ? (<Text style={styles.emptyText}>No members yet.</Text>) : (members.map((m) => (<View key={m.id} style={styles.memberRow}>
                ...
                {m.role !== 'owner' && (<TouchableOpacity onPress={() => handleRemove(m)} style={styles.removeBtn} disabled={removeMutation.isPending}>

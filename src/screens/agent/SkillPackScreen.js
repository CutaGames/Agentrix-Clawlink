...
export function SkillPackScreen() {
    ...
    const [selected, setSelected] = useState(() => new Set(CORE_SKILLS.map(s => s.id)));
    ...
    const { data: skills } = useQuery({
        ...
        queryFn: async () => {
            try {
                const res = await apiFetch('/unified-marketplace/trending?limit=8');
    ...
    return (<SafeAreaView style={styles.container}>
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent}>
        ...
        {displaySkills.map(skill => {
            ...
            return (<TouchableOpacity key={skill.id} style={[styles.skillCard, isOn && styles.skillCardSelected]} onPress={() => toggleSkill(skill.id)} activeOpacity={0.7}>

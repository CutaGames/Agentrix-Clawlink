...
// ─── API helpers ──────────────────────────────────────────────
const fetchLogs = () => apiFetch('/notifications?limit=100');
const markRead = (id) => apiFetch(`/notifications/${id}/read`, { method: 'PUT' });
const markAllRead = () => apiFetch('/notifications/mark-all-read', { method: 'PUT' });
// ─── Helpers ──────────────────────────────────────────────────
const TYPE_ICON = {
    task_assigned: '📋',
    task_completed: '✅',
...
export function AgentLogsScreen() {
    ...
    const renderItem = ({ item }) => (<TouchableOpacity style={[styles.row, !item.read && styles.rowUnread]} onPress={() => !item.read && markReadMut.mutate(item.id)} activeOpacity={0.7}>
      ...
      <View style={styles.content}>
        ...
        {item.type && (<View style={styles.typeBadge}>
    ...
    return (<View style={styles.container}>
      ...
      <View style={styles.header}>
        <View style={styles.filterRow}>
          {['all', 'unread'].map((f) => (<TouchableOpacity key={f} style={[styles.filterBtn, filter === f && styles.filterBtnActive]} onPress={() => setFilter(f)}>
              <Text style={[styles.filterText, filter === f && styles.filterTextActive]}>
                {f === 'all' ? 'All' : `Unread${unreadCount > 0 ? ` (${unreadCount})` : ''}`}
              </Text>
            </TouchableOpacity>))}
        </View>
        {unreadCount > 0 && (<TouchableOpacity style={styles.markAllBtn} onPress={() => markAllMut.mutate()} disabled={markAllMut.isPending}>
      ...
      {isLoading ? (<View style={styles.loadingWrap}>
        ...
        </View>) : displayed.length === 0 ? (<View style={styles.emptyWrap}>
        ...
        </View>) : (<FlatList data={displayed} keyExtractor={(item) => item.id} renderItem={renderItem} refreshControl={<RefreshControl refreshing={isRefetching} onRefresh={refetch} tintColor={colors.accent}/>} contentContainerStyle={{ paddingBottom: 32 }} ItemSeparatorComponent={() => <View style={styles.separator}/>}/>)}
    </View>);
}

...
import { createRemoteDesktopCommand, fetchDesktopState, respondToDesktopApproval, } from '../../services/desktopSync';
...
export function DesktopControlScreen() {
    ...
    const loadState = useCallback(async (isRefresh = false) => {
        try {
            ...
            Alert.alert(t({ en: 'Desktop Sync', zh: '桌面同步' }), error?.message || t({ en: 'Failed to load desktop state.', zh: '加载桌面状态失败。' }));
        }
        finally {
            setLoading(false);
            setRefreshing(false);
    ...
    const selectedDevice = useMemo(() => devices.find((device) => device.deviceId === selectedDeviceId) || devices[0], [devices, selectedDeviceId]);
    const submitCommand = useCallback(async (kind, title, payload) => {
        try {
            setSubmitting(true);
            ...
            Alert.alert(t({ en: 'Queued', zh: '已下发' }), t({ en: 'Desktop command was queued successfully.', zh: '桌面命令已成功下发。' }));
        }
        catch (error) {
            Alert.alert(t({ en: 'Command Failed', zh: '命令失败' }), error?.message || t({ en: 'Could not queue desktop command.', zh: '无法下发桌面命令。' }));
        }
        finally {
            setSubmitting(false);
        }
    ...
    const handleApproval = useCallback(async (approvalId, decision) => {
        try {
            ...
            Alert.alert(t({ en: 'Approval Failed', zh: '审批失败' }), error?.message || t({ en: 'Could not respond to approval.', zh: '无法处理审批。' }));
        }
    }, [loadState, t]);
    const latestCommands = commands.slice(0, 8);
    if (loading && !state) {
    ...
    return (<ScrollView style={styles.container} contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => void loadState(true)}/>}>
      ...
      <View style={styles.section}>
        ...
        {devices.length === 0 ? (<Text style={styles.emptyText}>{t({ en: 'No desktop devices online.', zh: '当前没有在线桌面设备。' })}</Text>) : (devices.map((device) => (<TouchableOpacity key={device.deviceId} style={[styles.deviceCard, selectedDevice?.deviceId === device.deviceId && styles.deviceCardActive]} onPress={() => setSelectedDeviceId(device.deviceId)} activeOpacity={0.85}>
              ...
              {!!device.context?.workspaceHint && (<Text style={styles.deviceContext}>{device.context.workspaceHint}</Text>)}
              {!!device.context?.activeWindowTitle && (<Text style={styles.deviceContext}>{device.context.activeWindowTitle}</Text>)}
            </TouchableOpacity>)))}
      </View>
      ...
      <View style={styles.section}>
        ...
        <TextInput value={shellCommand} onChangeText={setShellCommand} placeholder={t({ en: 'npm test', zh: '例如 npm test' })} placeholderTextColor={colors.textMuted} style={styles.input}/>
        <TouchableOpacity style={styles.primaryButton} disabled={submitting || !selectedDevice || !shellCommand.trim()} onPress={() => void submitCommand('run-command', shellCommand.trim(), { command: shellCommand.trim(), timeoutMs: 60000 })}>
      ...
      <View style={styles.section}>
        ...
        <TouchableOpacity style={styles.primaryButton} disabled={submitting || !selectedDevice || !browserUrl.trim()} onPress={() => void submitCommand('open-browser', `Open ${browserUrl.trim()}`, { url: browserUrl.trim() })}>
      ...
      <View style={styles.section}>
        ...
        <TextInput value={fileContent} onChangeText={setFileContent} placeholder={t({ en: 'Optional file content for write-file', zh: '写文件时填写内容' })} placeholderTextColor={colors.textMuted} style={[styles.input, styles.multilineInput]} multiline/>
        <View style={styles.rowWrap}>
          <TouchableOpacity style={styles.secondaryButton} disabled={submitting || !selectedDevice || !filePath.trim()} onPress={() => void submitCommand('read-file', `Read ${filePath.trim()}`, { path: filePath.trim() })}>
            <Text style={styles.secondaryButtonText}>{t({ en: 'Read File', zh: '读取文件' })}</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.secondaryButton} disabled={submitting || !selectedDevice || !filePath.trim()} onPress={() => void submitCommand('write-file', `Write ${filePath.trim()}`, { path: filePath.trim(), content: fileContent })}>
      ...
      <View style={styles.section}>
        ...
        {approvals.filter((item) => item.status === 'pending').length === 0 ? (<Text style={styles.emptyText}>{t({ en: 'No pending approvals.', zh: '暂无待处理审批。' })}</Text>) : (approvals.filter((item) => item.status === 'pending').map((approval) => (<View key={approval.approvalId} style={styles.card}>
      ...
      <View style={styles.section}>
        ...
        {latestCommands.length === 0 ? (<Text style={styles.emptyText}>{t({ en: 'No commands yet.', zh: '暂无命令记录。' })}</Text>) : (latestCommands.map((command) => (<View key={command.commandId} style={styles.card}>
      ...
      <View style={styles.section}>
        ...
        {sessions.length === 0 ? (<Text style={styles.emptyText}>{t({ en: 'No synced sessions yet.', zh: '暂无同步会话。' })}</Text>) : (sessions.slice(0, 6).map((session) => (<View key={session.sessionId} style={styles.card}>

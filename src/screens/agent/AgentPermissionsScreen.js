...
import { View, Text, TouchableOpacity, StyleSheet, ScrollView, Switch, Alert, TextInput, Modal, FlatList, } from 'react-native';
...
import { fetchAgentPresenceAccounts, getAgentPresenceAccount, setAgentPresenceAccountStatus, updateAgentPresenceAccount, } from '../../services/agentPresenceAccount';
...
function normalizePermissions(value) {
    return {
        ...
        allowedCurrencies: Array.isArray(value?.allowedCurrencies) && value?.allowedCurrencies.length
            ? value.allowedCurrencies.filter((currency) => typeof currency === 'string')
            : DEFAULT_PERMISSIONS.allowedCurrencies,
        gpsAccuracy: value?.gpsAccuracy === 'district' || value?.gpsAccuracy === 'exact'
            ? value.gpsAccuracy
        ...
        confirmationThreshold: typeof value?.confirmationThreshold === 'number'
...
function PermRow({ label, sub, value, onChange, destructive, }) {
    return (<View style={styles.permRow}>
      <View style={{ flex: 1 }}>
        <Text style={[styles.permLabel, destructive && { color: colors.error }]}>{label}</Text>
        {sub ? <Text style={styles.permSub}>{sub}</Text> : null}
      </View>
      <Switch value={value} onValueChange={onChange} trackColor={{ false: colors.bgSecondary, true: colors.accent + '88' }} thumbColor={value ? colors.accent : colors.textMuted}/>
    </View>);
}
// ── Main Screen ─────────────────────────────────────────────────────
export function AgentPermissionsScreen() {
    ...
    const [selectedAgentId, setSelectedAgentId] = useState(agentAccountId);
    useEffect(() => {
        if (agentAccountId) {
    ...
    const [preferredProvider, setPreferredProvider] = useState(undefined);
    const [preferredModel, setPreferredModel] = useState(undefined);
    const [agentVoice, setAgentVoice] = useState(undefined);
    const [showModelPicker, setShowModelPicker] = useState(false);
    const [showVoicePicker, setShowVoicePicker] = useState(false);
    const availableVoices = ['alloy', 'echo', 'fable', 'onyx', 'nova', 'shimmer'];
    useEffect(() => {
        Promise.all([
            apiFetch('/ai-providers/catalog'),
            apiFetch('/ai-providers/configs'),
        ]).then(([cat, configs]) => {
            setCatalog(cat);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            Alert.alert(t({ en: 'Saved ✅', zh: '已保存 ✅' }), activeInstance?.id
    ...
    const handleSuspend = () => {
        if (!activeAgent)
            return;
        Alert.alert(t({ en: 'Suspend Agent', zh: '暂停智能体' }), t({ en: `Suspend "${activeAgent.name}"? It will be unable to make payments or use tools.`, zh: `确认暂停“${activeAgent.name}”吗？暂停后它将无法付款或使用工具。` }), [
    ...
    return (<ScrollView style={styles.container} contentContainerStyle={styles.content}>
      {/* Agent Selector */}
      {activeAgent ? (<View style={styles.agentBadge}>
        ...
        </View>) : (<View style={styles.noAgentBox}>
      ...
      {activeInstance?.id ? (<View style={styles.instanceBindingCard}>
      ...
      {agents.length > 1 && (<View style={styles.accountPickerWrap}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.accountPickerRow}>
            {agents.map((agent) => {
                ...
                return (<TouchableOpacity key={agent.id} style={[styles.accountChip, selected && styles.accountChipActive]} onPress={() => setSelectedAgentId(agent.id)}>
                  <Text style={[styles.accountChipText, selected && styles.accountChipTextActive]}>{agent.name}</Text>
                </TouchableOpacity>);
            })}
          </ScrollView>
          {!agentAccountId && (<Text style={styles.accountPickerHint}>{t({ en: 'Permissions are saved to the selected Agent Account and verified after saving.', zh: '权限会保存到当前选中的智能体账户，并在保存后回读校验。' })}</Text>)}
        </View>)}
      ...
      <View style={styles.section}>
        ...
        <TouchableOpacity style={[styles.permRow, { paddingVertical: 14 }]} onPress={() => setShowModelPicker(true)}>
          <Text style={{ fontSize: 14, color: colors.textPrimary, flex: 1 }}>{modelDisplayLabel}</Text>
          <Text style={{ fontSize: 12, color: colors.textMuted }}>▸</Text>
        </TouchableOpacity>
        {preferredModel && (<>
            <View style={styles.divider}/>
            <TouchableOpacity style={[styles.permRow, { paddingVertical: 10 }]} onPress={() => { setPreferredProvider(undefined); setPreferredModel(undefined); }}>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.divider}/>
        <TouchableOpacity style={[styles.permRow, { paddingVertical: 14 }]} onPress={() => setShowVoicePicker(true)}>
        ...
        {agentVoice && (<>
            <View style={styles.divider}/>
            <TouchableOpacity style={[styles.permRow, { paddingVertical: 10 }]} onPress={() => setAgentVoice(undefined)}>
              <Text style={{ fontSize: 13, color: colors.error }}>
      ...
      <Modal visible={showVoicePicker} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={styles.modalSheet}>
            ...
            <FlatList data={availableVoices} keyExtractor={v => v} renderItem={({ item: v }) => {
            const isSelected = agentVoice === v;
            return (<TouchableOpacity style={[styles.modelPickRow, isSelected && { backgroundColor: colors.primary + '10' }]} onPress={() => {
                    setAgentVoice(v);
      ...
      <Modal visible={showModelPicker} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={styles.modalSheet}>
            ...
            <FlatList data={configuredProviders.length > 0 ? configuredProviders : catalog} keyExtractor={p => p.id} ListHeaderComponent={configuredProviders.length > 0 ? (<Text style={{ fontSize: 11, color: colors.textMuted, paddingHorizontal: 16, paddingTop: 8 }}>
                    {t({ en: 'Showing your configured providers. Configure more in AI Settings.', zh: '显示已配置的厂商。可在 AI 设置中配置更多。' })}
                  </Text>) : (<Text style={{ fontSize: 11, color: colors.textMuted, paddingHorizontal: 16, paddingTop: 8 }}>
                    {t({ en: 'No providers configured yet. Configure API keys in Settings first.', zh: '尚未配置厂商。请先在设置中配置 API 密钥。' })}
                  </Text>)} renderItem={({ item: prov }) => (<View>
                ...
                return (<TouchableOpacity key={m.id} style={[styles.modelPickRow, isSelected && { backgroundColor: colors.primary + '10' }]} onPress={() => {
                        ...
                        <View style={{ flex: 1 }}>
                          ...
                          {(m.inputPrice || m.outputPrice) ? (<Text style={styles.modelPickCaps}>📥 {m.inputPrice}  📤 {m.outputPrice}  ·  {fmtCtx} ctx{m.multimodal ? '  ·  🖼' : ''}</Text>) : (<Text style={styles.modelPickCaps}>{fmtCtx} ctx{m.multimodal ? '  ·  🖼 多模态' : ''}{m.freeApi ? `  ·  🆓 ${m.freeNote || 'Free'}` : ''}</Text>)}
      ...
      <View style={styles.section}>
        <PermRow label={t({ en: 'Allow Autonomous Payment', zh: '允许自主支付' })} sub={t({ en: 'Agent can initiate transactions without asking you', zh: '智能体可在无需询问你的情况下发起交易' })} value={perms.autonomousPaymentEnabled} onChange={(v) => updatePerm('autonomousPaymentEnabled', v)}/>
        <View style={styles.divider}/>
        ...
        <View style={styles.permRow}>
          ...
          {editingThreshold ? (<View style={styles.thresholdEdit}>
              <Text style={styles.currency}>$</Text>
              <TextInput style={styles.thresholdInput} value={thresholdInput} onChangeText={setThresholdInput} keyboardType="decimal-pad" autoFocus onBlur={() => {
                updatePerm('confirmationThreshold', Number(thresholdInput) || 50);
                setEditingThreshold(false);
            }}/>
            </View>) : (<TouchableOpacity onPress={() => setEditingThreshold(true)} style={styles.editableValue}>
        ...
        <View style={styles.permRow}>
          ...
          <View style={styles.currencyRow}>
            {CURRENCY_OPTIONS.map((c) => (<TouchableOpacity key={c} style={[styles.currencyChip, perms.allowedCurrencies.includes(c) && styles.currencyChipActive]} onPress={() => toggleCurrency(c)}>
      ...
      <View style={styles.section}>
        <PermRow label={t({ en: '📁 File Read', zh: '📁 文件读取' })} sub={perms.fileReadEnabled ? `Scope: ${perms.fileReadScope}` : undefined} value={perms.fileReadEnabled} onChange={(v) => updatePerm('fileReadEnabled', v)}/>
        <View style={styles.divider}/>
        <PermRow label={t({ en: '🖥 Launch Programs', zh: '🖥 启动程序' })} value={perms.programLaunchEnabled} onChange={(v) => updatePerm('programLaunchEnabled', v)}/>
        <View style={styles.divider}/>
        <PermRow label={t({ en: '📸 Screenshot', zh: '📸 截图' })} value={perms.screenshotEnabled} onChange={(v) => updatePerm('screenshotEnabled', v)}/>
        <View style={styles.divider}/>
        <PermRow label={t({ en: '📍 GPS Location', zh: '📍 GPS 定位' })} sub={perms.gpsEnabled ? `Precision: ${perms.gpsAccuracy}` : undefined} value={perms.gpsEnabled} onChange={(v) => updatePerm('gpsEnabled', v)}/>
      </View>

      {/* ── Network & Tools ── */}
      <SectionHeader icon="🌐" title={t({ en: 'Network & Tool Permissions', zh: '网络与工具权限' })}/>
      <View style={styles.section}>
        <PermRow label={t({ en: '🔍 Web Search', zh: '🔍 网络搜索' })} value={perms.webSearchEnabled} onChange={(v) => updatePerm('webSearchEnabled', v)}/>
        <View style={styles.divider}/>
        <PermRow label={t({ en: '📧 Email Send', zh: '📧 邮件发送' })} sub={!perms.emailEnabled ? t({ en: 'Requires approval to enable', zh: '启用前需要审批' }) : undefined} value={perms.emailEnabled} onChange={(v) => updatePerm('emailEnabled', v)}/>
        <View style={styles.divider}/>
        <PermRow label={t({ en: '🐦 Twitter / X Post', zh: '🐦 Twitter / X 发帖' })} sub={perms.twitterEnabled ? t({ en: 'Auto mode', zh: '自动模式' }) : undefined} value={perms.twitterEnabled} onChange={(v) => updatePerm('twitterEnabled', v)}/>
        <View style={styles.divider}/>
        <PermRow label={t({ en: '💬 Telegram Reply', zh: '💬 Telegram 回复' })} sub={perms.telegramEnabled ? t({ en: 'Auto mode', zh: '自动模式' }) : undefined} value={perms.telegramEnabled} onChange={(v) => updatePerm('telegramEnabled', v)}/>
        <View style={styles.divider}/>
        <View style={styles.permRow}>
          <View style={{ flex: 1 }}>
      ...
      <View style={styles.section}>
        <PermRow label={t({ en: '🔎 Search Skills', zh: '🔎 搜索技能' })} sub={t({ en: 'Agent can search for AI skills and plugins', zh: '智能体可搜索 AI 技能和插件' })} value={perms.skillSearchEnabled} onChange={(v) => updatePerm('skillSearchEnabled', v)}/>
        <View style={styles.divider}/>
        <PermRow label={t({ en: '📥 Install Skills', zh: '📥 安装技能' })} sub={t({ en: 'Agent can install skills from marketplace', zh: '智能体可从市场安装技能' })} value={perms.skillInstallEnabled} onChange={(v) => updatePerm('skillInstallEnabled', v)}/>
        <View style={styles.divider}/>
        <PermRow label={t({ en: '▶️ Execute Skills', zh: '▶️ 执行技能' })} sub={t({ en: 'Agent can run installed skills', zh: '智能体可运行已安装的技能' })} value={perms.skillExecuteEnabled} onChange={(v) => updatePerm('skillExecuteEnabled', v)}/>
        <View style={styles.divider}/>
        <PermRow label={t({ en: '📤 Publish Skills', zh: '📤 发布技能' })} sub={t({ en: 'Agent can publish new skills to marketplace', zh: '智能体可向市场发布新技能' })} value={perms.skillPublishEnabled} onChange={(v) => updatePerm('skillPublishEnabled', v)}/>
      </View>

      {/* ── Commerce & Wallet ── */}
      <SectionHeader icon="🛒" title={t({ en: 'Commerce & Wallet', zh: '商城与钱包' })}/>
      <View style={styles.section}>
        <PermRow label={t({ en: '🏪 Browse Marketplace', zh: '🏪 浏览商城' })} sub={t({ en: 'Agent can search products, resources, and services', zh: '智能体可搜索商品、资源和服务' })} value={perms.commerceBrowseEnabled} onChange={(v) => updatePerm('commerceBrowseEnabled', v)}/>
        <View style={styles.divider}/>
        <PermRow label={t({ en: '🛍 Purchase Items', zh: '🛍 购买商品' })} sub={t({ en: 'Agent can buy and checkout items', zh: '智能体可购买并结算商品' })} value={perms.commercePurchaseEnabled} onChange={(v) => updatePerm('commercePurchaseEnabled', v)}/>
        <View style={styles.divider}/>
        <PermRow label={t({ en: '📦 Manage Orders', zh: '📦 管理订单' })} sub={t({ en: 'Agent can check order status and details', zh: '智能体可查看订单状态和详情' })} value={perms.orderManageEnabled} onChange={(v) => updatePerm('orderManageEnabled', v)}/>
        <View style={styles.divider}/>
        <PermRow label={t({ en: '💰 View Wallet Balance', zh: '💰 查看钱包余额' })} sub={t({ en: 'Agent can read wallet balance and asset overview', zh: '智能体可读取钱包余额和资产概览' })} value={perms.walletReadEnabled} onChange={(v) => updatePerm('walletReadEnabled', v)}/>
        <View style={styles.divider}/>
        <PermRow label={t({ en: '⚡ Quick Pay / Transfer', zh: '⚡ 快速支付/转账' })} sub={t({ en: 'Agent can send payments and transfers', zh: '智能体可发送付款和转账' })} value={perms.quickpayEnabled} onChange={(v) => updatePerm('quickpayEnabled', v)} destructive/>
        <View style={styles.divider}/>
        <PermRow label={t({ en: '🔐 x402 Paywall Access', zh: '🔐 x402 付费墙访问' })} sub={t({ en: 'Agent can pay for paywalled content/APIs', zh: '智能体可支付付费内容/API 访问' })} value={perms.x402PayEnabled} onChange={(v) => updatePerm('x402PayEnabled', v)} destructive/>
      </View>

      {/* ── Agent-to-Agent (A2A) ── */}
      <SectionHeader icon="🤖" title={t({ en: 'Agent-to-Agent (A2A)', zh: '智能体协作 (A2A)' })}/>
      <View style={styles.section}>
        <PermRow label={t({ en: '🔍 Discover Agents', zh: '🔍 发现智能体' })} sub={t({ en: 'Agent can find other agents on the network', zh: '智能体可在网络上发现其他智能体' })} value={perms.a2aDiscoverEnabled} onChange={(v) => updatePerm('a2aDiscoverEnabled', v)}/>
        <View style={styles.divider}/>
        <PermRow label={t({ en: '🤝 Invoke Agents', zh: '🤝 调用智能体' })} sub={t({ en: 'Agent can delegate tasks to other agents', zh: '智能体可将任务委托给其他智能体' })} value={perms.a2aInvokeEnabled} onChange={(v) => updatePerm('a2aInvokeEnabled', v)}/>
      </View>

      {/* ── Task Marketplace ── */}
      <SectionHeader icon="📋" title={t({ en: 'Task Marketplace', zh: '任务市场' })}/>
      <View style={styles.section}>
        <PermRow label={t({ en: '🔎 Search Tasks', zh: '🔎 搜索任务' })} sub={t({ en: 'Agent can browse tasks and bounties', zh: '智能体可浏览任务和悬赏' })} value={perms.taskSearchEnabled} onChange={(v) => updatePerm('taskSearchEnabled', v)}/>
        <View style={styles.divider}/>
        <PermRow label={t({ en: '📝 Post Tasks', zh: '📝 发布任务' })} sub={t({ en: 'Agent can create new tasks and bounties', zh: '智能体可创建新任务和悬赏' })} value={perms.taskPostEnabled} onChange={(v) => updatePerm('taskPostEnabled', v)}/>
        <View style={styles.divider}/>
        <PermRow label={t({ en: '✅ Accept Tasks', zh: '✅ 接受任务' })} sub={t({ en: 'Agent can accept tasks from marketplace', zh: '智能体可从市场接受任务' })} value={perms.taskAcceptEnabled} onChange={(v) => updatePerm('taskAcceptEnabled', v)}/>
        <View style={styles.divider}/>
        <PermRow label={t({ en: '📤 Submit Deliverables', zh: '📤 提交交付物' })} sub={t({ en: 'Agent can submit completed work', zh: '智能体可提交完成的工作' })} value={perms.taskSubmitEnabled} onChange={(v) => updatePerm('taskSubmitEnabled', v)}/>
      </View>

      {/* ── Resources ── */}
      <SectionHeader icon="📦" title={t({ en: 'Resources & Publishing', zh: '资源与发布' })}/>
      <View style={styles.section}>
        <PermRow label={t({ en: '🔎 Search Resources', zh: '🔎 搜索资源' })} sub={t({ en: 'Agent can find APIs, datasets, models', zh: '智能体可搜索 API、数据集、模型' })} value={perms.resourceSearchEnabled} onChange={(v) => updatePerm('resourceSearchEnabled', v)}/>
        <View style={styles.divider}/>
        <PermRow label={t({ en: '📤 Publish Resources', zh: '📤 发布资源' })} sub={t({ en: 'Agent can publish resources to marketplace', zh: '智能体可向市场发布资源' })} value={perms.resourcePublishEnabled} onChange={(v) => updatePerm('resourcePublishEnabled', v)}/>
      </View>

      {/* Save Button */}
      <TouchableOpacity style={[styles.saveBtn, (isSaving || !activeAgent) && styles.saveBtnDisabled]} onPress={handleSave} disabled={isSaving || !activeAgent}>
      ...
      <View style={styles.dangerRow}>
        ...
        <TouchableOpacity style={[styles.dangerBtn, { borderColor: colors.error }]} onPress={() => Alert.alert(t({ en: 'Terminate Agent', zh: '终止智能体' }), t({ en: 'Permanently terminate this agent? This cannot be undone.', zh: '确认永久终止该智能体吗？此操作不可撤销。' }), [
            { text: t({ en: 'Cancel', zh: '取消' }), style: 'cancel' },

...
import { View, Text, TouchableOpacity, StyleSheet, ScrollView, TextInput, FlatList, ActivityIndicator, Alert, KeyboardAvoidingView, Platform, } from 'react-native';
...
export function AgentToolsScreen() {
    ...
    return (<View style={styles.container}>
      ...
      <View style={styles.tabBar}>
        {([
            { key: 'terminal', icon: '⌨️', label: t({ en: 'Terminal', zh: '终端' }) },
            { key: 'files', icon: '📁', label: t({ en: 'Files', zh: '文件' }) },
            { key: 'system', icon: 'ℹ️', label: t({ en: 'System', zh: '系统' }) },
        ]).map((item) => (<TouchableOpacity key={item.key} style={[styles.tabItem, tab === item.key && styles.tabItemActive]} onPress={() => setTab(item.key)}>
      ...
      {tab === 'terminal' && (<TerminalTab instanceId={instanceId} token={token}/>)}
      {tab === 'files' && (<FileBrowserTab instanceId={instanceId} token={token}/>)}
      {tab === 'system' && (<SystemInfoTab instanceId={instanceId} token={token}/>)}
    </View>);
}
// ─── Terminal Tab ────────────────────────────────────────────────────
function TerminalTab({ instanceId, token }) {
    const { t } = useI18n();
    ...
    const runCommand = useCallback(async () => {
        ...
        if (!trimmed || running)
            return;
        const inputLine = { id: `in-${Date.now()}`, type: 'input', text: `$ ${trimmed}` };
        setLines(prev => [...prev, inputLine]);
        setCmd('');
        setRunning(true);
        try {
            const res = await apiFetch(`/openclaw/proxy/${instanceId}/exec`, {
                method: 'POST',
    ...
    return (<KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={90}>
      <FlatList ref={listRef} data={lines} keyExtractor={item => item.id} style={styles.terminalOutput} contentContainerStyle={{ padding: 12 }} renderItem={({ item }) => (<Text style={[
                styles.termLine,
                item.type === 'input' && styles.termInput,
                item.type === 'error' && styles.termError,
            ]}>
            {item.text}
          </Text>)} onContentSizeChange={() => listRef.current?.scrollToEnd()}/>
      <View style={styles.termInputBar}>
        <TextInput style={styles.termTextInput} value={cmd} onChangeText={setCmd} placeholder={t({ en: 'Enter command...', zh: '输入命令…' })} placeholderTextColor={colors.textTertiary} autoCapitalize="none" autoCorrect={false} returnKeyType="send" onSubmitEditing={runCommand}/>
        <TouchableOpacity style={[styles.sendBtn, running && { opacity: 0.5 }]} onPress={runCommand} disabled={running}>
          {running ? (<ActivityIndicator size="small" color="#fff"/>) : (<Text style={styles.sendBtnText}>▶</Text>)}
        </TouchableOpacity>
      </View>
    </KeyboardAvoidingView>);
}
// ─── File Browser Tab ────────────────────────────────────────────────
function FileBrowserTab({ instanceId, token }) {
    const { t } = useI18n();
    ...
    const loadDir = useCallback(async (path) => {
        ...
        setError(null);
        try {
            const res = await apiFetch(`/openclaw/proxy/${instanceId}/files?path=${encodeURIComponent(path)}`);
    ...
    return (<View style={{ flex: 1 }}>
      ...
      <View style={styles.breadcrumb}>
        ...
        {currentPath !== '/' && (<TouchableOpacity onPress={goUp} style={styles.upBtn}>
      ...
      {loading ? (<View style={styles.centered}>
        ...
        </View>) : (<FlatList data={entries} keyExtractor={item => item.name} contentContainerStyle={{ padding: 12 }} renderItem={({ item }) => (<TouchableOpacity style={styles.fileRow} onPress={() => navigateTo(item)}>
              <Text style={styles.fileIcon}>
                {item.type === 'directory' ? '📁' : getFileIcon(item.name)}
              </Text>
              <View style={{ flex: 1 }}>
                <Text style={styles.fileName} numberOfLines={1}>{item.name}</Text>
                {item.type === 'file' && (<Text style={styles.fileMeta}>{formatSize(item.size)}</Text>)}
              </View>
              {item.type === 'directory' && <Text style={styles.fileArrow}>›</Text>}
            </TouchableOpacity>)} ListEmptyComponent={<Text style={styles.emptyText}>{t({ en: 'Empty directory', zh: '空目录' })}</Text>}/>)}
    </View>);
}
// ─── System Info Tab ─────────────────────────────────────────────────
function SystemInfoTab({ instanceId, token }) {
    const { t } = useI18n();
    ...
    useEffect(() => {
        (async () => {
            try {
                const res = await apiFetch(`/openclaw/proxy/${instanceId}/system-info`);
                setInfo(res);
            }
            catch (err) {
    ...
    return (<ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16 }}>
      {rows.map((row) => (<View key={row.label} style={styles.infoRow}>

...
export function OpenClawBindScreen() {
    ...
    const handleManualBind = async () => {
        ...
        if (!url) {
            Alert.alert('Required', 'Enter your OpenClaw instance URL');
            return;
        }
        setLoading(true);
        try {
    ...
    return (<ScrollView style={styles.container} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      <View style={styles.tabs}>
        {['manual', 'qr'].map((m) => (<TouchableOpacity key={m} style={[styles.tab, mode === m && styles.tabActive]} onPress={() => setMode(m)}>
      ...
      {mode === 'manual' && (<View style={styles.fields}>
          <Text style={styles.label}>Instance Name (optional)</Text>
          <TextInput style={styles.input} placeholder="Home Server, Work Agent..." placeholderTextColor={colors.textMuted} value={instanceName} onChangeText={setInstanceName}/>
          <Text style={styles.label}>Instance URL</Text>
          <TextInput style={styles.input} placeholder="http://localhost:3001" placeholderTextColor={colors.textMuted} value={instanceUrl} onChangeText={setInstanceUrl} autoCapitalize="none" keyboardType="url"/>
          <Text style={styles.label}>API Token (optional)</Text>
          <TextInput style={styles.input} placeholder="sk-oc-xxxxx" placeholderTextColor={colors.textMuted} value={apiToken} onChangeText={setApiToken} secureTextEntry/>
          <TouchableOpacity style={[styles.primaryBtn, loading && styles.btnDisabled]} onPress={handleManualBind} disabled={loading}>
            {loading ? <ActivityIndicator color="#fff" size="small"/> :
                <Text style={styles.primaryBtnText}>🔗 Bind Instance</Text>}
          </TouchableOpacity>
        </View>)}

      {mode === 'qr' && (<View style={styles.qrSection}>
          {qrLoading ? (<ActivityIndicator size="large" color={colors.accent} style={{ marginTop: 32 }}/>) : qrCode ? (<>
              ...
              {qrStatus === 'error' && (<TouchableOpacity style={styles.secondaryBtn} onPress={startQRSession}>

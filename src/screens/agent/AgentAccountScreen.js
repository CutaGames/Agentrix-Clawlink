...
import { View, Text, TouchableOpacity, StyleSheet, FlatList, RefreshControl, ActivityIndicator, Alert, Modal, TextInput, ScrollView, Platform, StatusBar, } from 'react-native';
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import { colors } from '../../theme/colors';
import { useI18n } from '../../stores/i18nStore';
import { fetchAgentPresenceAccounts, createAgentPresenceAccount, setAgentPresenceAccountStatus, } from '../../services/agentPresenceAccount';
...
async function createAgentAccount(dto) {
    return createAgentPresenceAccount(dto);
}
async function openWalletForAgent(agentId) {
    // Creates or retrieves the MPC wallet associated with this agent
    const res = await apiFetch('/mpc-wallet/create', {
        method: 'POST',
...
function CreateAgentModal({ visible, onClose, onCreate, loading, }) {
    const { t } = useI18n();
      ...
      <View style={modal.root}>
        <View style={modal.header}>
          ...
          <TouchableOpacity onPress={handleCreate} disabled={loading}>
            {loading ? (<ActivityIndicator color={colors.accent}/>) : (<Text style={modal.createBtn}>{t({ en: 'Create', zh: '创建' })}</Text>)}
          </TouchableOpacity>
        </View>

        <ScrollView style={modal.body} keyboardShouldPersistTaps="handled">
          <Text style={modal.label}>{t({ en: 'Agent Name *', zh: '智能体名称 *' })}</Text>
          <TextInput style={modal.input} placeholder={t({ en: 'e.g. My Research Agent', zh: '例如：我的研究智能体' })} placeholderTextColor={colors.textMuted} value={name} onChangeText={setName} autoFocus/>

          <Text style={modal.label}>{t({ en: 'Description', zh: '描述' })}</Text>
          <TextInput style={[modal.input, { minHeight: 70, textAlignVertical: 'top' }]} placeholder={t({ en: 'What does this agent do?', zh: '这个智能体负责什么？' })} placeholderTextColor={colors.textMuted} value={description} onChangeText={setDescription} multiline/>

          <Text style={modal.label}>{t({ en: 'Agent Type', zh: '智能体类型' })}</Text>
          <View style={modal.typeRow}>
            {AGENT_TYPES.map((typeKey) => (<TouchableOpacity key={typeKey} style={[modal.typeChip, agentType === typeKey && modal.typeChipActive]} onPress={() => setAgentType(typeKey)}>
          ...
          <View style={modal.limitsGrid}>
            <View style={modal.limitItem}>
              ...
              <TextInput style={modal.limitInput} value={singleTxLimit} onChangeText={setSingleTxLimit} keyboardType="decimal-pad" placeholderTextColor={colors.textMuted}/>
            </View>
            <View style={modal.limitItem}>
              <Text style={modal.limitLabel}>{t({ en: 'Daily', zh: '每日' })}</Text>
              <TextInput style={modal.limitInput} value={dailyLimit} onChangeText={setDailyLimit} keyboardType="decimal-pad" placeholderTextColor={colors.textMuted}/>
            </View>
            <View style={modal.limitItem}>
              <Text style={modal.limitLabel}>{t({ en: 'Monthly', zh: '每月' })}</Text>
              <TextInput style={modal.limitInput} value={monthlyLimit} onChangeText={setMonthlyLimit} keyboardType="decimal-pad" placeholderTextColor={colors.textMuted}/>
            </View>
          </View>
...
export function AgentAccountScreen() {
    ...
    const handleOpenWallet = async (agent) => {
        if (agent.walletAddress) {
            Alert.alert(t({ en: 'Agent Wallet', zh: '智能体钱包' }), t({ en: `Address: ${agent.walletAddress}`, zh: `地址：${agent.walletAddress}` }), [{ text: t({ en: 'OK', zh: '确定' }) }]);
            return;
        }
        Alert.alert(t({ en: 'Open Independent Wallet', zh: '打开独立钱包' }), t({ en: `Create an MPC wallet for "${agent.name}"?\n\nThis generates a self-custody wallet where only this agent can sign transactions within its spending limits.`, zh: `要为“${agent.name}”创建 MPC 钱包吗？\n\n这会生成一个自托管钱包，只有该智能体能在其支出限制内签署交易。` }), [
            { text: t({ en: 'Cancel', zh: '取消' }), style: 'cancel' },
            {
                ...
                onPress: async () => {
                    ...
                    try {
                        ...
                        queryClient.invalidateQueries({ queryKey: ['agent-accounts'] });
                        Alert.alert(t({ en: 'Wallet Created 🎉', zh: '钱包已创建 🎉' }), t({ en: `Your agent wallet is ready!\n\nAddress: ${result.walletAddress}`, zh: `你的智能体钱包已就绪！\n\n地址：${result.walletAddress}` }));
                    }
                    catch (err) {
    ...
    const handleSuspend = (agent) => {
        Alert.alert(t({ en: 'Suspend Agent', zh: '暂停智能体' }), t({ en: `Suspend "${agent.name}"? It will no longer be able to make payments.`, zh: `要暂停“${agent.name}”吗？暂停后它将无法继续支付。` }), [
            { text: t({ en: 'Cancel', zh: '取消' }), style: 'cancel' },
            {
    ...
    const handleResume = (agent) => {
        Alert.alert(t({ en: 'Resume Agent', zh: '恢复智能体' }), t({ en: `Reactivate "${agent.name}"?`, zh: `要重新激活“${agent.name}”吗？` }), [
            { text: t({ en: 'Cancel', zh: '取消' }), style: 'cancel' },
            {
    ...
    const renderAgent = ({ item: agent }) => (<View style={styles.card}>
      {/* Card header */}
      ...
      {/* Spending limits */}
      {agent.spendingLimits && (<View style={styles.limitsRow}>
      ...
      {/* Wallet */}
      {agent.walletAddress ? (<TouchableOpacity style={styles.walletRowActive} onPress={() => handleOpenWallet(agent)}>
          <Text style={styles.walletIcon}>💳</Text>
          <Text style={styles.walletAddress} numberOfLines={1}>
            {agent.walletAddress.slice(0, 14)}...{agent.walletAddress.slice(-8)}
          </Text>
          {/* Balance chip */}
          {agent.balance != null && (<View style={styles.balanceChip}>
              <Text style={styles.balanceText}>
                {agent.balance.toFixed(4)} {agent.balanceCurrency ?? 'USDT'}
              </Text>
            </View>)}
        </TouchableOpacity>) : (<TouchableOpacity style={styles.openWalletBtn} onPress={() => handleOpenWallet(agent)} disabled={walletLoading === agent.id}>
          {walletLoading === agent.id ? (<ActivityIndicator color={colors.accent} size="small"/>) : (<>
      ...
      <View style={styles.actionsRow}>
        {(agent.status === 'active' || agent.status === 'draft') && (<TouchableOpacity style={styles.actionBtn} onPress={() => handleSuspend(agent)}>
            <Text style={styles.actionBtnText}>⏸ {t({ en: 'Suspend', zh: '暂停' })}</Text>
          </TouchableOpacity>)}
        {agent.status === 'suspended' && (<TouchableOpacity style={[styles.actionBtn, styles.actionBtnResume]} onPress={() => handleResume(agent)} disabled={actionLoading === agent.id}>
            {actionLoading === agent.id ? (<ActivityIndicator color={colors.accent} size="small"/>) : (<Text style={[styles.actionBtnText, { color: colors.accent }]}>▶ {t({ en: 'Resume', zh: '恢复' })}</Text>)}
          </TouchableOpacity>)}
        {agent.walletAddress && (<TouchableOpacity style={[styles.actionBtn, styles.actionBtnFund]} onPress={() => Alert.alert(t({ en: 'Fund Agent', zh: '给智能体充值' }), t({ en: `Top up agent wallet:\n${agent.walletAddress}\n\nSend USDT (BSC) to this address.`, zh: `请向该智能体钱包充值：\n${agent.walletAddress}\n\n请向此地址发送 USDT（BSC）。` }))}>
            <Text style={[styles.actionBtnText, { color: '#22c55e' }]}>💰 {t({ en: 'Fund', zh: '充值' })}</Text>
          </TouchableOpacity>)}
        <TouchableOpacity style={styles.actionBtn} onPress={() => Alert.alert(t({ en: 'Transactions', zh: '交易记录' }), t({ en: `Transaction history for ${agent.name} will open in the Orders tab.`, zh: `${agent.name} 的交易记录会在订单页中打开。` }))}>
          <Text style={styles.actionBtnText}>📋 {t({ en: 'Txs', zh: '交易' })}</Text>
        </TouchableOpacity>
      </View>
    </View>);
    return (<View style={styles.container}>
      {isLoading ? (<ActivityIndicator color={colors.accent} style={{ marginTop: 60 }}/>) : (<FlatList data={agents} keyExtractor={(a) => a.id} renderItem={renderAgent} refreshControl={<RefreshControl refreshing={isLoading} onRefresh={refetch} tintColor={colors.accent}/>} contentContainerStyle={styles.list} showsVerticalScrollIndicator={false} ListHeaderComponent={<View style={styles.headerCard}>
      ...
      {/* Create modal */}
      <CreateAgentModal visible={showCreate} onClose={() => setShowCreate(false)} onCreate={create} loading={isCreating}/>
    </View>);
}
// ──────────────────────────────────────────────
// Styles

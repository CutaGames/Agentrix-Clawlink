...
import { View, Text, TextInput, TouchableOpacity, StyleSheet, Image, FlatList, KeyboardAvoidingView, Platform, ActivityIndicator, Alert, Modal, ScrollView, Linking, Dimensions, Animated, } from 'react-native';
...
import { ChatSessionTabs, loadSessions, saveSessions, MAX_SESSIONS } from '../../components/ChatSessionTabs';
import { uploadChatAttachment, apiFetch } from '../../services/api';
import { fetchLatestDesktopClipboard } from '../../services/desktopSync';
...
function buildOutgoingMessageContent(text, attachments) {
...
function serializeMessageForModel(message) {
...
function buildDisplayMessageText(content) {
    if (!content)
        return '';
...
const InlineAudioPlayer = ({ uri }) => {
    ...
    return (<TouchableOpacity style={styles.audioPlayerCard} onPress={togglePlay} activeOpacity={0.7}>
...
const MessageContextMenu = ({ visible, onClose, message, onQuote, onExportNote, onDeepSearch, onCrossDevice, onCopy, }) => {
    const { t } = useI18n();
    if (!visible || !message)
        return null;
    const actions = [
      ...
      <TouchableOpacity style={sf.ctxOverlay} onPress={onClose} activeOpacity={1}>
        <BlurView intensity={30} tint="dark" style={sf.ctxMenuWrap}>
          <View style={sf.ctxMenu}>
            ...
            {actions.map((a, i) => (<TouchableOpacity key={i} style={sf.ctxItem} onPress={() => { a.onPress(); onClose(); }}>
...
const ThoughtRibbon = ({ thoughts, streaming }) => {
    const { t } = useI18n();
    const [expanded, setExpanded] = useState(false);
    const shimmerAnim = useRef(new Animated.Value(0)).current;
    useEffect(() => {
        if (streaming) {
            const loop = Animated.loop(Animated.timing(shimmerAnim, { toValue: 1, duration: 2000, useNativeDriver: true }));
            loop.start();
            return () => loop.stop();
        }
    ...
    return (<View style={sf.ribbonWrap}>
      <TouchableOpacity style={sf.ribbonHeader} onPress={() => setExpanded(!expanded)} activeOpacity={0.7}>
        <View style={sf.ribbonHeaderLeft}>
          {streaming ? (<View style={sf.ribbonShimmerWrap}>
            ...
            </View>) : (<Text style={sf.ribbonDoneIcon}>⚡</Text>)}
      ...
      {expanded && (<ScrollView style={sf.ribbonBody} nestedScrollEnabled showsVerticalScrollIndicator={false}>
          {thoughts.map((thought, idx) => {
                ...
                return (<View key={idx} style={sf.ribbonStep}>
...
const MessageBubble = ({ item, onSpeak, onStopSpeaking, speakingMessageId, onPreviewImage, onQuoteMessage, onExportNote, }) => {
    const { t } = useI18n();
    ...
    const renderLeftActions = useCallback(() => (<View style={sf.swipeAction}>
      <Text style={sf.swipeActionIcon}>💬</Text>
      <Text style={sf.swipeActionLabel}>{t({ en: 'Quote', zh: '引用' })}</Text>
    </View>), [t]);
    // Swipe right → Export as note
    const renderRightActions = useCallback(() => (<View style={[sf.swipeAction, sf.swipeActionRight]}>
      <Text style={sf.swipeActionIcon}>📝</Text>
    ...
    const messageContent = (<TouchableOpacity activeOpacity={0.9} onLongPress={handleLongPress} delayLongPress={400} style={[sf.msgContainer, isUser ? sf.msgContainerUser : sf.msgContainerBot]}>
      {/* AI avatar */}
      {!isUser && (<View style={sf.botAvatarCol}>
          <LinearGradient colors={['#6C5CE7', '#a78bfa']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={sf.avatarGradient}>
      ...
      <View style={{ flex: 1, maxWidth: '100%' }}>
        ...
        {!isUser && hasThoughts && (<ThoughtRibbon thoughts={item.thoughts} streaming={item.streaming}/>)}

        {/* Main message body — borderless */}
        {(bubbleText || item.streaming) && (<View testID={`chat-message-${item.role}`} accessibilityLabel={`chat-message-${item.role}`} style={[
                sf.msgBody,
                isUser ? sf.msgBodyUser : sf.msgBodyBot,
                item.error && sf.msgBodyError,
            ]}>
            {/* Subtle gradient background for bot messages */}
            {!isUser && !item.error && (<LinearGradient colors={['rgba(108,92,231,0.06)', 'rgba(0,212,255,0.03)', 'transparent']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={[StyleSheet.absoluteFill, { borderRadius: 16 }]}/>)}

            <Text testID={`chat-message-text-${item.role}`} accessibilityLabel={`chat-message-text-${item.role}`} style={[
                sf.msgText,
            ...
            {item.streaming && (<ActivityIndicator size="small" color={colors.accent} style={{ alignSelf: 'flex-start', marginTop: 4 }}/>)}

            {/* Inline actions row */}
            {!item.streaming && (canSpeak || !!getCopyableMessageText(item)) && (<View style={sf.msgActions}>
                {canSpeak && (<TouchableOpacity style={sf.actionChip} onPress={() => (isThisMessageSpeaking ? onStopSpeaking() : onSpeak(item))}>
                    <Text style={sf.actionChipText}>
                      {isThisMessageSpeaking ? t({ en: '⏹ Stop', zh: '⏹ 停止' }) : t({ en: '▶ Play', zh: '▶ 播放' })}
                    </Text>
                  </TouchableOpacity>)}
                {!!getCopyableMessageText(item) && (<TouchableOpacity style={sf.actionChip} onPress={handleCopy}>
        ...
        {!!item.attachments?.length && (<View style={sf.mediaList}>
            {item.attachments.map((attachment) => (<TouchableOpacity key={`${item.id}-${attachment.fileName}`} style={sf.mediaCard} activeOpacity={0.8} onPress={() => attachment.isImage ? onPreviewImage(attachment.publicUrl) : openExternalUrl(attachment.publicUrl)}>
                {attachment.isImage ? (<Image source={{ uri: attachment.publicUrl }} style={sf.mediaThumb} resizeMode="cover"/>) : (<View style={sf.mediaFileIcon}><Text style={{ fontSize: 20 }}>📎</Text></View>)}
        ...
        {!!imageUrls.length && (<View style={sf.mediaList}>
            {imageUrls.map((url) => (<TouchableOpacity key={`${item.id}-${url}`} style={sf.mediaCard} activeOpacity={0.8} onPress={() => onPreviewImage(url)}>
        ...
        {!!audioUrls.length && (<View style={sf.mediaList}>
            {audioUrls.map((url) => (<InlineAudioPlayer key={`${item.id}-audio-${url}`} uri={url}/>))}
          </View>)}
        {!!fileUrls.length && (<View style={sf.mediaList}>
            {fileUrls.map((url) => (<TouchableOpacity key={`${item.id}-${url}`} style={sf.mediaCard} activeOpacity={0.8} onPress={() => openExternalUrl(url)}>
      ...
      {/* Context menu */}
      <MessageContextMenu visible={ctxVisible} onClose={() => setCtxVisible(false)} message={item} onCopy={handleCopy} onQuote={() => onQuoteMessage?.(item)} onExportNote={() => onExportNote?.(item)} onDeepSearch={() => {
            const q = item.content?.slice(0, 200);
            if (q)
                Linking.openURL(`https://www.google.com/search?q=${encodeURIComponent(q)}`).catch(() => { });
        }} onCrossDevice={async () => {
            const text = getCopyableMessageText(item);
            if (text) {
                try {
    ...
    return (<Swipeable ref={swipeableRef} renderLeftActions={renderLeftActions} renderRightActions={renderRightActions} onSwipeableOpen={(direction) => {
            if (direction === 'left')
                handleSwipeLeft();
            else
                handleSwipeRight();
...
export function AgentChatScreen() {
    ...
    const [availableModels, setAvailableModels] = useState(SUPPORTED_MODELS);
    // The ACTUAL model this agent is running on (resolved by backend)
    const [resolvedModelLabel, setResolvedModelLabel] = useState(null);
    // Per-agent preferred model (from agent account)
    const [agentPreferredModel, setAgentPreferredModel] = useState(null);
    const [agentVoiceId, setAgentVoiceId] = useState(null);
    ...
    const [chatSessions, setChatSessions] = useState(() => {
        ...
        const initial = { id: sessionIdRef.current, label: t({ en: 'New Chat', zh: '新对话' }), createdAt: Date.now() };
        return [initial];
    });
    ...
    const lastOfflineDiagnosticRef = useRef(null);
    ...
    useEffect(() => {
        if (isOffline || flushingOfflineQueueRef.current || offlineQueueRef.current.length === 0)
            return;
        let cancelled = false;
    ...
    useEffect(() => {
        (async () => {
            try {
                const models = await apiFetch('/ai-providers/available-models');
    ...
    const loadOlderMessages = useCallback(() => {
        ...
        if (all.length <= messages.length || loadingOlder)
            return;
    ...
    useEffect(() => {
        if (messages.length > 1) {
            ...
            try {
                mmkv.set(storageKey, JSON.stringify(mergedMessages.slice(-80)));
            }
            catch { }
        }
    }, [messages]);
    useEffect(() => {
        try {
            mmkv.set(draftStorageKey, input);
        }
        catch { }
    ...
    const loadHistory = async () => {
        ...
        try {
            ...
            let resp;
            try {
                resp = await fetch(`${API_BASE}/openclaw/proxy/${instanceId}/history?sessionId=${sessionIdRef.current}`, { headers: { Authorization: `Bearer ${token}` }, signal: _acH.signal });
            }
            finally {
                clearTimeout(_tH);
            }
            ...
            const historyItems = Array.isArray(data) ? data : data.messages ?? [];
            if (historyItems.length > 0) {
                const historyMessages = historyItems.map((m) => ({
    ...
    const handleCopyDesktopClipboard = useCallback(async () => {
        ...
        Haptics.selectionAsync().catch(() => { });
        Alert.alert(t({ en: 'Desktop Clipboard Copied', zh: '桌面剪贴板已复制' }), t({ en: 'The latest desktop clipboard text is now on your phone.', zh: '最新桌面剪贴板内容已复制到手机剪贴板。' }));
    }, [remoteClipboard, t]);
    ...
    const stopCurrentResponse = useCallback((showInterruptedHint = false) => {
        ...
        setMessages((prev) => prev.map((m) => {
            if (m.id !== activeMessageId || !m.streaming)
                return m;
    ...
    const appendToStreamingMessage = useCallback((msgId, chunk) => {
        setMessages((prev) => prev.map((m) => {
            if (m.id !== msgId)
                return m;
    ...
    const completeStreamingAssistantMessage = useCallback((errorMessage) => {
        const activeMessageId = activeAssistantMessageIdRef.current;
        activeAssistantMessageIdRef.current = null;
        setSending(false);
        if (!activeMessageId) {
    ...
    const beginRealtimeVoiceTurn = useCallback((text) => {
    ...
    const { voiceMode, setVoiceMode, duplexMode, setDuplexMode, voicePhase, setVoicePhase, isRecording, isSpeaking, liveListening, liveVoiceVolume, transcriptPreview, setTranscriptPreview, voiceInteractionMode, setVoiceInteractionMode, autoSpeak, setAutoSpeak, liveVoiceAvailable, liveSpeechPermissionState, realtimeConnected, sendRealtimeInterrupt, speakingMessageId, handleVoicePressIn, handleVoicePressOut, handleVoiceTapToggle, speakText, stopSpeaking, handleSpeakMessage: speakMessageText, enqueueStreamedSpeech, resetVoicePhaseAfterResponse, resumeLiveSpeech, } = useVoiceSession({
        token,
        language,
        voiceModeRequested,
        duplexModeRequested,
    ...
    const buildHistory = (msgs, newText) => [
        ...msgs
    ...
    const removePendingAttachment = useCallback((fileName) => {
    ...
    const handleSend = async (overrideText, overrideAttachments) => {
        ...
        if ((!text && attachments.length === 0) || sending || uploadingAttachment)
            return;
        // Offline: queue the message instead of streaming
        if (isOffline) {
            offlineQueueRef.current.push({ text, attachments });
            const queueMsg = { id: `user-${Date.now()}`, role: 'user', content: text, attachments, createdAt: Date.now() };
            ...
            if (instanceId) {
                await new Promise((resolve) => {
                    const ac = streamProxyChatSSE({
                        ...
                        onMeta: (meta) => {
                            if (meta.resolvedModelLabel)
                                setResolvedModelLabel(meta.resolvedModelLabel);
                        },
                        onChunk: (chunk) => {
    ...
    const openInAppCamera = useCallback(async () => {
        ...
        try {
            ...
            if (!permission?.granted) {
                Alert.alert(t({ en: 'Camera Permission Required', zh: '需要相机权限' }), t({ en: 'Allow camera access to take a photo.', zh: '请先授予相机权限后再拍照。' }));
                return;
            }
    ...
    const handleCaptureCameraPhoto = useCallback(async () => {
        if (capturingPhoto || !cameraRef.current)
            return;
        try {
        ...
        Alert.alert(t({ en: 'Start new session?', zh: '开始新会话？' }), t({ en: 'Chat history will be cleared.', zh: '当前聊天记录将被清空。' }), [
            { text: t({ en: 'Cancel', zh: '取消' }), style: 'cancel' },
            {
                ...
                onPress: () => {
                    ...
                    const newSession = { id: sessionIdRef.current, label: t({ en: 'New Chat', zh: '新对话' }), createdAt: Date.now() };
                    setChatSessions((prev) => {
    ...
    const handleSessionNew = useCallback(() => {
        ...
        const newSession = { id: newId, label: t({ en: 'New Chat', zh: '新对话' }), createdAt: Date.now() };
        streamAbortRef.current?.abort();
    ...
    const handleSessionClose = useCallback((sid) => {
        setChatSessions((prev) => {
            const updated = prev.filter(s => s.id !== sid);
            if (updated.length === 0)
                return prev; // Don't close last session
            saveSessions(instanceId, updated);
        ...
        try {
            mmkv.delete(`chat_hist_${instanceId}_${sid}`);
        }
        catch { }
    ...
    const handleExportNote = useCallback((msg) => {
    ...
    const renderMessage = ({ item }) => {
        return (<MessageBubble item={item} onSpeak={handleSpeakMessage} onStopSpeaking={stopSpeaking} speakingMessageId={speakingMessageId} onPreviewImage={(uri) => setPreviewImageUri(uri)} onQuoteMessage={handleQuoteMessage} onExportNote={handleExportNote}/>);
    };
    ...
    const handleAutoProvision = useCallback(async () => {
        ...
        try {
            const result = await apiFetch('/openclaw/auto-provision', { method: 'POST' });
            if (result?.instance) {
                const newInstance = {
    ...
    if (!activeInstance && !instanceId) {
        ...
        <View style={styles.welcomeContent}>
          ...
          <TouchableOpacity style={styles.welcomeBtn} onPress={handleAutoProvision} disabled={provisioning}>
            {provisioning ? (<ActivityIndicator color="#fff"/>) : (<Text style={styles.welcomeBtnText}>{t({ en: 'Get Started', zh: '立即开始' })}</Text>)}
          </TouchableOpacity>
          <TouchableOpacity style={styles.welcomeSecondaryBtn} onPress={() => navigation.navigate('DeploySelect')}>
            <Text style={styles.welcomeSecondaryText}>{t({ en: 'Advanced Setup', zh: '高级配置' })}</Text>
          </TouchableOpacity>
        </View>
      </SafeAreaView>);
    }
    return (<KeyboardAvoidingView testID="agent-chat-screen" accessibilityLabel="agent-chat-screen" style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={88}>
      {/* Simplified Chat toolbar */}
      <SafeAreaView edges={['top']} style={{ backgroundColor: colors.bgCard }}>
        <View style={styles.chatBar}>
          <TouchableOpacity testID="agent-chat-drawer-button" accessibilityLabel="agent-chat-drawer-button" onPress={() => { try {
        navigation.openDrawer();
    }
    catch { } }} style={styles.chatBarBackBtn}>
            <Text style={styles.chatBarBackIcon}>{'☰'}</Text>
          </TouchableOpacity>
          <Text style={styles.chatBarTitle} numberOfLines={1}>🤖 {instanceName}</Text>
          <TouchableOpacity testID="agent-chat-settings-button" accessibilityLabel="agent-chat-settings-button" onPress={() => setShowSettingsSheet(true)} style={styles.chatBarGearBtn}>
      ...
      {/* Multi-session tabs */}
      <ChatSessionTabs instanceId={instanceId} sessions={chatSessions} activeSessionId={activeSessionId} onSelect={handleSessionSelect} onNew={handleSessionNew} onClose={handleSessionClose} t={t}/>

      {loadingHistory && (<View style={styles.historyLoader}>
          <ActivityIndicator size="small" color={colors.accent}/>
          <Text style={styles.historyLoaderText}>{t({ en: 'Loading history…', zh: '正在加载历史记录…' })}</Text>
        </View>)}

      <DesktopDiscoveryBanner sessionId={sessionIdRef.current} agentId={activeInstance?.metadata?.agentAccountId || instanceId} instanceName={instanceName} messages={allMessagesRef.current.length > 0 ? allMessagesRef.current.slice(-10) : messages.slice(-10)}/>

      {isOffline && (<View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: '#f59e0b', paddingVertical: 6 }}>
          <Text style={{ color: '#fff', fontSize: 13, fontWeight: '600' }}>
            {t({ en: 'Offline — messages will be sent when reconnected', zh: '离线模式 — 恢复连接后自动发送' })}
          </Text>
        </View>)}

      {!!remoteClipboard?.text && (<View style={styles.remoteClipboardBanner}>
      ...
      <FlatList ref={flatListRef} data={messages} keyExtractor={(m) => m.id} renderItem={renderMessage} contentContainerStyle={styles.messageList} showsVerticalScrollIndicator={false} onContentSizeChange={() => scrollToBottom()} onScroll={(event) => {
            const { contentOffset, contentSize, layoutMeasurement } = event.nativeEvent;
            const distanceFromBottom = contentSize.height - (contentOffset.y + layoutMeasurement.height);
            isNearBottomRef.current = distanceFromBottom < 120;
        }} scrollEventThrottle={16} initialNumToRender={PAGE_SIZE} maxToRenderPerBatch={10} windowSize={5} onStartReached={loadOlderMessages} onStartReachedThreshold={0.3} ListHeaderComponent={allMessagesRef.current.length > messages.length ? (<TouchableOpacity onPress={loadOlderMessages} style={{ alignItems: 'center', paddingVertical: 8 }}>
      ...
      {voiceMode && (<>
          <View testID="voice-status-bar" accessibilityLabel="voice-status-bar" style={styles.voiceStatusBar}>
            <View testID="voice-session-state" accessibilityLabel={`voice-session-state:${voiceInteractionMode}:${duplexMode ? 'duplex' : 'basic'}:${voicePhase}:${realtimeConnected ? 'connected' : 'disconnected'}`} style={styles.e2eHiddenMarker}/>
            <View testID="voice-permission-state" accessibilityLabel={`voice-permission-state:${liveSpeechPermissionState}`} style={styles.e2eHiddenMarker}/>
            <View testID="chat-selected-voice" accessibilityLabel={`chat-selected-voice:${agentVoiceId || 'alloy'}`} style={styles.e2eHiddenMarker}/>
            ...
            <View style={{ flex: 1 }}>
              ...
              {!!transcriptPreview && (voicePhase === 'transcribing' || voicePhase === 'thinking') && (<Text style={styles.voiceTranscriptPreview} numberOfLines={2}>
                  {transcriptPreview}
                </Text>)}
              {liveSpeechPermissionState === 'denied' && (<Text style={styles.voiceTranscriptPreview}>
                  {t({ en: 'Microphone permission is blocked. Re-enable it to resume live voice.', zh: '麦克风权限被拒绝，恢复权限后才能继续实时语音。' })}
                </Text>)}
              {duplexMode && liveListening && (<Text style={styles.voiceTranscriptPreview} numberOfLines={1}>
                  {t({ en: `Input level ${Math.max(0, liveVoiceVolume).toFixed(1)}`, zh: `输入音量 ${Math.max(0, liveVoiceVolume).toFixed(1)}` })}
                </Text>)}
            </View>
          </View>

          {shouldShowVoiceQuickGuide && (<View style={styles.voiceQuickGuideCard}>
              ...
              <View style={styles.voiceQuickGuideActions}>
                {liveSpeechPermissionState === 'denied' && (<TouchableOpacity style={styles.voiceQuickGuideActionBtn} onPress={() => Linking.openSettings().catch(() => { })}>
      ...
      <BlurView intensity={40} tint="dark" style={sf.floatingInputWrap}>
        {/* Quoted message chip */}
        {quotedMessage && (<View style={sf.quoteChip}>
        ...
        {!!pendingAttachments.length && (<ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.pendingAttachmentRow}>
            {pendingAttachments.map((attachment) => (<View key={attachment.fileName} style={styles.pendingAttachmentChip}>
                {attachment.isImage ? (<Image source={{ uri: attachment.publicUrl }} style={styles.pendingAttachmentThumb} resizeMode="cover"/>) : (<Text style={styles.pendingAttachmentIcon}>📎</Text>)}
      ...
      {showAttachToolbar && (<View style={styles.attachToolbar}>
      ...
      <View style={styles.inputRow}>
        {voiceMode ? (duplexMode && realtimeConnected ? (
        /* Active realtime voice call — status display */
        <TouchableOpacity testID="chat-voice-action-button" accessibilityLabel={`chat-voice-action-button:call:${voicePhase}:${liveListening ? 'live' : 'idle'}`} style={[styles.holdTalkBtn, styles.holdTalkBtnCall]} onPress={() => {
                if (liveListening)
            ...
            </TouchableOpacity>) : duplexMode && !realtimeConnected ? (
        /* Connecting to realtime voice — show connecting state */
        <View style={[styles.holdTalkBtn, styles.holdTalkBtnCall]}>
              <Text style={styles.holdTalkText}>
                {t({ en: '📞  Connecting…', zh: '📞  正在连接…' })}
              </Text>
            </View>) : (
        /* Push-to-talk (hold to record) */
        <TouchableOpacity testID="chat-voice-action-button" accessibilityLabel={`chat-voice-action-button:ptt:${voicePhase}:${isRecording ? 'recording' : 'idle'}`} style={[styles.holdTalkBtn, isRecording && styles.holdTalkBtnActive]} onPressIn={handleVoicePressIn} onPressOut={handleVoicePressOut} activeOpacity={0.85}>
              <Text style={styles.holdTalkText}>
                {isRecording ? t({ en: '🔴  Release to Send', zh: '🔴  松开发送' }) : t({ en: '🎙  Hold to Talk', zh: '🎙  按住说话' })}
              </Text>
            </TouchableOpacity>)) : (
        /* Text mode */
        <TextInput testID="chat-text-input" accessibilityLabel="chat-text-input" style={styles.input} placeholder={t({ en: `Message ${instanceName}...`, zh: `给 ${instanceName} 发消息…` })} placeholderTextColor={colors.textMuted} value={input} onChangeText={setInput} onFocus={() => setShowAttachToolbar(false)} multiline maxLength={2000} returnKeyType="send" onSubmitEditing={() => handleSend()}/>)}

        {/* Attach button — always visible (both voice & text mode) */}
        <TouchableOpacity style={[styles.attachBtn, (sending || uploadingAttachment) && styles.sendBtnDisabled]} onPress={handleAttachmentAction} disabled={sending || uploadingAttachment}>
          {uploadingAttachment ? (<ActivityIndicator size="small" color={colors.textPrimary}/>) : (<Text style={styles.attachBtnIcon}>{showAttachToolbar ? '✕' : '+'}</Text>)}
        </TouchableOpacity>

        {/* Right: Send (when has text) or Mic toggle (when empty) */}
        {(input.trim().length > 0 || pendingAttachments.length > 0) && !voiceMode ? (<TouchableOpacity style={[styles.sendBtn, (sending || uploadingAttachment) && styles.sendBtnDisabled]} onPress={() => handleSend()} disabled={sending || uploadingAttachment}>
            {sending || uploadingAttachment ? (<ActivityIndicator size="small" color="#fff"/>) : (<Text style={styles.sendIcon}>⬆</Text>)}
          </TouchableOpacity>) : !voiceMode ? (<View style={{ flexDirection: 'row', alignItems: 'center' }}>
            <TouchableOpacity testID="chat-realtime-voice-btn" accessibilityLabel="chat-realtime-voice-btn" style={styles.modeToggleBtn} onPress={() => {
                if (!liveVoiceAvailable) {
                    Alert.alert(t({ en: 'Realtime Voice Unavailable', zh: '实时语音不可用' }), t({ en: 'This build does not have native live speech recognition available yet.', zh: '当前构建暂未提供原生实时语音识别能力。' }));
                    return;
                }
                setVoiceMode(true);
                setDuplexMode(true);
            ...
            <TouchableOpacity testID="chat-voice-mode-toggle" accessibilityLabel={`chat-voice-mode-toggle:text`} style={styles.modeToggleBtn} onPress={() => { setVoiceMode(true); setShowAttachToolbar(false); }}>
              <Text style={styles.modeToggleIcon}>🎤</Text>
            </TouchableOpacity>
          </View>) : (<TouchableOpacity testID="chat-voice-mode-toggle" accessibilityLabel={`chat-voice-mode-toggle:voice`} style={styles.modeToggleBtn} onPress={() => { setVoiceMode(false); setShowAttachToolbar(false); }}>
      ...
      <VoiceOnboardingTooltip visible={voiceMode} voiceInteractionMode={voiceInteractionMode} duplexMode={duplexMode} t={t}/>
      ...
      <Modal visible={showCameraModal} animationType="fade" onRequestClose={() => !capturingPhoto && setShowCameraModal(false)}>
        <View style={styles.cameraModalRoot}>
          {cameraPermission?.granted ? (<CameraView ref={cameraRef} style={StyleSheet.absoluteFill} facing="back" mode="picture"/>) : (<View style={styles.cameraPermissionState}>
          ...
          <View style={styles.cameraTopBar}>
            <TouchableOpacity style={styles.cameraCloseBtn} onPress={() => !capturingPhoto && setShowCameraModal(false)} disabled={capturingPhoto}>
          ...
          <View style={styles.cameraBottomBar}>
            ...
            <TouchableOpacity style={[styles.cameraCaptureBtn, capturingPhoto && styles.cameraCaptureBtnDisabled]} onPress={handleCaptureCameraPhoto} disabled={capturingPhoto || !cameraPermission?.granted}>
      ...
      <Modal visible={!!previewImageUri} transparent animationType="fade" onRequestClose={() => setPreviewImageUri(null)}>
        <TouchableOpacity style={styles.imagePreviewOverlay} activeOpacity={1} onPress={() => setPreviewImageUri(null)}>
          <View style={styles.imagePreviewContainer}>
            {previewImageUri && (<Image source={{ uri: previewImageUri }} style={styles.imagePreviewImage} resizeMode="contain"/>)}
          </View>
      ...
      <Modal visible={showSettingsSheet} transparent animationType="slide" onRequestClose={() => setShowSettingsSheet(false)}>
        <TouchableOpacity style={styles.modalOverlay} onPress={() => setShowSettingsSheet(false)} activeOpacity={1}>
          <View testID="chat-settings-sheet" accessibilityLabel="chat-settings-sheet" style={styles.settingsSheet}>
        ...
        }} style={[styles.sheetToggle, duplexMode && styles.sheetToggleActive]} testID="chat-duplex-toggle">
            ...
            <View style={styles.sheetRow}>
              ...
              <TouchableOpacity onPress={() => { setAutoSpeak(!autoSpeak); if (isSpeaking)
        stopSpeaking(); }} style={[styles.sheetToggle, autoSpeak && styles.sheetToggleActive]}>
            ...
            <View style={styles.sheetRow}>
              ...
              <View style={{ flexDirection: 'row', gap: 6 }}>
                {[0.8, 1.0, 1.2, 1.5].map((rate) => (<TouchableOpacity key={rate} onPress={() => setSpeechRate(rate)} style={[
                styles.sheetToggle,
            ...
            return (<TouchableOpacity key={v.id} testID={`chat-voice-persona-${v.id}`} accessibilityLabel={`chat-voice-persona-${v.id}:${isActive ? 'active' : 'inactive'}`} onPress={() => setAgentVoiceId(v.id)} style={[
                    styles.sheetToggle,
            ...
            <View style={styles.sheetRow}>
              ...
              <TouchableOpacity onPress={() => { setShowSettingsSheet(false); setTimeout(() => setShowModelPicker(true), 300); }} style={styles.sheetModelBtn}>
            ...
            {/* Agent management */}
            <TouchableOpacity style={[styles.sheetActionBtn, { backgroundColor: 'transparent', borderWidth: 1, borderColor: colors.border }]} onPress={() => { setShowSettingsSheet(false); navigation.navigate('AgentConsole'); }}>
              <Text style={[styles.sheetActionText, { color: colors.textSecondary }]}>{'⚙️ '}{t({ en: 'Agent Management', zh: '智能体管理' })}</Text>
            </TouchableOpacity>

            {/* Voice diagnostics */}
            <TouchableOpacity style={[styles.sheetActionBtn, { backgroundColor: 'transparent', borderWidth: 1, borderColor: colors.border }]} onPress={() => { setShowSettingsSheet(false); setTimeout(() => setShowDiagnostics(true), 300); }}>
      ...
      <Modal visible={showModelPicker} transparent animationType="slide">
        <TouchableOpacity style={styles.modalOverlay} onPress={() => setShowModelPicker(false)} activeOpacity={1}>
          <View style={styles.modelSheet}>
            ...
            return (<TouchableOpacity key={m.id} style={[
                    styles.modelOption,
                    ...
                    if (instanceId) {
                        try {
                            await switchInstanceModel(instanceId, m.id);
                        }
                        catch { }
                    }
                }}>
                    <View style={styles.modelOptionRow}>
                      ...
                      <View style={styles.modelOptionInfo}>
                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                          ...
                          {m.badge && (<View style={styles.modelBadge}>
              ...
              {availableModels.length <= 1 && (<Text style={{ color: colors.textMuted, textAlign: 'center', paddingVertical: 16, fontSize: 13 }}>

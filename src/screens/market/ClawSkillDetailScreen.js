...
import { View, Text, TouchableOpacity, StyleSheet, ScrollView, Alert, ActivityIndicator, Share, } from 'react-native';
...
const buildSkillShareCardParams = (skill, skillId, language, refId) => {
...
export function ClawSkillDetailScreen() {
    ...
    const reviews = reviewsData?.reviews ?? skill?.reviews ?? [];
    // ── Actions ──
    ...
    const handleInstallToAgent = useCallback(async () => {
        if (!activeInstance) {
            Alert.alert('No Agent', 'Connect an OpenClaw instance first to install skills.', [
                { text: 'Connect Agent', onPress: () => navigation.navigate('Agent', { screen: 'DeploySelect' }) },
        ...
        try {
            ...
            if (result?.dbRecorded || result?.pendingDeploy) {
                Alert.alert('✅ Installed!', `${skillName} has been added to ${activeInstance.name}.${result?.pendingDeploy ? '\n\nIt will sync automatically when the agent reconnects.' : ''}`);
            }
            else {
    ...
    const handlePromote = useCallback(async () => {
        ...
        Alert.alert('🔗 Share & Promote', `Share "${skillDisplayName}" to earn commission!`, [
            {
                text: '📱 Share via Apps',
                onPress: async () => {
                    await Share.share({
    ...
    return (<View style={styles.container}>
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        {/* ── Features ── */}
        {skill.features?.length > 0 && (<View style={styles.section}>
            <Text style={styles.sectionTitle}>✨ Features</Text>
            {skill.features.map((f, i) => (<Text key={i} style={styles.featureItem}>• {f}</Text>))}
          </View>)}

        {/* ── Tags ── */}
        {skill.tags?.length > 0 && (<View style={styles.section}>
            <Text style={styles.sectionTitle}>🏷️ Tags</Text>
            <View style={styles.tagsRow}>
              {skill.tags.map((tag) => (<View key={tag} style={styles.tag}>
        ...
        <View style={styles.section}>
          ...
          {reviews.length === 0 ? (<Text style={styles.emptyReviews}>No reviews yet. Be the first!</Text>) : (reviews.slice(0, 3).map((review) => (<View key={review.id} style={styles.reviewCard}>
          ...
          <View style={styles.reviewActions}>
            {reviews.length > 0 && (<TouchableOpacity style={styles.reviewActionBtn} onPress={() => {
                try {
                    navigation.navigate('Reviews', { skillId });
                }
                catch { }
            }}>
                <Text style={styles.reviewActionText}>View All</Text>
              </TouchableOpacity>)}
            <TouchableOpacity style={[styles.reviewActionBtn, styles.reviewActionBtnPrimary]} onPress={() => {
            try {
                navigation.navigate('WriteReview', { skillId });
            }
            catch {
                Alert.alert('Coming Soon', 'Review submission will be available soon.');
            }
        }}>
        ...
        {/* ── Commission Hint ── */}
        {commissionPerCall > 0 && (<View style={styles.commissionHint}>
      ...
      <View style={styles.bottomBar}>
        <TouchableOpacity style={[styles.buyBtn, installing && styles.btnDisabled]} onPress={price === 0 ? handleInstallToAgent : handleBuy} disabled={installing}>
          {installing ? (<ActivityIndicator color="#fff" size="small"/>) : (<Text style={styles.buyBtnText}>
              {!activeInstance
                ? '🔗 Connect Agent First'
                : price === 0
                    ? '⚡ Install (Free)'

...
function OpenClawSkillsTab() {
    ...
    const skills = data ?? FALLBACK_SKILLS;
    return (<View style={styles.tabContainer}>
      <View style={styles.searchRow}>
        <TextInput style={styles.searchInput} placeholder={t({ en: 'Search OpenClaw skills...', zh: '搜索 OpenClaw 技能...' })} placeholderTextColor={colors.textMuted} value={search} onChangeText={setSearch} returnKeyType="search"/>
      </View>
      <View>
        <FlatList data={SKILL_CATEGORIES} horizontal showsHorizontalScrollIndicator={false} keyExtractor={(c) => c} style={styles.catList} contentContainerStyle={{ paddingHorizontal: 16, gap: 8 }} renderItem={({ item }) => (<TouchableOpacity style={[styles.catChip, category === item && styles.catChipActive]} onPress={() => setCategory(item)}>
              <Text style={[styles.catText, category === item && styles.catTextActive]}>{item}</Text>
            </TouchableOpacity>)}/>
      </View>
      {isLoading ? (<ActivityIndicator size="large" color={colors.accent} style={{ marginTop: 40 }}/>) : (<FlatList data={skills} keyExtractor={(s) => String(s.id ?? s._id ?? s.name)} numColumns={2} style={{ flex: 1 }} contentContainerStyle={styles.grid} columnWrapperStyle={{ gap: 12 }} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={isFetching && !isLoading} onRefresh={refetch} tintColor={colors.accent}/>} renderItem={({ item: skill }) => (<TouchableOpacity style={styles.skillCard} activeOpacity={0.8} onPress={() => {
...
function ResourcesTab() {
    ...
    const { data, isLoading, isFetching, refetch } = useQuery({
        ...
        queryFn: async () => {
            ...
            if (category !== 'All')
                params.set('resourceType', category.toLowerCase());
            if (search)
                params.set('q', search);
            params.set('limit', '20');
            try {
                const raw = await apiFetch(`/unified-marketplace/search?${params.toString()}`);
    ...
    const resources = data ?? FALLBACK_RESOURCES;
    return (<View style={styles.tabContainer}>
      ...
      <View style={styles.searchRow}>
          <TextInput style={styles.searchInput} placeholder={t({ en: 'Search resources & goods...', zh: '搜索资源和商品...' })} placeholderTextColor={colors.textMuted} value={search} onChangeText={setSearch} returnKeyType="search"/>
      </View>
      <View>
        <FlatList data={RESOURCE_CATEGORIES} horizontal showsHorizontalScrollIndicator={false} keyExtractor={(c) => c} style={styles.catList} contentContainerStyle={{ paddingHorizontal: 16, gap: 8 }} renderItem={({ item }) => (<TouchableOpacity style={[styles.catChip, category === item && styles.catChipActive]} onPress={() => setCategory(item)}>
              <Text style={[styles.catText, category === item && styles.catTextActive]}>{item}</Text>
            </TouchableOpacity>)}/>
      </View>
      {isLoading ? (<ActivityIndicator size="large" color={colors.accent} style={{ marginTop: 40 }}/>) : (<FlatList data={resources} keyExtractor={(s) => String(s.id ?? s._id ?? s.name)} numColumns={2} style={{ flex: 1 }} contentContainerStyle={styles.grid} columnWrapperStyle={{ gap: 12 }} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={isFetching && !isLoading} onRefresh={refetch} tintColor={colors.accent}/>} renderItem={({ item: res }) => (<TouchableOpacity style={styles.skillCard} activeOpacity={0.8} onPress={() => {
...
export function ClawMarketplaceScreen() {
    ...
    return (<View style={styles.container}>
      ...
      <View style={styles.tabBar}>
        <TouchableOpacity style={[styles.tabItem, activeTab === 'skills' && styles.tabItemActive]} onPress={() => setActiveTab('skills')}>
        ...
        <TouchableOpacity style={[styles.tabItem, activeTab === 'resources' && styles.tabItemActive]} onPress={() => setActiveTab('resources')}>

...
import { View, Text, StyleSheet, FlatList, TouchableOpacity, ActivityIndicator, RefreshControl, TextInput, ScrollView, Dimensions, } from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import { useNavigation } from '@react-navigation/native';
import { colors } from '../theme/colors';
import { useI18n } from '../stores/i18nStore';
import { taskMarketplaceApi, TASK_TYPE_CONFIG, TASK_STATUS_CONFIG, } from '../services/taskMarketplace.api';
...
export function getSeedTaskById(id) {
...
export default function TaskMarketScreen() {
    ...
    const onEndReached = () => {
        if (loadingMore || tasks.length >= total)
            return;
        setLoadingMore(true);
    ...
    const renderTask = ({ item }) => {
        ...
        return (<TouchableOpacity style={styles.taskCard} activeOpacity={0.7} onPress={() => navigation.navigate('TaskDetail', { taskId: item.id })}>
        {/* Type badge + Status */}
        <View style={styles.taskTopRow}>
          ...
          {isUrgent && (<View style={styles.urgentBadge}>
              <Text style={styles.urgentText}>🔥 {tr('Urgent', '紧急')}</Text>
            </View>)}
          <View style={{ flex: 1 }}/>
          <TouchableOpacity style={styles.shareBtn} onPress={() => handleShareTask(item)} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
        ...
        <View style={styles.tagsRow}>
          {hasMilestones && (<View style={styles.milestoneBadge}>
              <Text style={styles.milestoneIcon}>🔒</Text>
              <Text style={styles.milestoneText}>{tr('Escrowed in Budget Pool', '已托管至预算池')}</Text>
            </View>)}
          {item.tags && item.tags.slice(0, 3).map((tag, i) => (<View key={i} style={styles.tagChip}>
        ...
        <View style={styles.taskBottom}>
          ...
          {daysLeft !== null && (<View style={styles.deadlineBox}>
    ...
    const handleBannerPress = useCallback(async (id) => {
        if (id !== 'share')
            return;
    ...
    return (<SafeAreaView style={styles.container} edges={['bottom']}>
      ...
      <View style={styles.bannerCarousel}>
        <ScrollView ref={bannerScrollRef} horizontal pagingEnabled showsHorizontalScrollIndicator={false} scrollEventThrottle={16} onMomentumScrollEnd={ev => {
            const idx = Math.round(ev.nativeEvent.contentOffset.x / SCREEN_W);
            setBannerPage(idx);
        }}>
          {BANNER_SLIDES.map(b => (<TouchableOpacity key={b.id} activeOpacity={b.id === 'share' ? 0.75 : 1} onPress={() => handleBannerPress(b.id)} style={[styles.bannerSlide, { backgroundColor: b.bg, borderBottomColor: b.borderColor }]}>
              <Text style={styles.bannerSlideIcon}>{b.icon}</Text>
              {b.badge ? (<View style={[styles.bannerBadge, { borderColor: b.color }]}>
        ...
        <View style={styles.bannerDots}>
          {BANNER_SLIDES.map((_, i) => (<View key={i} style={[styles.bannerDot, i === bannerPage && styles.bannerDotActive]}/>))}
        </View>
      </View>

      {/* Search Bar */}
      {showSearch && (<View style={styles.searchBar}>
          <TextInput style={styles.searchInput} placeholder={tr('Search tasks...', '搜索任务…')} placeholderTextColor={colors.muted} value={searchQuery} onChangeText={setSearchQuery} onSubmitEditing={handleSearch} returnKeyType="search" autoFocus/>
      ...
      <View style={styles.filterScrollWrapper}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filterScroll} contentContainerStyle={styles.filterContent}>
          {TYPE_FILTERS.map(f => (<TouchableOpacity key={f.key} style={[styles.filterTab, typeFilter === f.key && styles.filterTabActive]} onPress={() => setTypeFilter(f.key)}>
      ...
      <View style={styles.sortRow}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 6 }}>
          {SORT_OPTIONS.map(s => (<TouchableOpacity key={s.key} style={[styles.sortChip, sortMode === s.key && styles.sortChipActive]} onPress={() => setSortMode(s.key)}>
      ...
      {loading ? (<View style={styles.loadingContainer}>
        ...
        </View>) : (<FlatList data={tasks} renderItem={renderTask} keyExtractor={item => item.id} contentContainerStyle={styles.listContent} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={colors.primary}/>} onEndReached={onEndReached} onEndReachedThreshold={0.3} ListHeaderComponent={<View style={{ height: 8 }}/>} ListFooterComponent={loadingMore ? (<ActivityIndicator color={colors.primary} style={{ paddingVertical: 16 }}/>) : tasks.length > 0 && tasks.length >= total ? (<Text style={styles.endText}>{t({ en: `— All ${total} tasks loaded —`, zh: `— 已加载全部 ${total} 个任务 —` })}</Text>) : null} ListEmptyComponent={<View style={styles.emptyContainer}>
      ...
      {/* FAB: Post Task */}
      <TouchableOpacity style={styles.fab} activeOpacity={0.8} onPress={() => navigation.navigate('PostTask')}>
        <Text style={styles.fabIcon}>+</Text>
        <Text style={styles.fabText}>{tr('Post Task', '发布任务')}</Text>
      </TouchableOpacity>

      {/* Commission-tracked Share Bottom Sheet */}
      <ShareBottomSheet visible={shareVisible} onClose={() => setShareVisible(false)} title={tr('Share task · Earn commission', '分享任务 · 赚取佣金')} shareContent={shareContent}/>
    </SafeAreaView>);
}
const styles = StyleSheet.create({
    ...
    bannerBadgeText: { fontSize: 11, fontWeight: '800', letterSpacing: 0.5 },
    bannerSlideTexts: { flex: 1 },
    bannerSlideTitle: { fontSize: 12, fontWeight: '700', marginBottom: 1 },
    bannerSlideSub: { fontSize: 11, color: colors.muted },
    bannerArrow: { fontSize: 20, fontWeight: '700' },

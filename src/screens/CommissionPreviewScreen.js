...
export const CommissionPreviewScreen = () => {
    ...
    return (<ScrollView contentContainerStyle={{ padding: 16, gap: 16 }}>
      ...
      <Card>
        ...
        <TextInput value={amount} onChangeText={setAmount} keyboardType="numeric" style={{
        ...
        <TextInput value={currency} onChangeText={setCurrency} style={{
            backgroundColor: colors.cardAlt,
            color: colors.text,
            padding: 12,
            borderRadius: 10,
      ...
      {result ? (<Card>
          ...
          <View style={{ height: 8 }}/>
          {result.allocations?.map((a, idx) => (<Text key={idx} style={{ color: colors.text }}>
              • {a.role}: {a.amount}
            </Text>))}
        </Card>) : null}
    </ScrollView>);

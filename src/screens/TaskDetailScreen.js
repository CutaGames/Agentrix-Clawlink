...
export function TaskDetailScreen({ route, navigation }) {
    ...
    const handleSubmitBid = async () => {
        ...
        if (!budget || budget <= 0) {
            Alert.alert(tr('Please enter a valid bid amount', '请输入有效的出价金额'));
            return;
        }
        if (!days || days <= 0) {
            Alert.alert(tr('Please enter estimated days', '请输入预计天数'));
            return;
        }
        if (bidProposal.trim().length < 10) {
            Alert.alert(tr('Proposal must be at least 10 characters', '方案描述至少需要 10 个字符'));
            return;
        }
        setSubmitting(true);
        try {
    ...
    return (<KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.budgetCard}>
          ...
          {task.requirements?.deadline && (<View style={styles.budgetSide}>
        ...
        {/* Tags */}
        {task.tags && task.tags.length > 0 && (<View style={styles.section}>
            <Text style={styles.sectionTitle}>🏷️ {tr('Tags', '标签')}</Text>
            <View style={styles.tagsRow}>
              {task.tags.map((tag, i) => (<View key={i} style={styles.tagChip}>
        ...
        {/* Requirements */}
        {task.requirements?.deliverables && task.requirements.deliverables.length > 0 && (<View style={styles.section}>
            <Text style={styles.sectionTitle}>📋 {tr('Deliverables', '交付物')}</Text>
            {task.requirements.deliverables.map((d, i) => (<View key={i} style={styles.deliverableRow}>
        ...
        {isOwner && (<View style={styles.section}>
            <Text style={styles.sectionTitle}>{t({ en: `📨 Bids Received (${bids.length})`, zh: `📨 已收到竞标（${bids.length}）` })}</Text>
            {bids.length === 0 ? (<Text style={styles.noBidsText}>{tr('No bids yet, please wait...', '暂时还没有竞标，请稍候…')}</Text>) : (bids.map(bid => (<View key={bid.id} style={styles.bidCard}>
                  ...
                  {bid.status === 'pending' && (<View style={styles.bidActions}>
        ...
        {/* Bid Form */}
        {showBidForm && !isOwner && task.status === 'pending' && (<View style={styles.section}>
            ...
            <View style={styles.formRow}>
              <View style={styles.formField}>
                ...
                <TextInput style={styles.formInput} placeholder={tr('Enter bid amount', '输入报价金额')} placeholderTextColor={colors.muted} value={bidBudget} onChangeText={setBidBudget} keyboardType="decimal-pad"/>
              </View>
              <View style={styles.formField}>
                <Text style={styles.formLabel}>{tr('Est. Days', '预计天数')}</Text>
                <TextInput style={styles.formInput} placeholder={tr('Days', '天数')} placeholderTextColor={colors.muted} value={bidDays} onChangeText={setBidDays} keyboardType="number-pad"/>
              </View>
            </View>
            <Text style={styles.formLabel}>{tr('Proposal', '方案说明')}</Text>
            <TextInput style={[styles.formInput, styles.formTextarea]} placeholder={tr('Describe your approach, experience and strengths...', '请描述你的方法、经验和优势…')} placeholderTextColor={colors.muted} value={bidProposal} onChangeText={setBidProposal} multiline numberOfLines={6} textAlignVertical="top" maxLength={2000}/>
            <Text style={styles.charCount}>{bidProposal.length}/2000</Text>
            <TouchableOpacity style={[styles.submitBidBtn, submitting && { opacity: 0.5 }]} onPress={handleSubmitBid} disabled={submitting}>
      ...
      {/* Bottom CTA */}
      {!isOwner && task.status === 'pending' && !showBidForm && (<View style={styles.bottomBar}>

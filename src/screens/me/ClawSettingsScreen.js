...
import { getAndroidOverlayPermissionStatus, requestAndroidOverlayPermission, syncAndroidBackgroundWakeWordConfig, isAndroidBackgroundWakeWordAvailable, } from '../../services/androidBackgroundWakeWord.service';
import { appendLocalWakeWordSample, captureLocalWakeWordSample, getLocalWakeWordModelReadiness, getLocalWakeWordSampleCount, hasLocalWakeWordModel, runLocalWakeWordSelfCheck, thresholdFromSensitivity, } from '../../services/localWakeWord.service';
export function ClawSettingsScreen() {
    ...
    const [overlayPermissionGranted, setOverlayPermissionGranted] = useState(null);
    const localWakeWordSampleCount = useMemo(() => getLocalWakeWordSampleCount(wakeWordConfig.localModel), [wakeWordConfig.localModel]);
    const localWakeWordReadiness = useMemo(() => getLocalWakeWordModelReadiness(wakeWordConfig.localModel), [wakeWordConfig.localModel]);
    const hasLocalModel = hasLocalWakeWordModel(wakeWordConfig.localModel);
    const effectiveWakeEngine = useMemo(() => {
    ...
    const handleToggleWakeWord = () => {
        ...
        if (nextEnabled
            ...
            && !hasLocalModel) {
            Alert.alert(t({ en: 'Local sample required for Android background wake word', zh: 'Android 后台唤醒需要先录制本地样本' }), t({
    ...
    const handleRecordLocalSample = async () => {
        ...
        try {
            ...
            const nextModel = appendLocalWakeWordSample(wakeWordConfig.localModel, sample, wakeWordConfig.displayName);
            setWakeWordConfig({
                localModel: nextModel,
                engine: 'local-template',
            });
            const nextReadiness = getLocalWakeWordModelReadiness(nextModel);
            setLocalWakeWordStatus(nextReadiness.ready
                ? t({
    ...
    const handleRunLocalSelfCheck = async () => {
        if (!wakeWordConfig.localModel || !hasLocalModel) {
            Alert.alert(t({ en: 'Model required', zh: '需要先训练模型' }), t({
        ...
        try {
            const result = await runLocalWakeWordSelfCheck(wakeWordConfig.localModel, thresholdFromSensitivity(wakeWordConfig.sensitivity));
            setLocalWakeWordStatus(t({
            ...
            Alert.alert(t({ en: 'Local wake-word self-check', zh: '本地唤醒词自检' }), result.matched
    ...
    const handleClearLocalModel = () => {
        Alert.alert(t({ en: 'Clear local model', zh: '清空本地模型' }), t({ en: 'This removes all recorded wake-word samples on this device.', zh: '这会删除当前设备上的所有本地唤醒词样本。' }), [
    ...
    return (<ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.group}>
        ...
        <View style={styles.modeCard}>
          ...
          <View style={styles.modeRow}>
            {uiModes.map((mode) => (<TouchableOpacity key={mode.id} style={[styles.modeBtn, uiComplexity === mode.id && styles.modeBtnActive]} onPress={() => setUiComplexity(mode.id)}>
          ...
          {uiModes.find((m) => m.id === uiComplexity) && (<Text style={styles.modeCurrentDesc}>
      ...
      <View style={styles.group}>
        ...
        ]).map((item) => (<TouchableOpacity key={item.code} style={[styles.modeBtn, language === item.code && styles.modeBtnActive]} onPress={() => setLanguage(item.code)}>
      ...
      <View style={styles.group}>
        ...
        <View style={styles.modeCard}>
          ...
          <TouchableOpacity style={[styles.toggleRow, wakeWordConfig.enabled && styles.toggleRowActive]} onPress={handleToggleWakeWord}>
          ...
          <View style={styles.modeRow}>
            {wakeWordEngineOptions.map((option) => (<TouchableOpacity key={option.id} style={[styles.modeBtn, styles.engineBtn, wakeWordConfig.engine === option.id && styles.modeBtnActive]} onPress={() => setWakeWordConfig({ engine: option.id })}>
          ...
          <View style={styles.actionRow}>
            <TouchableOpacity style={[styles.actionBtn, localWakeWordBusy && styles.actionBtnDisabled]} onPress={() => void handleRecordLocalSample()} disabled={localWakeWordBusy}>
            ...
            <TouchableOpacity style={[styles.actionBtn, (!hasLocalModel || localWakeWordBusy) && styles.actionBtnDisabled]} onPress={() => void handleRunLocalSelfCheck()} disabled={!hasLocalModel || localWakeWordBusy}>
              <Text style={styles.actionBtnText}>{t({ en: 'Self-check', zh: '自检' })}</Text>
            </TouchableOpacity>
          </View>
          <TouchableOpacity style={[styles.secondaryBtn, !hasLocalModel && styles.actionBtnDisabled]} onPress={handleClearLocalModel} disabled={!hasLocalModel}>
            <Text style={styles.secondaryBtnText}>{t({ en: 'Clear local model', zh: '清空本地模型' })}</Text>
          </TouchableOpacity>
          {!!localWakeWordStatus && (<Text style={styles.localStatusText}>{localWakeWordStatus}</Text>)}
          ...
          <TextInput value={wakeWordConfig.displayName} onChangeText={(text) => setWakeWordConfig({ displayName: text })} placeholder={t({ en: 'Primary wake phrase, e.g. Hey Agentrix', zh: '主唤醒短语，例如 Hey Agentrix' })} placeholderTextColor={colors.textMuted} style={styles.textInput}/>

          <TextInput value={wakeWordConfig.fallbackPhrases.join(', ')} onChangeText={(text) => setWakeWordConfig({ fallbackPhrases: text.split(',').map((item) => item.trim()).filter(Boolean) })} placeholder={t({ en: 'System wake phrases, comma separated', zh: '系统唤醒短语，逗号分隔' })} placeholderTextColor={colors.textMuted} autoCapitalize="none" style={styles.textInput}/>

          <TextInput value={String(wakeWordConfig.sensitivity)} onChangeText={(text) => {
        ...
        }} placeholder="0.65" placeholderTextColor={colors.textMuted} keyboardType="decimal-pad" style={styles.textInput}/>
      ...
      {settingGroups.map((group) => (<View key={group.title} style={styles.group}>
          <Text style={styles.groupTitle}>{group.title}</Text>
          <View style={styles.groupItems}>
            {group.items.map((item, i) => (<TouchableOpacity key={item.id} style={[styles.item, i < group.items.length - 1 && styles.itemBorder]} onPress={() => {
                    if (item.id === 'notify') {
                    ...
                    if (item.id === 'logs') {
                        ...
                        Alert.alert(t({ en: 'Debug Logs', zh: '调试日志' }), diagnosticsCount > 0
      ...
      <TouchableOpacity style={styles.dangerBtn} onPress={() => Alert.alert(t({ en: 'Sign Out', zh: '退出登录' }), t({ en: 'Are you sure?', zh: '确定要退出登录吗？' }), [
            { text: t({ en: 'Cancel', zh: '取消' }), style: 'cancel' },

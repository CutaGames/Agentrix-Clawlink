...
export function ReferralDashboardScreen() {
    ...
    return (<ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      {/* Stats Row */}
      {statsLoading ? (<ActivityIndicator color={colors.accent}/>) : (<View style={styles.statsRow}>
      ...
      {linksLoading ? (<ActivityIndicator color={colors.accent}/>) : (!links?.length) ? (<Text style={styles.emptyText}>{t({ en: 'Share Agentrix-Claw to create your first link!', zh: '分享 Agentrix-Claw，创建你的第一条链接吧！' })}</Text>) : (links.map((link) => (<View key={link.id} style={styles.linkRow}>
            <View>

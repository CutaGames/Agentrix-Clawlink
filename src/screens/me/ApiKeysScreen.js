...
import { View, Text, TextInput, StyleSheet, TouchableOpacity, Alert, ScrollView, ActivityIndicator, Modal, FlatList, } from 'react-native';
...
export function ApiKeysScreen() {
    ...
    // Model picker modal
    const [modelPickerProvider, setModelPickerProvider] = useState(null);
    ...
    // ─── Helpers ──────
    const updateForm = (providerId, patch) => {
        setForms(prev => ({ ...prev, [providerId]: { ...prev[providerId], ...patch } }));
    };
    const getSaved = (providerId) => savedConfigs.find(c => c.providerId === providerId);
    ...
    const handleTest = async (provider) => {
        ...
        try {
            const res = await apiFetch('/ai-providers/test', {
                method: 'POST',
                body: JSON.stringify({
            ...
            if (res.success) {
                Alert.alert('✅ ' + t({ en: 'Connected', zh: '连接成功' }), t({ en: `Latency: ${res.latencyMs}ms`, zh: `延迟: ${res.latencyMs}ms` }));
            }
            else {
    ...
    const handleDelete = (provider) => {
        Alert.alert(t({ en: 'Remove Provider', zh: '移除厂商' }), t({ en: `Remove ${provider.name} configuration?`, zh: `确定移除 ${provider.name}？` }), [
    ...
    const renderProvider = (provider) => {
        ...
        return (<View key={provider.id} style={[styles.card, saved && styles.cardActive, isDefault && styles.cardDefault]}>
        {/* Header row */}
        <TouchableOpacity style={styles.cardHeader} onPress={() => setExpandedId(isExpanded ? null : provider.id)} activeOpacity={0.7}>
          ...
          <View style={{ flex: 1, marginLeft: 10 }}>
            ...
            {saved && (<Text style={styles.keyHint}>
                {t({ en: 'Key: ', zh: '密钥: ' })}{saved.apiKeyPrefix || '••••'}
                {saved.lastTestResult === 'success' ? '  ✅' : ''}
                {selectedModel ? `  ·  ${selectedModel.label}` : ''}
              </Text>)}
            {!saved && provider.models.some(m => m.freeApi) && (<Text style={styles.freeBadge}>
        ...
        {/* Expanded form */}
        {isExpanded && form && (<View style={styles.cardBody}>
            {/* API Key */}
            <Text style={styles.fieldLabel}>{provider.credentialLabel || 'API Key'} *</Text>
            <TextInput style={styles.input} placeholder={saved ? `${saved.apiKeyPrefix || '••••'}  (leave blank to keep)` : provider.placeholder.apiKey || 'Enter API key'} placeholderTextColor={colors.textMuted} value={form.apiKey} onChangeText={v => updateForm(provider.id, { apiKey: v })} secureTextEntry autoCapitalize="none"/>

            {(provider.id === 'bedrock' || provider.id === 'bedrock-cn') && (<Text style={styles.fieldHint}>
                {t({
            ...
            {provider.id === 'copilot-subscription' && (<View style={styles.tokenGuide}>
            ...
            {provider.id === 'chatgpt-subscription' && (<View style={styles.tokenGuide}>
                <Text style={styles.tokenGuideTitle}>
            ...
            {provider.requiredFields.includes('secretKey') && (<>
                ...
                <TextInput style={styles.input} placeholder={saved ? '(leave blank to keep)' : provider.placeholder.secretKey || 'Enter secret key'} placeholderTextColor={colors.textMuted} value={form.secretKey} onChangeText={v => updateForm(provider.id, { secretKey: v })} secureTextEntry autoCapitalize="none"/>
              </>)}

            {/* Region (Bedrock) */}
            {provider.requiredFields.includes('region') && (<>
                <Text style={styles.fieldLabel}>Region *</Text>
                <TextInput style={styles.input} placeholder={provider.placeholder.region || 'us-east-1'} placeholderTextColor={colors.textMuted} value={form.region} onChangeText={v => updateForm(provider.id, { region: v })} autoCapitalize="none"/>
              </>)}

            {/* Base URL (optional override) */}
            {provider.optionalFields.includes('baseUrl') && (<>
                <Text style={styles.fieldLabel}>Base URL ({t({ en: 'optional', zh: '可选' })})</Text>
                <TextInput style={styles.input} placeholder={provider.baseUrl || 'https://...'} placeholderTextColor={colors.textMuted} value={form.baseUrl} onChangeText={v => updateForm(provider.id, { baseUrl: v })} autoCapitalize="none" keyboardType="url"/>
              </>)}

            {/* Model selector */}
            <Text style={styles.fieldLabel}>{t({ en: 'Model', zh: '模型' })}</Text>
            <TouchableOpacity style={styles.modelSelector} onPress={() => setModelPickerProvider(provider)}>
              <View style={{ flex: 1 }}>
                <Text style={styles.modelSelectorText}>{getModelLabel(provider.id)}</Text>
                {selectedModel?.positioning && (<Text style={styles.modelSelectorSub}>{selectedModel.positioning}</Text>)}
              </View>
            ...
            <View style={styles.actionRow}>
              <TouchableOpacity style={[styles.btn, styles.btnTest]} onPress={() => handleTest(provider)} disabled={testing === provider.id}>
                {testing === provider.id
                    ? <ActivityIndicator size="small" color="#fff"/>
                    : <Text style={styles.btnText}>{t({ en: '🔌 Test', zh: '🔌 测试' })}</Text>}
              </TouchableOpacity>

              <TouchableOpacity style={[styles.btn, styles.btnSave]} onPress={() => handleSave(provider)} disabled={saving === provider.id}>
                {saving === provider.id
                    ? <ActivityIndicator size="small" color="#fff"/>
                    : <Text style={styles.btnText}>{t({ en: '💾 Save', zh: '💾 保存' })}</Text>}
              </TouchableOpacity>

              {saved && (<TouchableOpacity style={[styles.btn, styles.btnDelete]} onPress={() => handleDelete(provider)}>
                  <Text style={styles.btnText}>{t({ en: '🗑', zh: '🗑' })}</Text>
                </TouchableOpacity>)}
            </View>

            {/* Set as default / Default indicator */}
            {saved && !isDefault && (<TouchableOpacity style={styles.setDefaultBtn} onPress={() => handleSetDefault(provider.id)}>
                <Text style={styles.setDefaultText}>
                  ⭐ {t({ en: 'Set as Default for All Agents', zh: '设为所有 Agent 的默认' })}
                </Text>
              </TouchableOpacity>)}
            {isDefault && (<View style={styles.defaultIndicator}>
    ...
    return (<View style={styles.container}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.summaryCard}>
          ...
          {defaultProvider ? (<Text style={styles.summaryLine}>
            ...
            </Text>) : (<Text style={[styles.summaryLine, { color: colors.textMuted }]}>
      ...
      <Modal visible={!!modelPickerProvider} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={styles.modalSheet}>
            ...
            {modelPickerProvider && (<Text style={styles.modalCurrencyHint}>
                {t({ en: 'Prices per million tokens', zh: '价格为每百万 token' })}
                {modelPickerProvider.currency === 'CNY' ? ' (¥ CNY)' : ' ($ USD)'}
              </Text>)}
            <FlatList data={modelPickerProvider?.models || []} keyExtractor={m => m.id} renderItem={({ item: m }) => {
            const badge = COST_BADGE[m.costTier];
            const isSelected = forms[modelPickerProvider.id]?.selectedModel === m.id;
            return (<TouchableOpacity style={[styles.modelRow, isSelected && styles.modelRowSelected]} onPress={() => {
                    ...
                    <View style={{ flex: 1 }}>
                      <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 3 }}>
                        ...
                        {m.freeApi && (<View style={[styles.freeBadgePill, { marginLeft: 6 }]}>
                            <Text style={styles.freeBadgePillText}>🆓 {m.freeNote || 'Free'}</Text>
                          </View>)}
                      </View>
                      {/* Pricing */}
                      {m.inputPrice || m.outputPrice ? (<Text style={styles.modelPricing}>
                          📥 {m.inputPrice}  📤 {m.outputPrice}
                        </Text>) : m.premiumMultiplier != null && m.premiumMultiplier === 0 ? (<Text style={[styles.modelPricing, { color: '#22c55e' }]}>
                          🎫 {t({ en: '0x — no premium request used', zh: '0x — 不消耗高级请求额度' })}
                        </Text>) : m.premiumMultiplier != null && m.premiumMultiplier > 0 ? (<Text style={[styles.modelPricing, { color: m.premiumMultiplier >= 3 ? '#ef4444' : '#f59e0b' }]}>
                          🎫 {m.premiumMultiplier}x {t({ en: 'premium request', zh: '高级请求额度' })}
                        </Text>) : m.costTier === 'free' ? (<Text style={[styles.modelPricing, { color: '#22c55e' }]}>
                          {t({ en: 'Free', zh: '免费' })}
                        </Text>) : null}
                      {/* Positioning & context */}

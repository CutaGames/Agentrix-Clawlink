...
export function WalletBackupScreen() {
    ...
    const [shardA, setShardA] = useState(null);
    ...
    const handleDone = async () => {
        ...
        Alert.alert(t({ en: 'Backup Saved', zh: '备份已确认' }), t({ en: 'Your MPC recovery code has been marked as backed up.', zh: '已将你的 MPC 恢复码标记为完成备份。' }), [{ text: t({ en: 'OK', zh: '知道了' }), onPress: () => navigation.goBack() }]);
    };
    ...
    return (<SafeAreaView style={styles.safe} edges={['bottom']}>
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        {loading ? (<ActivityIndicator color={colors.accent} style={{ marginVertical: 24 }}/>) : recoveryCode ? (<>

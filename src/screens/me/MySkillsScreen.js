...
export function MySkillsScreen() {
    ...
    return (<View style={styles.container}>
      {isLoading ? (<ActivityIndicator color={colors.accent} style={{ marginTop: 40 }}/>) : (<FlatList data={skills} keyExtractor={(s) => s.id} contentContainerStyle={styles.list} ListEmptyComponent={<Text style={styles.empty}>No skills yet. Browse the market!</Text>} renderItem={({ item: skill }) => (<View style={styles.row}>

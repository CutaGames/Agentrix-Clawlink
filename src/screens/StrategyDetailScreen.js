...
export default function StrategyDetailScreen({ route, navigation }) {
    const { strategyId, strategyName, apy, riskLevel, description = '该策略通过智能合约自动执行投资操作，优化收益并控制风险。', minDeposit = 100, totalDeposited = 0, } = route.params;
    ...
    return (<SafeAreaView style={styles.container} edges={['bottom']}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Card style={styles.positionCard}>
          ...
          {myDeposit > 0 && (<View style={styles.returnsRow}>
          ...
          {!actionMode ? (<View style={styles.actionButtons}>
              <TouchableOpacity style={[styles.actionBtn, styles.depositBtn]} onPress={() => setActionMode('deposit')}>
                <Text style={styles.depositBtnText}>存入</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.actionBtn, styles.withdrawBtn, myDeposit === 0 && styles.disabledBtn]} onPress={() => setActionMode('withdraw')} disabled={myDeposit === 0}>
              ...
              <View style={styles.inputRow}>
                ...
                <TextInput style={styles.amountInput} value={amount} onChangeText={setAmount} placeholder="0.00" placeholderTextColor={colors.textSecondary} keyboardType="decimal-pad" autoFocus/>
                {actionMode === 'withdraw' && myDeposit > 0 && (<TouchableOpacity onPress={() => setAmount(myDeposit.toString())}>
              ...
              <View style={styles.confirmButtons}>
                ...
                <TouchableOpacity style={[styles.confirmBtn, loading && styles.disabledBtn]} onPress={actionMode === 'deposit' ? handleDeposit : handleWithdraw} disabled={loading}>
                  {loading ? (<ActivityIndicator size="small" color="#FFFFFF"/>) : (<Text style={styles.confirmBtnText}>
        ...
        {/* 暂停/启用按钮 */}
        <TouchableOpacity style={[styles.toggleBtn, loading && styles.disabledBtn]} onPress={handleToggleStrategy} disabled={loading}>

...
export const BudgetPoolsScreen = () => {
    ...
    return (<ScrollView contentContainerStyle={{ padding: 16, gap: 16 }}>
      ...
      <Card>
        {pools.map((pool) => (<ListItem key={pool.id} title={pool.name} subtitle={`${pool.currency} · ${pool.status}`} right={<Text style={{ color: colors.primary }}>{pool.funded}/{pool.totalBudget}</Text>}/>))}
        {!loading && pools.length === 0 ? (<View style={{ paddingVertical: 12 }}>

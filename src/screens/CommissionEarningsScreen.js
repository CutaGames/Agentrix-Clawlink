...
export function CommissionEarningsScreen() {
    ...
    const loadData = useCallback(async () => {
        try {
            const [statsData, commData] = await Promise.all([
                apiFetch('/human-commissions/stats').catch(() => MOCK_STATS),
                apiFetch('/human-commissions').catch(() => ({ items: MOCK_ITEMS })),
            ]);
            setStats(statsData);
    ...
    const renderTabs = () => (<View style={styles.tabsContainer}>
      {['all', 'pending', 'settled'].map(tab => (<TouchableOpacity key={tab} style={[styles.tab, activeTab === tab && styles.tabActive]} onPress={() => setActiveTab(tab)}>
    ...
    const renderItem = ({ item }) => {
        ...
        return (<View style={styles.commissionCard}>
        <View style={styles.cardHeader}>
          <View style={styles.cardTitleRow}>
            <Text style={styles.cardTitle}>{item.skillName}</Text>
            {item.metadata?.level === 2 && (<View style={styles.levelBadge}>
    ...
    return (<View style={styles.container}>
      <FlatList data={filteredItems} renderItem={renderItem} keyExtractor={item => item.id} ListHeaderComponent={<>
            {renderStats()}
            {renderTabs()}
          </>} ListEmptyComponent={renderEmpty} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={colors.primary}/>} contentContainerStyle={styles.listContent} showsVerticalScrollIndicator={false}/>
    </View>);
}

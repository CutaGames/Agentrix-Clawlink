...
function typeIconAndColor(type) {
    switch (type) {
...
function NotificationItem({ item, onMarkRead, }) {
    ...
    return (<TouchableOpacity style={[styles.item, !item.read && styles.itemUnread]} onPress={() => onMarkRead(item.id)} activeOpacity={0.7}>
...
export function NotificationCenterScreen() {
    ...
    const renderEmpty = () => (<View style={styles.empty}>
    ...
    return (<SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerActions}>
          {unreadCount > 0 && (<TouchableOpacity style={styles.headerBtn} onPress={markAllRead}>
              <Text style={styles.headerBtnText}>Mark all read</Text>
            </TouchableOpacity>)}
          {notifications.length > 0 && (<TouchableOpacity style={styles.headerBtn} onPress={handleClearAll}>
      ...
      {/* Badge */}
      {unreadCount > 0 && (<View style={styles.unreadBanner}>
          <Text style={styles.unreadBannerText}>{unreadCount} unread</Text>
        </View>)}

      <FlatList data={notifications} keyExtractor={(item) => item.id} renderItem={({ item }) => <NotificationItem item={item} onMarkRead={handleMarkRead}/>} ListEmptyComponent={renderEmpty} contentContainerStyle={notifications.length === 0 ? { flex: 1 } : { paddingBottom: 32 }} ItemSeparatorComponent={() => <View style={styles.separator}/>}/>
    </SafeAreaView>);
}

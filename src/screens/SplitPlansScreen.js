...
export const SplitPlansScreen = () => {
    ...
    return (<ScrollView contentContainerStyle={{ padding: 16, gap: 16 }}>
      ...
      <Card>
        {plans.map((plan) => (<ListItem key={plan.id} title={plan.name} subtitle={`${plan.productType} · ${plan.status}`} right={<Text style={{ color: colors.primary }}>{(plan.feeConfig?.splitFeeBps || 0) / 100}%</Text>}/>))}
        {!loading && plans.length === 0 ? (<View style={{ paddingVertical: 12 }}>

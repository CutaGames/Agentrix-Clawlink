...
import { View, Text, StyleSheet, TouchableOpacity, ActivityIndicator, Alert, SafeAreaView, TextInput, ScrollView, Linking, Platform, AppState, Modal, } from 'react-native';
import * as Clipboard from 'expo-clipboard';
import * as WebBrowser from 'expo-web-browser';
import { colors } from '../theme/colors';
import { useAuthStore } from '../stores/authStore';
import { useI18n } from '../stores/i18nStore';
import { getWalletNonce, walletSignatureLogin, } from '../services/walletConnect';
const WALLET_ID_ALIASES = {
    metamask: 'metamask',
    okx: 'okx',
    okex: 'okx',
...
export const WalletConnectScreen = ({ navigation, route }) => {
    const { t } = useI18n();
    ...
    const autoHandledWalletRef = useRef(null);
    ...
    useEffect(() => {
        (async () => {
            ...
            for (const def of WALLET_DEFS) {
                let installed = false;
                try {
                    installed = await Linking.canOpenURL(def.scheme);
                }
                catch { }
    ...
    useEffect(() => {
        if (detecting || step !== 'select' || !requestedWalletId)
            return;
        const normalizedId = WALLET_ID_ALIASES[requestedWalletId.toLowerCase()];
        if (!normalizedId || autoHandledWalletRef.current === normalizedId)
            return;
    ...
    // 自动登录（签名检测到后）
    const handleAutoLogin = useCallback(async (sig) => {
        const addr = walletAddress.trim();
        if (!addr || !sig || !signMessage)
            return;
        setLoading(true);
    ...
    const handleInstalledWallet = useCallback(async (wallet) => {
    ...
    // 未安装 — 引导安装流程
    const handleInstallWallet = useCallback((wallet) => {
        setSelectedWallet(wallet);
    ...
    return (<SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
        ...
        {step === 'select' && (<>
            ...
            {isAuthenticated && (<TouchableOpacity style={styles.walletSetupCard} onPress={() => navigation?.navigate?.('WalletSetup')} activeOpacity={0.8}>
            ...
            {detecting ? (<View style={styles.detectingRow}>
                <ActivityIndicator size="small" color={colors.primary}/>
                <Text style={styles.detectingText}>{tr('Detecting wallets...', '正在检测钱包…')}</Text>
              </View>) : (<>
                {/* 已安装钱包 — 一键连接（原生深链接） */}
                {installedWallets.length > 0 && (<View style={styles.section}>
                    <Text style={styles.sectionTitle}>{tr('Installed Wallets', '已安装钱包')}</Text>
                    {installedWallets.map(w => (<TouchableOpacity key={w.id} style={styles.walletCard} onPress={() => handleInstalledWallet(w)} activeOpacity={0.7} disabled={isDisabled}>
                ...
                {/* 未安装钱包 — 引导安装 */}
                {uninstalledWallets.length > 0 && (<View style={styles.section}>
                    <Text style={styles.sectionTitle}>
                      {installedWallets.length > 0 ? tr('Get a Wallet', '获取钱包') : tr('Install a Wallet to Get Started', '先安装钱包再开始')}
                    </Text>
                    {uninstalledWallets.map(w => (<TouchableOpacity key={w.id} style={[styles.walletCard, styles.walletCardDim]} onPress={() => handleInstallWallet(w)} activeOpacity={0.7}>
                ...
                <View style={styles.section}>
                  ...
                  <TouchableOpacity style={styles.walletCard} onPress={() => handleWalletConnect()} activeOpacity={0.7} disabled={isDisabled}>
                    <View style={styles.walletIconCircle}>
                ...
                <TouchableOpacity style={styles.manualBtn} onPress={() => { setSelectedWallet(null); setStep('manual'); }} activeOpacity={0.7}>
        ...
        {/* ────── STEP: INSTALL GUIDE ────── */}
        {step === 'install-guide' && selectedWallet && (<View style={styles.guideContainer}>
            ...
            <TouchableOpacity style={[styles.primaryButton, { backgroundColor: '#8B5CF6' }]} onPress={() => Linking.openURL(selectedWallet.downloadUrl)} activeOpacity={0.7}>
              <Text style={styles.primaryButtonText}>{t({ en: `Install ${selectedWallet.name}`, zh: `安装 ${selectedWallet.name}` })}</Text>
            </TouchableOpacity>

            <TouchableOpacity style={[styles.primaryButton, { marginTop: 12 }]} onPress={async () => {
                let installed = false;
                try {
                    installed = await Linking.canOpenURL(selectedWallet.scheme);
                }
                catch { }
                if (installed) {
                    ...
                    Alert.alert(tr('Not Found', '未找到'), t({ en: `${selectedWallet.name} doesn't appear to be installed yet. Install it first, then come back.`, zh: `${selectedWallet.name} 似乎尚未安装。请先完成安装后再返回。` }), [{ text: tr('OK', '确定') }]);
                }
            }} activeOpacity={0.7}>
        ...
        {step === 'connecting' && (<View style={styles.connectingContainer}>
            {selectedWallet && (<View style={styles.connectingWalletIcon}>
                <Text style={{ fontSize: 48 }}>{selectedWallet.icon}</Text>
              </View>)}
            ...
            {selectedWallet?.installed && !loading && (<TouchableOpacity style={[styles.primaryButton, { marginTop: 16 }]} onPress={() => Linking.openURL(selectedWallet.scheme)}>
            ...
            <View style={{ flexDirection: 'row', gap: 12, marginTop: 20 }}>
              ...
              <TouchableOpacity style={[styles.cancelBtn, { borderColor: colors.primary }]} onPress={() => setStep('manual')}>
        ...
        {/* ────── STEP: WALLETCONNECT ────── */}
        {step === 'walletconnect' && (<View style={styles.connectingContainer}>
            <View style={styles.connectingWalletIcon}>
        ...
        {/* ────── STEP: MANUAL INPUT ────── */}
        {step === 'manual' && (<View style={styles.section}>
            <View style={styles.header}>
            ...
            <View style={styles.inputRow}>
              <TextInput style={styles.input} placeholder="0x..." placeholderTextColor={colors.muted + '60'} value={walletAddress} onChangeText={setWalletAddress} autoCapitalize="none" autoCorrect={false} editable={!isDisabled}/>
              <TouchableOpacity style={styles.pasteBtn} onPress={async () => {
                const text = await Clipboard.getStringAsync();
                if (text)
            ...
            {!signMessage ? (<TouchableOpacity style={[styles.primaryButton, (!walletAddress.trim() || isDisabled) && styles.buttonDisabled]} onPress={handleManualSubmit} disabled={!walletAddress.trim() || isDisabled} activeOpacity={0.7}>
                {loading ? (<ActivityIndicator size="small" color="#fff"/>) : (<Text style={styles.primaryButtonText}>{tr('Get Sign Message', '获取签名消息')}</Text>)}
              </TouchableOpacity>) : (<>
                ...
                <View style={styles.inputRow}>
                  <TextInput style={[styles.input, { minHeight: 56 }]} placeholder="0x..." placeholderTextColor={colors.muted + '60'} value={signature} onChangeText={setSignature} autoCapitalize="none" autoCorrect={false} multiline editable={!isDisabled}/>
                  <TouchableOpacity style={styles.pasteBtn} onPress={async () => {
                    const text = await Clipboard.getStringAsync();
                    if (text)
                        setSignature(text.trim());
                ...
                <TouchableOpacity style={[styles.primaryButton, (!signature.trim() || isDisabled) && styles.buttonDisabled]} onPress={() => handleAutoLogin(signature.trim())} disabled={!signature.trim() || isDisabled} activeOpacity={0.7}>
                  {loading ? (<ActivityIndicator size="small" color="#fff"/>) : (<Text style={styles.primaryButtonText}>{tr('Sign In', '登录')}</Text>)}
                </TouchableOpacity>
              </>)}
      ...
      <Modal visible={showSignModal} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            {!signMessage ? (<View style={styles.modalInputRow}>
                <TextInput style={styles.modalInput} placeholder="0x..." placeholderTextColor={colors.muted + '60'} value={walletAddress} onChangeText={setWalletAddress} autoCapitalize="none" autoCorrect={false}/>
                <TouchableOpacity style={styles.modalPasteBtn} onPress={async () => {
                const text = await Clipboard.getStringAsync();
                if (text)
                    setWalletAddress(text.trim());
            }}>
              ...
              </View>) : (<View style={styles.modalInputRow}>
                <TextInput style={[styles.modalInput, { minHeight: 56 }]} placeholder={tr('Paste signature (0x...)', '粘贴签名（0x...）')} placeholderTextColor={colors.muted + '60'} value={signature} onChangeText={setSignature} autoCapitalize="none" autoCorrect={false} multiline/>
                <TouchableOpacity style={styles.modalPasteBtn} onPress={async () => {
                const text = await Clipboard.getStringAsync();
                if (text)
                    setSignature(text.trim());
            ...
            <View style={styles.modalActions}>
              <TouchableOpacity style={styles.modalCancelBtn} onPress={() => { setShowSignModal(false); resetFlow(); }}>
                <Text style={styles.modalCancelText}>{tr('Cancel', '取消')}</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.modalConfirmBtn, loading && styles.buttonDisabled]} onPress={() => { setShowSignModal(false); handleManualSubmit(); }} disabled={loading}>
                {loading ? (<ActivityIndicator size="small" color="#fff"/>) : (<Text style={styles.modalConfirmText}>

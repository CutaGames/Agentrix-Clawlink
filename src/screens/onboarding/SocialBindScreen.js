...
import { View, Text, TouchableOpacity, StyleSheet, ScrollView, ActivityIndicator, Alert, Linking, Image, } from 'react-native';
...
export function SocialBindScreen() {
    ...
    const openDeepLink = () => {
        if (deepLink) {
            Linking.openURL(deepLink).catch(() => Alert.alert('Error', 'Could not open link. Please install the app first.'));
        }
    };
    ...
    return (<ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.platformRow}>
        {PLATFORMS.map((p, idx) => (<TouchableOpacity key={idx} style={[
                styles.platformBtn,
            ...
            ]} activeOpacity={p.comingSoon ? 1 : 0.8} onPress={() => {
                if (!p.comingSoon)
            ...
            {p.comingSoon && (<View style={styles.comingSoonBadge}>
      ...
      <View style={styles.qrCard}>
        {loading ? (<View style={styles.qrPlaceholder}>
            ...
            }} style={styles.qrImage} resizeMode="contain"/>
            <Text style={styles.qrHint}>Scan with Telegram to connect your agent</Text>
            <TouchableOpacity style={styles.openLinkBtn} onPress={openDeepLink} activeOpacity={0.85}>
      ...
      <View style={styles.stepsBox}>
        ...
        {STEPS.map((s, i) => (<View key={i} style={styles.stepRow}>

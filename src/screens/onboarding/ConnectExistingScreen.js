...
import { probeInstanceUrl, migrateInstance, getDiscoveryConfig, lanScan, } from '../../services/openclaw-bridge.service';
// ─────────────────────────────────────────────────────────────────────────────
export function ConnectExistingScreen() {
    ...
    const [discoverTab, setDiscoverTab] = useState('manual');
    // Form fields
    ...
    const [boundInstanceId, setBoundInstanceId] = useState(null);
        ...
        try {
            ...
            try {
                ...
                result = {
                    ...
                    instanceName: (data.name ?? data.agentName),
                    version: data.version,
                    model: (data.model ?? data.currentModel),
                };
            }
            catch (_) {
            ...
            if (result.reachable) {
                ...
                Alert.alert('无法连接', result.error ?? '连接失败。请确认 OpenClaw 正在运行，且手机与实例在同一网络。');
            }
        }
        catch (err) {
    ...
    const startLanScan = useCallback(async () => {
        ...
        try {
            ...
            const found = await lanScan(config, '192.168.1', (checked, total) => setLanProgress(Math.round((checked / total) * 100)));
    ...
    const selectLanCandidate = useCallback((candidate) => {
    ...
    const handleBindAndMigrate = useCallback(async () => {
        ...
        try {
            ...
            const derivedName = probeResult?.instanceName ||
    ...
    return (<ScrollView style={styles.container} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      {step === 'discover' && (<View>
          ...
          <View style={styles.tabBar}>
            {['manual', 'lan'].map((t) => (<TouchableOpacity key={t} style={[styles.tab, discoverTab === t && styles.tabActive]} onPress={() => setDiscoverTab(t)}>
          ...
          {discoverTab === 'manual' && (<View>
              <Text style={styles.label}>实例地址</Text>
              <TextInput style={styles.input} placeholder="http://192.168.1.42:3001" placeholderTextColor={colors.textMuted} value={instanceUrl} onChangeText={setInstanceUrl} autoCapitalize="none" autoCorrect={false} keyboardType="url"/>
              <Text style={styles.label}>API Token（可选）</Text>
              <TextInput style={styles.input} placeholder="留空若未设置密码" placeholderTextColor={colors.textMuted} value={apiToken} onChangeText={setApiToken} autoCapitalize="none" autoCorrect={false} secureTextEntry/>
              <TouchableOpacity style={[styles.primaryBtn, loading && styles.primaryBtnDisabled]} onPress={handleProbeManual} disabled={loading}>
                {loading
          ...
          {discoverTab === 'lan' && (<View>
              <Text style={styles.sectionHint}>自动扫描本地网络，发现正在运行的 OpenClaw 实例</Text>

              {!lanScanning && lanProgress === 0 && (<TouchableOpacity style={styles.primaryBtn} onPress={startLanScan}>
              ...
              {!lanScanning && lanProgress === 100 && lanCandidates.length === 0 && (<View style={styles.emptyState}>
              ...
              {!lanScanning && lanCandidates.length > 0 && (<View>
                  <Text style={styles.sectionHint}>发现 {lanCandidates.length} 个实例：</Text>
                  {lanCandidates.map((c) => (<TouchableOpacity key={c.url} style={styles.candidateCard} onPress={() => selectLanCandidate(c)}>
                      <View style={{ flex: 1 }}>
      ...
      {step === 'verify' && probeResult && (<View>
          ...
          <View style={styles.infoCard}>
            ...
            {probeResult.skillCount !== undefined && (<InfoRow icon="⚡" label="已安装技能" value={`${probeResult.skillCount} 个`}/>)}
            {probeResult.memoryEntries !== undefined && (<InfoRow icon="💾" label="记忆条目" value={`${probeResult.memoryEntries} 条`}/>)}
            {probeResult.latencyMs !== undefined && (<InfoRow icon="⏱" label="延迟" value={`${probeResult.latencyMs} ms`}/>)}
          ...
          <TouchableOpacity style={[styles.primaryBtn, loading && styles.primaryBtnDisabled]} onPress={handleBindAndMigrate} disabled={loading}>
            {loading
                ? <ActivityIndicator color="#fff"/>
      ...
      {step === 'migrate' && (<View>
          ...
          <View style={styles.migrationStepList}>
            {[
                ...
                { icon: '💾', label: '记忆与上下文' },
                { icon: '💬', label: '对话历史' },
            ].map((item) => (<View key={item.label} style={styles.migrationStepItem}>
      ...
      {step === 'done' && (<View>
          ...
          {migrationResult && (<View style={styles.summaryGrid}>
              {[
                ...
                ].map((t) => (<View key={t.label} style={styles.summaryTile}>
          ...
          {!migrationResult && (<View style={styles.migrationNotice}>
              <Text style={styles.migrationNoticeBody}>
...
// Sub-components
// ─────────────────────────────────────────────────────────────────────────────
const STEPS = ['discover', 'verify', 'migrate', 'done'];
const STEP_LABELS = { discover: '发现', verify: '验证', migrate: '迁移', done: '完成' };

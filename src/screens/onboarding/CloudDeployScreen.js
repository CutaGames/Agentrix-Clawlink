...
export function CloudDeployScreen() {
    ...
    const handleDeploy = async () => {
        ...
        try {
            ...
            clearInterval(interval);
            setWizardStep('setup');
            Alert.alert('Deploy Failed', err?.message || 'Could not provision cloud agent. Please try again later.', [{ text: 'OK' }]);
        }
    };
    const handleFinish = () => {
    ...
    return (<ScrollView style={styles.container} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      <TextInput style={styles.input} placeholder="e.g. My Awesome Agent" placeholderTextColor={colors.textMuted} value={agentName} onChangeText={setAgentName} maxLength={32}/>

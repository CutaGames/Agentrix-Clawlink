...
export default function MyAgentsScreen() {
    ...
    const fetchAgents = async () => {
        try {
            ...
            if (result && result.length > 0) {
                ...
                const storeInstances = result.map((inst) => ({
                    id: inst.id,
                    name: inst.name || 'My Agent',
    ...
    const handleCleanupErrors = () => {
        ...
        if (errorCount === 0)
            return;
        Alert.alert('清理错误实例', `确认删除 ${errorCount} 个错误状态的 Agent 实例？`, [
    ...
    const getCategoryIcon = (category, deployType) => {
        if (deployType === 'cloud')
            return '☁️';
        if (deployType === 'local')
            return '🖥️';
    ...
    const renderAgent = ({ item }) => (<TouchableOpacity style={styles.agentCard} onPress={() => {
      ...
      <View style={styles.agentInfo}>
        ...
        {item.lastActiveAt && (<Text style={styles.lastActive}>
    ...
    return (<SafeAreaView style={styles.container} edges={['bottom']}>
      <FlatList data={agents} renderItem={renderAgent} keyExtractor={item => item.id} contentContainerStyle={styles.listContent} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={colors.primary}/>} ListHeaderComponent={<Card style={styles.summaryCard}>
            ...
            <View style={styles.summaryStats}>
              ...
              {agents.filter(a => a.status === 'error').length > 0 && (<View style={styles.statItem}>
            ...
            {agents.filter(a => a.status === 'error').length > 0 && (<TouchableOpacity style={styles.cleanupButton} onPress={handleCleanupErrors}>
                <Text style={styles.cleanupButtonText}>🗑️ 清理错误实例</Text>
              </TouchableOpacity>)}
          </Card>} ListEmptyComponent={<View style={styles.emptyContainer}>
            <Text style={styles.emptyIcon}>🤖</Text>
            <Text style={styles.emptyTitle}>还没有 Agent</Text>
            <Text style={styles.emptyText}>
              绑定或部署一个 OpenClaw 实例，开始使用 AI Agent
            </Text>
            <TouchableOpacity style={styles.createButton} onPress={() => navigation.navigate('AgentOnboarding')}>

...
const IdentityTabs = () => {
    ...
    return (<View style={styles.tabContainer}>
      {tabs.map((tab) => {
            ...
            return (<TouchableOpacity key={tab.type} style={[
                    styles.tab,
...
const IdentityContent = () => {
    ...
    if (type !== 'personal' && !status?.activated) {
        return (<LockedIdentityContent identity={type} isPending={status?.pending || false} onActivate={() => navigation.navigate('IdentityActivation', { identity: type })}/>);
    }
    // 根据身份显示不同内容
    switch (type) {
        case 'personal':

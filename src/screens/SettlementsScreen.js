...
export const SettlementsScreen = () => {
    ...
    return (<ScrollView contentContainerStyle={{ padding: 16, gap: 16 }}>
      ...
      <Card>
        {settlements.map((s) => (<ListItem key={s.id} title={`Order ${s.orderId || 'N/A'}`} subtitle={`${s.currency} · ${s.status}`} right={<Text style={{ color: colors.primary }}>{s.totalAmount}</Text>}/>))}
        {!loading && settlements.length === 0 ? (<View style={{ paddingVertical: 12 }}>

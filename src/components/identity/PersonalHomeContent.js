...
const LaunchBannerCarousel = ({ navigation }) => {
    ...
    const onScroll = (e) => {
        ...
        if (idx !== activeIdx && idx >= 0 && idx < LAUNCH_BANNERS.length)
            setActiveIdx(idx);
    };
    return (<View style={styles.bannerContainer}>
      <ScrollView ref={scrollRef} horizontal pagingEnabled showsHorizontalScrollIndicator={false} onMomentumScrollEnd={onScroll} decelerationRate="fast" snapToInterval={BANNER_W} contentContainerStyle={{ gap: 0 }}>
        {LAUNCH_BANNERS.map((b) => (<TouchableOpacity key={b.id} style={[styles.bannerCard, { width: BANNER_W }]} activeOpacity={0.85} onPress={() => navigation.navigate(b.target)}>
      ...
      <View style={styles.bannerDots}>
        {LAUNCH_BANNERS.map((b, i) => (<View key={b.id} style={[styles.bannerDot, i === activeIdx && styles.bannerDotActive]}/>))}
      </View>
    </View>);
};
export const PersonalHomeContent = () => {
    ...
    return (<View style={styles.container}>
      ...
      <Card style={styles.assetCard}>
        ...
        <View style={styles.assetList}>
          {mockAssets.topAssets.map((asset) => (<View key={asset.symbol} style={styles.assetItem}>
      ...
      <Card>
        ...
        <View style={styles.airdropList}>
          {mockAirdrops.slice(0, 2).map((airdrop) => (<TouchableOpacity key={airdrop.id} style={styles.airdropCard} onPress={() => navigation.navigate('Airdrop')}>
        ...
        <PrimaryButton title="View All Airdrops" onPress={() => navigation.navigate('Airdrop')}/>
      </Card>
      ...
      <Card>
        ...
        <PrimaryButton title="Manage Strategies" onPress={() => navigation.navigate('AutoEarn')}/>
      </Card>

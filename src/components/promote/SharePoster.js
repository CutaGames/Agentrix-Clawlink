...
export function SharePoster({ visible, onClose, skillName, skillPrice, skillPriceUnit, shareUrl, description, tags }) {
    ...
    return (<View style={styles.container}>
      ...
      <View style={styles.themeRow}>
        ...
        {THEMES.map((t, i) => (<TouchableOpacity key={i} style={[styles.themeChip, { borderColor: t.accent }, themeIdx === i && { backgroundColor: t.accent + '30' }]} onPress={() => setThemeIdx(i)}>
      ...
      <ViewShot ref={viewShotRef} options={{ format: 'png', quality: 1.0 }} style={styles.posterWrapper}>
        <View style={[styles.poster, { backgroundColor: theme.bg[0] }]}>
          ...
          {/* Description */}
          {description && (<Text style={[styles.posterDesc, { color: theme.text + 'AA' }]} numberOfLines={3}>
              {description}
            </Text>)}

          {/* Tags */}
          {tags && tags.length > 0 && (<View style={styles.posterTags}>
              {tags.slice(0, 4).map((tag, i) => (<View key={i} style={[styles.posterTag, { backgroundColor: theme.accent + '20', borderColor: theme.accent + '40' }]}>
                  <Text style={[styles.posterTagText, { color: theme.accent }]}>{tag}</Text>
                </View>))}
            </View>)}

          {/* Price */}
          {skillPrice !== undefined && (<View style={styles.priceSection}>
      ...
      <View style={styles.actionRow}>
        ...
        <TouchableOpacity style={[styles.actionBtn, styles.shareBtn]} onPress={handleShare} disabled={saving}>
          {saving ? (<ActivityIndicator color="#fff" size="small"/>) : (<Text style={styles.shareBtnText}>📤 Share Poster</Text>)}
        </TouchableOpacity>
      </View>

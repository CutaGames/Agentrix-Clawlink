...
import { startDeviceDiscovery, stopDeviceDiscovery, onDevicesChanged, getMobileDeviceId, } from '../services/deviceDiscovery.service';
import { apiFetch } from '../services/api';
export default function DesktopDiscoveryBanner({ sessionId, agentId, instanceName, messages = [] }) {
    ...
    const handleHandoff = useCallback(async (device) => {
        if (!sessionId || !agentId || handingOff)
            return;
        setHandingOff(true);
        try {
    ...
    return (<TouchableOpacity style={styles.banner} activeOpacity={0.8} onPress={() => handleHandoff(device)}>

...
import { SUPPORTED_MODELS } from '../stores/settingsStore';
...
export function SelectEngineModal({ visible, onClose, selectedModelId, onSelect }) {
    const { t } = useI18n();
            ...
            try {
                ...
                if (Array.isArray(models) && models.length > 0) {
                    setDynamicModels(models.map((m) => ({
                        ...
                        costTier: m.costTier,
                        billingType: m.billingType || 'platform',
      ...
      <TouchableOpacity style={styles.overlay} activeOpacity={1} onPress={onClose}>
        <View style={styles.content} onStartShouldSetResponder={() => true}>
          ...
          <ScrollView style={styles.modelList} nestedScrollEnabled showsVerticalScrollIndicator={false}>
            ...
            return (<TouchableOpacity key={m.id} style={[styles.modelItem, isSelected && styles.modelItemActive, { marginBottom: 12 }]} onPress={() => {
                    onSelect(m.id);
                    onClose();
                }}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 4, gap: 6 }}>
                      ...
                      {m.providerName && (<Text style={styles.providerName} numberOfLines={1}>{m.providerName}</Text>)}
                    </View>
                  </View>
                </TouchableOpacity>);
        })}

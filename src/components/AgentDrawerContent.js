...
export function AgentDrawerContent({ navigation }) {
    const { t } = useI18n();
    ...
    const navigateAndClose = useCallback((screen, params) => {
    ...
    return (<View style={styles.container}>
      <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false}>
        {/* ── Instance Switcher ── */}
        <TouchableOpacity style={styles.instanceSwitcher} onPress={() => setInstancePickerOpen(!instancePickerOpen)} activeOpacity={0.7}>
        ...
        {instancePickerOpen && (<View style={styles.instanceDropdown}>
            {instances.map((inst) => (<TouchableOpacity key={inst.id} style={[
                    styles.instanceOption,
                    inst.id === activeInstance?.id && styles.instanceOptionActive,
                ]} onPress={() => handleInstanceSelect(inst.id)} activeOpacity={0.7}>
                <View style={[
                    styles.instanceDot,
            ...
            <TouchableOpacity style={styles.addAgentBtn} onPress={handleAddAgent} activeOpacity={0.7}>
        ...
        {/* ── Status Card ── */}
        {activeInstance && (<View style={styles.statusCard}>
            ...
            <View style={styles.progressBar}>
              <View style={[
                styles.progressFill,
                {
                    ...
                    backgroundColor: tokenPercent > 85 ? '#ef4444' : tokenPercent > 60 ? '#F59E0B' : '#22c55e',
                },
            ]}/>
            </View>
        ...
        ].map((item) => (<TouchableOpacity key={item.screen} style={styles.menuItem} onPress={() => navigateAndClose(item.screen, item.params)} activeOpacity={0.7}>
        ...
        ].map((item) => (<TouchableOpacity key={item.screen} style={styles.menuItem} onPress={() => navigateAndClose(item.screen)} activeOpacity={0.7}>
            <Text style={styles.menuIcon}>{item.icon}</Text>
            <Text style={styles.menuLabel}>{item.label}</Text>
            <Text style={styles.menuArrow}>❯</Text>
          </TouchableOpacity>))}
        ...
        {/* ── Full Console Link ── */}
        <TouchableOpacity style={[styles.menuItem, styles.consoleLink]} onPress={() => navigateAndClose('AgentConsole')} activeOpacity={0.7}>

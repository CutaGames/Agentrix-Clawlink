...
import { isLiveSpeechRecognitionAvailable, requestLiveSpeechPermissions, startLiveSpeechRecognition, } from '../services/liveSpeech.service';
...
import { RealtimeVoiceService } from '../services/realtimeVoice.service';
...
function isVoiceUiE2EEnabled() {
    if (typeof window === 'undefined')
        return false;
    try {
        const params = new URLSearchParams(window.location?.search || '');
        if (params.get('e2e') === 'voice-ui')
            return true;
    }
    catch { }
    return !!window.__AGENTRIX_VOICE_UI_E2E_BOOTSTRAPPED__;
}
...
export function useVoiceSession(options) {
    const { token, language, voiceModeRequested, duplexModeRequested, agentVoiceId, instanceName, instanceId, isSending, onSendMessage, onRealtimeUserMessage, onRealtimeAssistantChunk, onRealtimeAssistantResponseEnd, onRealtimeError, onStopCurrentResponse, t, useRealtimeChannel, realtimeModelId, speechRate, } = options;
    const isVoiceUiE2E = isVoiceUiE2EEnabled();
    // ── State ──
    const [voiceMode, setVoiceMode] = useState(!!voiceModeRequested);
    const [voiceInteractionMode, setVoiceInteractionMode] = useState(duplexModeRequested ? 'tap' : 'hold');
    ...
    const [liveSpeechPermissionState, setLiveSpeechPermissionState] = useState('unknown');
    ...
    const realtimeMicrophoneRef = useRef(null);
    ...
    const persistRealtimeAudioChunk = useCallback(async (audioBase64, format) => {
    ...
    const resumeLiveSpeechFn = useCallback(() => {
        if (!duplexModeRef.current || sendingRef.current)
            return;
    ...
    const speakText = useCallback((text) => {
        if (!text || text.startsWith('⚠️') || text.startsWith('Error:'))
            return;
    ...
    const handleSpeakMessage = useCallback((text, messageId) => {
        if (!text)
            return;
        if (messageId)
    ...
    const enqueueStreamedSpeech = useCallback((chunk, flush = false) => {
        ...
        if (!(duplexModeRef.current || autoSpeak))
            return;
        if (duplexModeRef.current) {
            stopLiveSpeech(true, false);
        }
        ...
        const shouldEarlyFlush = !matches.length &&
    ...
    const startLiveSpeechInternal = useCallback(async () => {
        ...
        try {
            ...
            if (!permission?.granted) {
                setLiveSpeechPermissionState('denied');
                addVoiceDiagnostic('voice-session', 'live-speech-permission-denied');
                Alert.alert(t({ en: 'Speech Permission', zh: '语音权限' }), t({ en: 'Realtime voice needs microphone and speech recognition permissions.', zh: '实时语音需要麦克风和语音识别权限。' }));
                return;
            }
            setLiveSpeechPermissionState('granted');
            liveSpeechManualStopRef.current = false;
            lastLiveFinalTranscriptRef.current = '';
            liveSpeechRef.current = startLiveSpeechRecognition(voiceLanguageHint, {
                onStart: () => {
                    if (!isMountedRef.current)
                        return;
    ...
    useEffect(() => {
        ...
        window.addEventListener('agentrix:e2e-live-speech-permission', handlePermissionChange);
        return () => {
            window.removeEventListener('agentrix:e2e-live-speech-permission', handlePermissionChange);
        };
    ...
    const startVoiceRecording = useCallback(async () => {
        ...
        try {
            if (!isRecordingRef.current) {
                ...
                const { recording } = await Audio.Recording.createAsync(voiceRecordingOptions || Audio.RecordingOptionsPresets.HIGH_QUALITY);
    ...
    const stopVoiceRecording = useCallback(async () => {
        if (!Audio || !isRecordingRef.current)
            return;
        try {
            // Stop VAD monitoring
            vadRef.current?.stop();
    ...
    const resumeLiveSpeech = useCallback(() => {
        if (!duplexModeRef.current || sendingRef.current || isSpeakingRef.current || !voiceModeRef.current)
            return;
        void startLiveSpeechInternal();

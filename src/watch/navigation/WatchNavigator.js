...
export function WatchNavigator() {
    ...
    return (<View style={styles.container}>
      <ScrollView ref={scrollRef} horizontal pagingEnabled showsHorizontalScrollIndicator={false} onMomentumScrollEnd={onMomentumEnd} style={styles.pager}>
        {SCREENS.map(({ key, component: Screen }) => (<View key={key} style={styles.page}>
            <Screen />
          </View>))}
      </ScrollView>
      ...
      <View style={styles.dots}>
        {SCREENS.map((s, i) => (<View key={s.key} style={[styles.dot, i === activeIdx && styles.dotActive]}/>))}
      </View>
    </View>);
}

...
export function WatchHomeScreen() {
    ...
    const fetchStatus = useCallback(async () => {
        try {
            ...
            const unread = alertRes.status === 'fulfilled' && alertRes.value.ok
    ...
    return (<ScrollView style={styles.container} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      {state.lastMessage ? (<View style={styles.messageCard}>
          <Text style={styles.messageLabel}>Agent</Text>
          <Text style={styles.messageText} numberOfLines={2}>
            {state.lastMessage}
          </Text>
        </View>) : (<TouchableOpacity style={styles.messageCard}>
          <Text style={styles.messagePlaceholder}>← 滑动开始对话</Text>
        </TouchableOpacity>)}

      {/* Alerts Badge */}
      {state.unreadAlerts > 0 && (<View style={styles.alertBadge}>

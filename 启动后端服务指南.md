# 启动 Agentrix 后端服务指南

## 🔍 当前状态

- ✅ 端口3001未被占用（服务未运行）
- ❌ curl无输出（服务未运行）

## 🚀 启动步骤

### 步骤1: 进入后端目录

```bash
cd backend
```

### 步骤2: 检查依赖

```bash
# 如果node_modules不存在，需要安装
if [ ! -d "node_modules" ]; then
    echo "安装依赖..."
    npm install
fi
```

### 步骤3: 检查环境配置

后端需要以下环境变量（可选，有默认值）：

```bash
# 检查.env文件是否存在
ls -la .env

# 如果没有.env文件，可以创建（使用默认值）
# 或者直接启动，使用代码中的默认值
```

**默认配置**（如果.env不存在）：
- 数据库: `localhost:5432`
- 数据库用户: `agentrix`
- 数据库密码: `agentrix_password`
- 数据库名: `agentrix_db`
- 端口: `3001`

### 步骤4: 启动服务

```bash
npm run start:dev
```

### 步骤5: 等待启动完成

等待30-60秒，看到以下输出表示成功：

```
🚀 Agentrix Backend is running on: http://0.0.0.0:3001
📚 API Documentation: http://0.0.0.0:3001/api/docs
```

---

## 🐛 常见启动错误

### 错误1: 数据库连接失败

**错误信息**: `Unable to connect to the database`

**解决方案**:

#### 选项A: 使用默认PostgreSQL配置

```bash
# 检查PostgreSQL是否运行
pg_isready -h localhost -p 5432

# 如果未运行，启动PostgreSQL
sudo service postgresql start

# 创建数据库和用户（如果需要）
sudo -u postgres psql
```

在psql中执行：
```sql
CREATE USER agentrix WITH PASSWORD 'agentrix_password';
CREATE DATABASE agentrix_db OWNER agentrix;
GRANT ALL PRIVILEGES ON DATABASE agentrix_db TO agentrix;
\q
```

#### 选项B: 修改.env文件使用现有PostgreSQL

创建 `backend/.env`:
```env
DB_HOST=localhost
DB_PORT=5432
DB_USERNAME=postgres
DB_PASSWORD=postgres
DB_DATABASE=agentrix
```

### 错误2: 端口被占用

**错误信息**: `EADDRINUSE: address already in use :::3001`

**解决方案**:
```bash
# 查找占用端口的进程
lsof -i :3001

# 杀死进程
kill -9 <PID>

# 或使用不同端口
PORT=3002 npm run start:dev
```

### 错误3: 依赖缺失

**错误信息**: `Cannot find module 'xxx'`

**解决方案**:
```bash
rm -rf node_modules package-lock.json
npm install
```

### 错误4: TypeScript编译错误

**错误信息**: 编译失败

**解决方案**:
```bash
# 查看详细错误
npm run build

# 修复错误后重新启动
npm run start:dev
```

---

## ✅ 验证服务已启动

### 方法1: 检查端口

```bash
lsof -i :3001
```

应该看到类似输出：
```
node    12345  user   23u  IPv6  ...  TCP *:3001 (LISTEN)
```

### 方法2: 测试API

```bash
curl http://localhost:3001/api
```

应该返回JSON响应。

### 方法3: 测试Swagger文档

```bash
curl http://localhost:3001/api/docs
```

应该返回HTML页面。

### 方法4: 浏览器访问

打开浏览器：
- http://localhost:3001/api/docs

---

## 📋 完整启动命令（一键执行）

```bash
cd backend

# 检查并安装依赖
if [ ! -d "node_modules" ]; then
    npm install
fi

# 启动服务
npm run start:dev
```

---

## 🔧 如果数据库连接失败但想先启动服务

可以临时禁用数据库连接检查（仅用于测试）：

**注意**: 这会导致数据库相关功能不可用，但服务可以启动。

修改 `backend/src/app.module.ts`，临时注释TypeORM：

```typescript
// 临时注释
// TypeOrmModule.forRootAsync({
//   useClass: DatabaseConfig,
// }),
```

**不推荐**，最好先配置好数据库。

---

## 📞 需要帮助？

如果启动失败，请提供：

1. **完整错误日志**（从npm run start:dev的输出）
2. **数据库状态**（`pg_isready -h localhost -p 5432`的结果）
3. **环境信息**（`node --version`, `npm --version`）

---

**下一步**: 运行 `npm run start:dev` 启动服务


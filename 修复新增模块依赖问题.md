# 修复新增模块依赖问题

## 🔍 问题分析

新增的模块可能导致启动错误，主要包括：

1. **MerchantModule** - 商户自动化模块
2. **IntegrationsModule** - 集成服务模块
3. **AutoEarnModule** - Auto-Earn模块（包含高级功能）
4. **MarketplaceModule** - Marketplace模块（包含Agent Marketplace）

## ✅ 检查清单

### 1. 实体文件检查

所有必需的实体文件都已存在：
- ✅ `marketing-campaign.entity.ts`
- ✅ `conversation-history.entity.ts`
- ✅ `agent-stats.entity.ts`
- ✅ `strategy-config.entity.ts`

### 2. 模块导入检查

所有模块都已正确导入到 `app.module.ts`：
- ✅ `MerchantModule`
- ✅ `IntegrationsModule`
- ✅ `AutoEarnModule`
- ✅ `MarketplaceModule`

### 3. 依赖注入检查

所有服务都正确使用 `@InjectRepository` 注入实体：
- ✅ `MerchantAutoOrderService`
- ✅ `MerchantAICustomerService`
- ✅ `MerchantAutoMarketingService`
- ✅ `StrategyService`
- ✅ `AgentMarketplaceService`

## 🐛 可能的启动错误

### 错误1: 数据库表不存在

**症状**: 
```
relation "marketing_campaigns" does not exist
relation "conversation_histories" does not exist
relation "agent_stats" does not exist
relation "strategy_configs" does not exist
```

**解决方案**: 运行数据库迁移

```bash
cd backend
npm run migration:run
```

### 错误2: 循环依赖

**症状**:
```
Nest can't resolve dependencies of the StrategyService
```

**解决方案**: 检查 `StrategyService` 的依赖注入

`StrategyService` 依赖 `ArbitrageService` 和 `LaunchpadService`，但这些服务都在同一个模块中，应该没问题。

如果出现循环依赖，需要：
1. 使用 `forwardRef()` 解决
2. 或者重构服务依赖关系

### 错误3: 实体未注册

**症状**:
```
EntityMetadataNotFoundError: No metadata for "MarketingCampaign"
```

**解决方案**: 确保实体在模块中正确注册

检查 `MerchantModule`:
```typescript
TypeOrmModule.forFeature([Payment, Product, Coupon, MarketingCampaign, ConversationHistory])
```

检查 `AutoEarnModule`:
```typescript
TypeOrmModule.forFeature([UserAgent, AutoEarnTask, Airdrop, StrategyConfig])
```

检查 `MarketplaceModule`:
```typescript
TypeOrmModule.forFeature([MarketplaceAsset, AssetSource, UserAgent, AgentStats])
```

## 🔧 快速修复步骤

### 步骤1: 运行数据库迁移

```bash
cd backend
npm run migration:run
```

### 步骤2: 检查编译

```bash
npm run build
```

### 步骤3: 启动服务

```bash
npm run start:dev
```

### 步骤4: 如果仍有错误

查看具体错误信息，可能是：
- 实体导入路径错误
- 服务依赖注入错误
- 模块导入顺序问题

## 📋 验证清单

- [ ] 所有实体文件存在
- [ ] 所有模块正确导入到 `app.module.ts`
- [ ] 所有实体在模块中正确注册
- [ ] 数据库迁移已执行
- [ ] 无编译错误
- [ ] 无循环依赖

---

**如果启动失败，请提供完整的错误日志。**


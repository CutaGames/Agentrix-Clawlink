...
查找以下信息：
- ✅ `🚀 Agentrix Backend is running on: http://localhost:3001`
...
```bash
# 测试数据库连接
PGPASSWORD=postgres psql -h localhost -U postgres -d agentrix -c "SELECT 1;"
...
2. **数据库**: 确保PostgreSQL正在运行且数据库 `agentrix` 已创建

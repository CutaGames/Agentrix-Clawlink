# Agentrix SDK for JavaScript/TypeScript

Official Agentrix SDK for JavaScript and TypeScript. Unified payment layer for AI Agents, merchants, and applications.
...
const agentrix = new Agentrix({
  ...
  baseUrl: 'https://api.agentrix.com/api', // Optional, defaults to production
});
...
const grant = await agentrix.agents.createAutoPayGrant({
  agentId: 'agent_123',
  singleLimit: 100,
  dailyLimit: 1000,
  currency: 'USD',
...
// Get agent earnings
const earnings = await agentrix.agents.getEarnings('agent_123');

// Get commissions
const commissions = await agentrix.agents.getCommissions('agent_123');
```
...
// Cancel subscription
await agentrix.subscriptions.cancel(subscription.id);
```
...
// Settle commission
await agentrix.commissions.settle(commission.id);
```
...
const stats = await agentrix.tips.getCreatorStats('creator_123');
```
...
const purchases = await agentrix.gaming.purchaseBatch([
...
// Poll payment status
const payment = await agentrix.payments.pollStatus('pay_123', {
  interval: 2000,
  timeout: 60000,
...
const results = await agentrix.agents.searchProducts(
  '适合跑步的鞋子，不要超过150美元',
  {
    priceMax: 150,
    currency: 'USD',
...
const results = await agentrix.marketplace.search('running shoes', {
...
const recommended = await agentrix.marketplace.getRecommendedProducts('agent_123');
```
...
app.post('/webhook', express.raw({ type: 'application/json' }), (req, res) => {
  try {
    const event = agentrix.webhooks.constructEvent(
      req.body,
      req.headers['agentrix-signature'] as string
    );
...
```typescript
import { AgentrixSDKError, AgentrixAPIError } from '@agentrix/sdk';

try {
  const payment = await agentrix.payments.create({...});
} catch (error) {
  if (error instanceof AgentrixAPIError) {
    console.error('API Error:', error.code, error.message);
    console.error('Status Code:', error.statusCode);
  } else if (error instanceof AgentrixSDKError) {
...
const agentrix = new Agentrix({
  apiKey: 'your-api-key',
  baseUrl: 'https://api.agentrix.com/api', // Optional
...
- Documentation: https://docs.agentrix.com
- Issues: https://github.com/agentrix/sdk-js/issues

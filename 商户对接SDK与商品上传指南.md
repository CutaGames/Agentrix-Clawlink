...
const agentrix = new Agentrix({
  ...
  baseUrl: 'https://api.agentrix.com/api', // 可选，默认生产环境
});
```
...
// 获取订单详情
const order = await agentrix.merchants.getOrder(orderId);
```
...
```typescript
// 使用Marketplace资源类
const product = await agentrix.marketplace.createProduct({
...
```typescript
// 创建NFT商品
const nftProduct = await agentrix.merchants.createProduct({
  name: 'Digital Art NFT #123',
